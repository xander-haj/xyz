# =============================================================================
# tables.py — Central Lookup Table Module for zelda3 Asset Pipeline
# =============================================================================
# Part of the zelda3 project: a C reimplementation of The Legend of Zelda:
# A Link to the Past (SNES). This file lives in the assets/ directory and
# provides ALL name-to-index and index-to-name mappings used when converting
# between the ROM's raw numeric data and the human-readable YAML/text
# intermediate files that the asset pipeline produces.
#
# The SNES ROM stores dungeon room objects, sprites, music tracks, palettes,
# and other game data as numeric indices. This module assigns a descriptive
# string name to each index so that the extracted YAML files are readable
# and editable by humans. Reverse-lookup dicts (suffixed with _rev) allow
# encoding those names back into numeric form when reassembling ROM data.
#
# Major data sections in this file:
#   - type0_names / type1_names / type2_names : dungeon room object names
#   - kTagNames          : room puzzle trigger descriptions
#   - kEffectNames       : room visual effect types
#   - kCollisionNames    : BG layer collision modes
#   - kBg2               : background layer 2 rendering modes
#   - kMusicNames        : music track name mappings
#   - kAmbientSoundName  : ambient/environmental sound mappings
#   - kPalaceNames       : dungeon palace identifiers
#   - kAreaNames         : all 161 overworld area names
#   - kEntranceNames     : all 133 dungeon/cave entrance descriptions
#   - kSpriteNames       : all 284 sprite type names
#   - kSecretNames       : hidden item drop / secret type names
#   - kCompSpritePtrs / kCompBgPtrs : ROM addresses for compressed tilesets
#   - kSpriteTilesets     : VRAM graphic sheet assignments per sprite set
#   - kDungPalinfos       : dungeon palette assignment table
#   - kOwSprPalInfo       : overworld sprite palette info
#   - kSpriteInit_Flags3  : sprite initialization flags (palette base, etc.)
# =============================================================================


# -----------------------------------------------------------------------------
# Utility: Invert a dict so values become keys and keys become values.
# Used to build reverse-lookup dicts (name -> index) from forward dicts
# (index -> name). This enables encoding human-readable names back into
# the ROM's numeric indices.
# -----------------------------------------------------------------------------
def invert_dict(xs):
  return {s:i for i,s in xs.items()}

# =============================================================================
# DUNGEON ROOM OBJECT NAMES — Type 0 (Subtype 0)
# =============================================================================
# The SNES ROM encodes dungeon room objects using three different subtype
# formats, each with its own index range and encoding scheme:
#
#   Type 0 (indices 0x00–0xF7): "Standard" objects drawn with L-R (horizontal),
#     U-D (vertical), or 4-way (rectangular) expansion. The suffix in brackets
#     indicates the draw direction: [L-R] = left-to-right, [U-D] = up-to-down,
#     [4-way] = expands in all four directions, [NW]/[SE]/etc. = corner piece.
#
#   Type 1 (indices 0xF80+): Extended objects that reuse the same name list
#     starting at offset 0xF8 in type0_names. These use a different ROM
#     encoding (12-bit format) for objects that did not fit in Type 0's range.
#
#   Type 2 (indices 0x100+): "Special" objects like wall corners, furniture,
#     doors, stairs, and interactive elements. Defined in a separate list.
#
# Naming conventions in the object names:
#   [N]/[S]/[E]/[W] = which wall or edge the object attaches to (North, etc.)
#   (HIGH)/(LOW)    = vertical layer position (high wall vs. low wall)
#   [Trans]         = translucent/transparent rendering
#   [BG2 X-RAY]    = drawn on background layer 2, used for X-ray scope
#   (Layer)/(Lower) = which BG layer or priority the object is placed on
#   N/A             = unused slot in the ROM's object table
...
# =============================================================================
# DUNGEON ROOM OBJECT NAMES — Type 2 (Special Objects)
# =============================================================================
# Type 2 objects (index 0x100+) are "special" dungeon objects: wall corners,
# S-bends, furniture, interactive elements (chests, switches, stairs, doors),
# pipe ride segments, and boss-specific room fixtures. Unlike Type 0/1 objects
# which are primarily wall/floor/ceiling tiles drawn in a direction, Type 2
# objects are usually fixed-size decorations or interactive game elements.
# =============================================================================
...
# -----------------------------------------------------------------------------
# Formatted name lists with hex index prefixes.
# These are the "display names" used in YAML output, e.g. "0A-/ Wall Wood Bot (HIGH) [NW]".
# The hex prefix matches the ROM's object index so each name is both human-readable
# and traceable back to the exact ROM address.
#   kType0Names: 2-digit hex prefix (00–F7), 248 standard objects
#   kType1Names: 3-digit hex prefix (F80–FFF), 128 extended objects
#   kType2Names: 3-digit hex prefix (100+), special objects
# -----------------------------------------------------------------------------
...
# Inverts a list into a dict mapping each element (name string) to its list index.
# Used for reverse lookups: given a formatted name, retrieve its numeric index.
def invert_list(x):
	return {s:i for i,s in enumerate(x)}

# Reverse-lookup dicts: map formatted name strings back to their list index.
# When encoding YAML back to ROM data, these dicts convert human-readable
# object names into the numeric indices the ROM expects.
kType0Names_rev = invert_list(kType0Names)
kType1Names_rev = invert_list(kType1Names)
kType2Names_rev = invert_list(kType2Names)

# =============================================================================
# TAG NAMES — Room Puzzle Trigger Descriptions
# =============================================================================
# Tags define the puzzle mechanic for a dungeon room: what the player must do
# to trigger a door opening, a chest appearing, holes forming in the floor, etc.
# Index 0 = no tag (no puzzle). Each subsequent index describes a specific
# condition (kill enemies, push a block, pull a lever, light torches, etc.)
# and a specific reward (door opens, chest appears, holes appear, etc.).
# The quadrant prefixes (NW, NE, SW, SE, N, S, W, E) indicate which portion
# of the room the trigger applies to.
# =============================================================================
...
# =============================================================================
# EFFECT NAMES — Room Visual Effect Types
# =============================================================================
# Each dungeon room can have one visual effect active. Index 0 = no effect.
# These control special rendering behaviors like conveyor-belt floors,
# flowing water, screen flashes, or the Ganon boss room's unique visuals.
# Indices "01" and "04" are unnamed/unknown effects in the ROM.
# =============================================================================
...
# =============================================================================
# COLLISION NAMES — BG Layer Collision Modes
# =============================================================================
# Determines how the two background layers (BG1 and BG2) interact for
# collision detection. "One" = only BG1 has collision. "Both" = both layers
# have collision. "Both w/scroll" = both layers collide and BG2 scrolls
# independently. "Moving floor/water" = special collision for conveyor or
# water-current rooms.
# =============================================================================
...
# =============================================================================
# BG2 MODES — Background Layer 2 Rendering Modes
# =============================================================================
# Controls how the second background layer is rendered in each dungeon room.
# "None" = no BG2. "Parallaxing" = BG2 scrolls at a different rate for depth.
# "Dark" = room is dark (needs lamp). "On top" = BG2 drawn over BG1.
# "Translucent" = BG2 is semi-transparent. "Normal" = standard overlay.
# "Addition" = additive blending mode. "Dark room" = variant dark mode.
# =============================================================================
kBg2 = ["None", "Parallaxing", "Dark", "On top", "Translucent", "Parallaxing2", "Normal", "Addition", "Dark room"]
kBg2_rev = invert_list(kBg2)

# =============================================================================
# MUSIC NAMES — Music Track Index-to-Name Mapping
# =============================================================================
# Maps the ROM's music track indices to descriptive names. The SNES sound
# engine uses a single byte to select which music track plays in a room or
# during a cutscene. Special control values at the high end (240–243) are
# not music tracks but playback commands (stop, fade out, volume control).
# Index 255 ("Same") means "keep playing whatever is currently playing."
# =============================================================================
kMusicNames = {
    255: "Same",
    0: "None",
...
# Reverse lookup: music name string -> ROM index byte
kMusicNamesRev = invert_dict(kMusicNames)

# =============================================================================
# AMBIENT SOUND NAMES — Environmental Sound Effect Mapping
# =============================================================================
# Maps ambient/environmental sound indices to names. These are background
# audio loops layered on top of (or instead of) music. The ROM uses odd
# indices (1, 3, 5, ...) for these; even indices are unused. Index 5
# ("Stop") silences any currently playing ambient sound.
# =============================================================================
kAmbientSoundName = {
    0: "None",
    1: "Heavy rain",
...
# Reverse lookup: ambient sound name string -> ROM index
kAmbientSoundNameRev = invert_dict(kAmbientSoundName)

# =============================================================================
# PALACE NAMES — Dungeon/Palace Identifiers
# =============================================================================
# Each dungeon room in the ROM references a "palace" index that determines
# which tileset and palette group to use. Index 0 = "None" (non-dungeon).
# The names correspond to the game's dungeon areas. The order matches the
# ROM's internal palace numbering, not the game's progression order.
# =============================================================================
kPalaceNames = (
    "None",
    "Church",
...
# =============================================================================
# AREA NAMES — All 161 Overworld Area Identifiers
# =============================================================================
# The overworld map is divided into 8x8 grids of screen-sized areas. Each area
# has a unique index and a descriptive name. The prefix indicates the world:
#   LW = Light World (indices 0–63, the normal Hyrule overworld)
#   DW = Dark World (indices 64–127, the corrupted mirror version)
#   SP = Special areas (indices 128+, overlays, underlays, and unique screens)
#   NA = Unused/non-existent slots in the ROM's area table
# The three-digit number after the prefix is the area's index. Areas marked
# "<NO SPRITES>" or "<AVAILABLE>" are either empty or have restrictions in
# the ROM. Overlay/underlay areas (SP 149+) are transparent layers composited
# on top of or behind normal areas (e.g., rain, mist, tree canopy).
# =============================================================================
...
# =============================================================================
# ENTRANCE NAMES — All 133 Dungeon/Cave Entrance Descriptions
# =============================================================================
# Each entrance in the game (cave mouths, dungeon doors, building doors, drop-in
# holes) has an index and a description. The format is "[Room #NNN] -- Name"
# where NNN is the internal room number the entrance leads to. After parsing,
# only the name portion (after " -- ") is kept in kEntranceNames. Multiple
# entrances can lead to the same room (e.g., different doors into the same
# building). Entries ending with "(Drop In)" are pit/hole entrances where
# Link falls into the room from above rather than walking through a door.
# =============================================================================
...
# Strip the "[Room #NNN] -- " prefix, keeping only the descriptive name portion
kEntranceNames = [a[a.find(' -- ') + 4:] for a in kEntranceNames]

# Reverse lookup: entrance name string -> entrance index
kEntranceNames_rev = {s:i for i,s in enumerate(kEntranceNames)}

# =============================================================================
# SPRITE NAMES — All 284 Sprite Type Identifiers
# =============================================================================
# Every sprite in the game (enemies, NPCs, items, projectiles, overlays, and
# boss components) has a hex index and a short name. Format: "HH:Name" where
# HH is the hex sprite type ID. Indices 00–FF are standard sprites; 100+ are
# "overlord" sprites (room-level spawners and event controllers like cannon
# rooms, floor tile attacks, and boss phase triggers). The colon separator is
# later replaced with a dash for consistency with other name formats. Empty
# names after the colon (e.g. "02:") indicate unnamed/unknown sprite types.
# =============================================================================
...
# Replace colon with dash to match the "HH-Name" format used in YAML output
kSpriteNames = [a.replace(':', '-') for a in kSpriteNames]

# Reverse lookups: sprite name string -> numeric sprite type index.
# kSpriteName2Idx and kSpriteNamesRev are functionally identical dicts.
...
# =============================================================================
# SECRET NAMES — Hidden Item Drop / Secret Type Mappings
# =============================================================================
# When Link pulls on something, dashes into a tree, or triggers a hidden
# secret, the ROM uses a "secret" index to determine what appears. Indices
# 1–22 map to specific sprite drops (the kSpriteDropToNameIdx table below
# translates each secret index to the sprite type that gets spawned).
# Special indices (0, 4, 0x80–0x88) represent non-sprite secrets like
# holes, warps, staircases, bombable walls, and switches.
# =============================================================================

# Maps secret indices 1–22 to the kSpriteNames index of the sprite that
# spawns when that secret is triggered (e.g., index 0xd9 = Rupee-G sprite)
...
# Builds the full secret name dict by combining sprite-drop secrets (1–22)
# with special non-sprite secrets (holes, warps, etc.). Returns a dict
# mapping secret index -> "HH-Name" formatted string.
...
# Reverse lookup: secret name string -> secret index
kSecretNamesRev = invert_dict(kSecretNames)

# =============================================================================
# COMPRESSED SPRITE TILESET POINTERS — ROM Addresses
# =============================================================================
# Each entry is a 24-bit SNES ROM address pointing to a compressed sprite
# graphics sheet. The SNES stores sprite tile graphics in a custom compressed
# format; these pointers tell the decompressor where each sheet starts in the
# ROM. There are 108 compressed sprite sheets total. The addresses span ROM
# banks 0x10–0x18 (approximately 1.06 MB to 1.6 MB into the ROM file).
# These are used by the asset extractor to locate and decompress each sprite
# graphic sheet for conversion to modern image formats.
# =============================================================================
kCompSpritePtrs = [
...
# =============================================================================
# COMPRESSED BACKGROUND TILESET POINTERS — ROM Addresses
# =============================================================================
# Same concept as kCompSpritePtrs but for background tile graphics. Each
# 24-bit address points to a compressed background graphics sheet in the ROM.
# There are 115 compressed background sheets. The addresses span ROM banks
# 0x11–0x18. The final entries (0x14FF93–0x14FFF5) are closely packed near
# the end of bank 0x14, and the last two entries share addresses with the
# sprite pointer table (0x18B520, 0x18B953), likely shared tileset data.
# =============================================================================
kCompBgPtrs = [
...
# =============================================================================
# SPRITE TILESETS — VRAM Graphic Sheet Assignments Per Sprite Set
# =============================================================================
# The SNES has limited VRAM for sprite graphics, so each area/room loads only
# 4 graphic sheets at a time into the 4 available VRAM sprite slots. Each
# tuple here is (slot0, slot1, slot2, slot3) where each value is an index
# into the kCompSpritePtrs table above, identifying which compressed graphic
# sheet to load into that VRAM slot. Value 0 typically means "use default
# sheet" or "empty slot." The list is indexed by sprite tileset number,
# which is assigned per dungeon room or overworld area. Entries with all
# zeros are unused tileset slots. Indices 64+ (0x40+) are overworld tilesets;
# indices below 64 are dungeon/indoor tilesets.
# =============================================================================
kSpriteTilesets = [
  ( 0, 73,   0,  0),
  (70, 73,  12, 29),
...
# =============================================================================
# DUNGEON PALETTE INFO — Palette Assignment Table for Dungeon Rooms
# =============================================================================
# Each entry is a 4-tuple (pal0, pal1, pal2, pal3) that specifies which
# color palettes to load for a dungeon room's sprite set. The values are
# indices into the ROM's palette data tables. Different combinations produce
# the distinctive color schemes for each dungeon (e.g., green for Eastern
# Palace, blue for Ice Palace). The list is indexed by the dungeon's
# palette info index, assigned per room in the room header data.
# =============================================================================
kDungPalinfos = [
  ( 0,  0,  3,  1),
  ( 2,  0,  3,  1),
...
# =============================================================================
# Palette assignment data for overworld sprite sets. Each pair of values
# represents palette configuration for an overworld area group. The -1
# entries at the start indicate "no override" (use default palette).
# This table is indexed by the overworld area's sprite palette group
# number and determines which color palettes sprites use in that area.
# =============================================================================
kOwSprPalInfo = [
...
# =============================================================================
# SPRITE INITIALIZATION FLAGS (FLAGS3)
# =============================================================================
# Per-sprite-type initialization flags byte. Each sprite type (indexed 0x00
# through 0xF2) has a flags3 value that controls various spawn-time behaviors.
# The asset pipeline extracts the palette base from bits 1–3 of this flag
# (value >> 1 & 7) to determine which sprite palette row the sprite uses.
# Other bits control hitbox properties, layer priority, harmlessness, and
# whether the sprite interacts with moving floors. The values here are copied
# directly from the ROM's sprite property tables.
# =============================================================================
kSpriteInit_Flags3 = [
