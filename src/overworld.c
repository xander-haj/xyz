...
/* Return true when extended-aspect rendering should also suppress the
 * legacy screen-scroll presentation.  Both widescreen flags are accepted
 * so custom configs that disable sprite widening still get the transition
 ...
 * so other files can import them without seeing the table extern. */
...
/* LookupInOwEntranceTab — Linear scan of the 44-entry overworld
 * entrance lookup pair. Returns true if (r0, r2) matches any
 * (Tab0[i], Tab1[i]) pair, meaning the tile in front of Link is one
 * of the recognized entrance tile tuples. */
...
/* LookupInOwEntranceTab2 — Find the entrance index whose stored
 * position matches `pos` AND whose area matches the current area.
 * Returns the entrance id, or -1 on no match. */
...
/* CanEnterWithTagalong — Decide whether Link's current follower can
 * enter the given entrance id. Most followers can (0/5/14/1 are
 * benign companions); follower 7/8 (the dwarves) are restricted to
 * entrances >= 59 (the post-rescue sword-quest area). */
bool CanEnterWithTagalong(int e) {
  uint8 t = follower_indicator;
  return t == 0 || t == 5 || t == 14 || t == 1 || (t == 7 || t == 8) && e >= 59;
}

/* DirToEnum — Convert a one-hot direction bitmask (1, 2, 4, 8) into
 * an enum index (3, 2, 1, 0). The loop walks the bit position by
 * decrementing xx and right-shifting until the LSB is set. */
...
/* Overworld_ResetMosaicDown — Inverse of ConditionalMosaicControl:
 * decrements the mosaic level on every other frame so the mosaic
 * dissolves back from full-pixel-block to per-pixel as a screen
 * fades in. */
...
/* Overworld_Func1D / Overworld_Func1E — Unreachable submodule slots.
 * Asserts on entry so any future bug that lands here is caught
 * immediately rather than producing silent garbage. */
...
/* Overworld_GetSignText — Look up the dialogue id of the sign on
 * overworld area `area`. Used by the messaging system when Link
 * reads a signpost. */
 ...
 * GFX + palette tables. The dark-world half (entries 64..127) is
 * filled from a fixed slice (kOverworldSpriteGfx + 0xc0); the light-
 * world half is filled by the helper below based on game progress. */
...
/* Sprite_LoadGraphicsProperties_light_world_only — Three-way pick
 * of the light-world sprite GFX + palette set keyed by game progress
 * (pre-master-sword, post-master-sword, post-Agahnim). Each set is
 * a 64-entry slice in the compiled-in tables. */
...
/* InitializeMirrorHDMA — Reset every per-line HDMA variable used by
 * the magic-mirror warp-out distortion effect, then disable HDMA so
 * the table can be repopulated cleanly before re-enabling. */
...
/*
 * MirrorWarp_BuildWavingHDMATable — Per-frame builder for the
 * "warping in" half of the magic-mirror animation. Slides each row
 * of the 240-entry HDMA table up by 8 (each line takes the BG2HOFS
 * value of the line 8 above), then computes a sinusoidal wobble for
 * the first 8 lines via mirror_vars.var0..var7.
 *
 * mirror_vars layout:
 *   var0   - phase index 0 or 2 (toggles direction at extremes).
 *   var1[] - amplitude bounds at each extreme.
 *   var3[] - per-step velocity for the current direction.
 *   var5   - high-byte phase accumulator.
 *   var6   - main signed offset.
 *   var7   - sub-pixel residue for next frame.
 *
 * When the palette filter is mostly faded out (>= 0x30) and the
 * accumulator has dropped below 8, advance to the dewaving phase.
 */
...
/*
 * MirrorWarp_BuildDewavingHDMATable — "Warping out" phase: same row
 * slide as the waving variant but no new perturbation is injected.
 * The warp completes when the four center-of-screen entries
 * (0xc0/0xc8/0xd0/0xd8) all match BG2HOFS_copy2 — that is, the
 * distortion has fully drained back to the rest scroll. At that
 * point HDMA is disabled, the iris fixed-color is rebuilt, and
 * BG1/BG2 scrolls are re-pinned.
 */
...
/*
 * TakeDamageFromPit — Apply pit damage when Link falls into a hole.
 * Drops his visibility (so he is briefly hidden during the fall),
 * jumps to the appropriate damaged-recovery submodule (20 indoors,
 * 42 overworld), and subtracts 8 from his health. The 0xa8 wrap
 * check covers the underflow case where the subtraction took him
 * negative — in that case clamp to zero (will trigger a death).
 */
...
/*
 * Module08_OverworldLoad — Module 8 dispatcher: runs one of three
 * pre-overworld load steps via kModule_PreOverworld[submodule_index].
 */
...
/*
 * PreOverworld_LoadProperties — Submodule 0 of Module 8: bring up
 * a brand-new overworld scene from either a dungeon exit (main
 * module 8) or a special-overworld unwind. Major steps:
 *   1. Apply bunny status, load overworld state from the appropriate
 *      backup (dungeon or special-OW).
 *   2. Pick the correct music + animated tile group from a long
 *      decision tree keyed on screen index, dungeon room, and
 *      sram_progress_indicator. The selected `xt` value is later
 *      assigned to queued_music_control.
 *   3. Decompress animated tiles, initialize tilesets, load palettes,
 *      apply the fixed-color, kick off the follower, and reload
 *      every overworld sprite.
 *   4. Clear stale per-room state (torches, pose bits, key counts).
 *   5. Set up bunny mode if Link is in the dark world without the
 *      moon pearl.
 *   6. Final BGMODE / dungeon-state cleanup, advance the submodule,
 *      flag a HUD update for the next NMI, and queue any music
 *      change pending in queued_music_control.
 */
...
/*
 * AdjustLinkBunnyStatus — If Link has the Moon Pearl, force his
 * non-bunny state (so he stays human in the dark world). Without
 * the pearl, his current bunny status is preserved.
 */
...
/*
 * ForceNonbunnyStatus — Snap Link out of every bunny-related state:
 * back to ground state machine, clear temp-bunny timer, clear the
 * "needs poof transform" flag, and drop both bunny flags. With the
 * "turn while dashing" enhancement enabled, also cancel any active
 * dash so the player does not get stuck.
 */
...
/*
 * RecoverPositionAfterDrowning — Restore Link's coordinates and the
 * scroll/camera variables that were cached when he stepped into
 * water without the swim ability. Also re-runs the camera scroll
 * and any pending map-scroll handler so the screen lines up.
 */
...
/*
 * Module0F_SpotlightClose — Module 0x0F dispatcher: closes the iris
 * spotlight when leaving an area. Submodule 0 preps the exit; later
 * submodules drive the iris animation. After the iris work, runs
 * one-frame velocity updates so Link's idle/walk animation continues
 * during the close, then renders his OAM.
 *
 * The kTab[4] selector translates link_direction_facing into the
 * SNES one-hot direction bits; the special case `which_entrance ==
 * 0x43` forces direction "south" so the player exits the water-
 * passageway warp facing the correct way.
 */
...
/*
 * Dungeon_PrepExitWithSpotlight — One-shot setup for the iris-close
 * dungeon-exit transition: clear the polyhedral NMI thread, terminate
 * waterfall splashes if outdoors, latch the music fade based on the
 * destination's music track id (with the music_unk1 == 12 special
 * case for the bonus-key chime), refresh the floor indicator, and
 * fire IrisSpotlight_close to start the actual iris animation.
 */
...
/*
 * Spotlight_ConfigureTableAndControl — Per-frame iris driver while
 * the spotlight is active. Runs the table-builder
 * (IrisSpotlight_ConfigureTable) and disables the polyhedral NMI
 * thread. On the very first frame (submodule == 0) seeds Link's
 * y-coord from the cached exit value (so dungeon-exit positioning
 * is consistent) and calls OpenSpotlight_Next2 to trigger the
 * post-iris setup.
 */
...
/*
 * OpenSpotlight_Next2 — Post-iris housekeeping for entrances and
 * dungeon transitions:
 *   - Outside module 9: blank the screen and reset Link's item state.
 *   - In module 9 (overworld): pick the appropriate walk-out
 *     submodule (0xa for facing-down, 0xb for facing-up) for room
 *     id != 0x20, set up a 16-frame transition window, and (if a
 *     "fancy door" mark is set) initialize the door-open animation
 *     in submodule 9 with sound effect 21.
 *   - Always: clear all four window registers and a few HUD-related
 *     flags, then patch COLDATA for the three Death Mountain LW/DW
 *     screen sets that need a darker tinting baseline.
 */
...
/*
 * Module10_SpotlightOpen — Module 0x10 dispatcher: opens the iris
 * "spotlight" transition that frames Link when entering a dungeon
 * exit / overworld scene. Runs sprites first, then either advances
 * the iris on submodule 0 or holds it open while subsequent submodules
 * configure the HDMA window table, and finally renders Link's OAM.
 */
...
/*
 * Module10_00_OpenIris — One-frame setup that kicks off the iris
 * opening animation and hands control to the next submodule.
 */
...
/*
 * SetTargetOverworldWarpToPyramid — Game-flow hook that warps the
 * player to the Pyramid of Power overworld screen (used by the
 * Agahnim defeat / mirror sequence). Only fires when the main module
 * index is 21 (the warp trigger module). Loads the overworld
 * environment from the dungeon-exit code path, decompresses the
 * Pyramid's animated tile set (0x5a), and resets ancillas/cutscene
 * state so Link's input is ready immediately on arrival.
 */
...
/*
 * ResetAncillaAndCutscene — Clears every "input/cutscene lockout"
 * flag so the next gameplay frame returns full control to the player.
 * Called at the end of warps, dungeon exits, and other transitions.
 */
...
/*
 * Module09_Overworld — Master dispatcher for Module 0x09 (overworld
 * gameplay). Runs the active submodule via kOverworldSubmodules,
 * then renders sprites with a temporary BG offset shift so any
 * sprites that are drawn relative to the BG1 layer (parallax,
 * distortion screens, etc.) line up correctly. The original BG
 * offsets are restored before Link's OAM, HUD refill, and rain
 * overlay are processed, so those subsystems see the unshifted
 * scroll values.
 */
void Module09_Overworld() {  // 82a475
  kOverworldSubmodules[submodule_index]();

  /* Snapshot current BG1/BG2 scroll so we can restore after Sprite_Main. */
  int bg2x = BG2HOFS_copy2;
  int bg2y = BG2VOFS_copy2;
  int bg1x = BG1HOFS_copy2;
  int bg1y = BG1VOFS_copy2;

  /* Bias both shadow copies (the live one and the next-frame one) by
   * the BG1 parallax offset so sprites land in BG1's frame of reference. */
  ...
  /* Restore unshifted scroll so the rest of the frame uses BG2 origin. */
  BG2HOFS_copy2 = bg2x;
  BG2VOFS_copy2 = bg2y;
  BG1HOFS_copy2 = bg1x;
  BG1VOFS_copy2 = bg1y;
...
/*
 * OverworldOverlay_HandleRain — Drives the Zora's Domain rain weather
 * effect: animated rain overlay tiles, periodic lightning flashes,
 * and a thunderclap SFX. Active only on screen 0x70 (Zora's Domain
 * waterfall area) before the player has progressed past the rain
 * event.
 *
 * Lightning is implemented as timed CGADSUB color-math toggles at
 * specific frame_counter values (3, 5, 36, 44, 88, 90) — these are
 * the brightness flashes you see during a strike. Frame 36 also
 * queues SFX 54 (thunder).
 *
 * The rain overlay BG1 is scrolled diagonally by adding small offsets
 * every 4 frames; move_overlay_ctr cycles 0..3 to step through the
 * (1,0), (0,17), (1,0), (0,17) deltas (stored in the high byte of the
 * scroll register, so the actual delta is multiplied by 256).
 */
void OverworldOverlay_HandleRain() {  // 82a4cd
  ...
  /* Skip if not on Zora's Domain, after main quest progress threshold,
   * or if the rain has already been turned off by an event flag. */
  if (BYTE(overworld_screen_index) != 0x70 && sram_progress_indicator >= 2 || (save_ow_event_info[0x70] & 0x20))
    return;
  /* Lightning flash schedule (CGADSUB color-math values change brightness). */
  ...
  /* Rain overlay scroll only updates every 4th frame. */
  if (frame_counter & 3)
    return;
...
/*
 * Module09_00_PlayerControl — Submodule 0 of the overworld module:
 * the per-frame "player has full control" handler.
 *
 * Order of operations:
 *   1. Menu-button polling (only when no cutscene/spell/menu lockout
 *      flag is active):
 *        - Start  -> open the inventory menu (module 14, submode 1).
 *        - Map    -> open the overworld map     (module 14, submode 7).
 *        - Select -> select-button DBG menu / item switcher.
 *      Each handoff stashes main_module_index in saved_module_for_menu
 *      so the inventory/map can return to gameplay afterwards.
 *   2. Run any "special entrance" cutscene if scheduled.
 *   3. Tick Link's main state machine (Link_Main).
 *   4. Update the super-bomb HUD indicator if the side-quest is active.
 *   5. Recompute current_area_of_player from the live world coordinate.
 *      The bit math packs (Y >> 9) into the high nibble and (X >> 9)
 *      into the low nibble, giving a 6-bit OW area index.
 *   6. Load any new BG character data (Graphics_LoadChrHalfSlot).
 *   7. Operate the camera scroll, then either run the normal screen
 *      transition pipeline or — when main module 11 is active (special
 *      overworld events such as the bird flute travel) — the SOW exit
 *      check.
 */
void Module09_00_PlayerControl() {  // 82a53c
  if (!(flag_custom_spell_anim_active | flag_is_link_immobilized | flag_block_link_menu | trigger_special_entrance)) {
    /* Start button -> inventory submenu. */
    ...
    /* X / map button -> overworld map view. */
  ...
  /* Derive the 6-bit overworld area index from Link's world coords:
   *   high 3 bits = (Y >> 9), low 3 bits = (X >> 9). */
...
/*
 * OverworldHandleTransitions — Detects when Link has walked off the
 * edge of the current overworld screen and kicks off a transition
 * to the neighboring screen.
 *
 * Strategy:
 *   1. If a transition is already in progress (overworld_screen_trans_dir_bits2
 *      is non-zero), continue scrolling the map and return.
 *   2. Otherwise, examine Link's current velocity. The X and Y branches
 *      each compute a "distance into edge band" (`t`) by subtracting the
 *      area's base offset, then check whether Link is within the
 *      narrow edge band that triggers a screen swap. The (y, x, t)
 *      triple encodes the candidate direction:
 *        Y axis: y=6/x=8 for the top edge, y=4/x=4 for the bottom.
 *        X axis: y=2/x=2 for the left edge, y=0/x=1 for the right.
 *     `compare:` then verifies Link's facing direction matches the
 *      edge AND that the engine actually allows a transition.
 *   3. On a miss, fall back to special-switch areas (whirlpools etc.).
 *   4. On a hit, set up everything the next-screen load needs: the
 *      new area head id, music/SFX overrides, screen geometry, scroll
 *      direction, mosaic reset (when crossing world halves), and a
 *      fresh palette load.
 */
...
/*
 * Overworld_LoadGFXAndScreenSize — Selects every per-screen graphics
 * resource (sprite GFX, aux tile theme, main tile theme, misc sprite
 * pack) and screen-geometry constant (X/Y origin, scroll bounds, big-
 * vs-small map flag) for the overworld area now stored in
 * overworld_screen_index.
 *
 * Big maps (4 quadrants) and small maps (single quadrant) have
 * different mask values: 0x3f0 vs 0x1f0 for Y, and >>3 of those for X.
 * The dark-world bit (overworld_screen_index & 0x40) selects between
 * the light-world and dark-world tile/sprite themes.
 */
...
/*
 * Module09_LoadAuxGFX — Submodule entry: clear a few stale event flags
 * (Hyrule Castle tower wall blocked, ganon-pyramid hint, etc.), then
 * stage the auxiliary tile graphics for the transition. Schedules NMI
 * subroutine 9 (the second-half DMA) and advances the submodule.
 */
...
/*
 * Overworld_FinishTransGfx — Submodule entry: schedule NMI subroutine
 * 10 (final transition VRAM DMA) and step to the next submodule.
 */
...
/*
 * Module09_LoadNewMapAndGFX — Submodule entry: rebuilds the next
 * screen's tilemap, increments the NMI core-update suppression so the
 * partial frame is not torn, and seeds the scroll-in source map plus
 * the new sprite GFX set.
 */
...
/*
 * Overworld_RunScrollTransition — Per-frame runner during a scrolling
 * screen change: advances Link's walk animation, drains the
 * incremental VRAM upload queue, and steps the scroll state machine.
 * When the scroll state machine reports its low nibble has hit 0
 * (a "phase boundary"), it triggers a same-frame map load through
...
/*
 * Module09_LoadNewSprites — Submodule entry: finalizes a screen
 * transition by reloading all overworld sprites, resetting the
 * memorized-tiles cache, and applying the fixed-column scroll fix
 * post game progress threshold 2. Then hands control off to the
 * scroll-transition starter.
 */
void Module09_LoadNewSprites() {  // 82abed
  /* Special case for downward transition: nudge BG2 + Link by 2 px so
   * Link does not appear to skip a frame as the new map loads. */
...
/* Complete the normal edge-scroll bookkeeping in one update for widescreen
 * play.  The map, graphics, palettes, and sprites have already been loaded by
 * submodules 1-4; this only drains the visible scroll/ease/final walk so the
 * destination screen becomes playable immediately. */
static void Overworld_FinishScrollTransitionInstantly() {
  uint8 dir = BYTE(overworld_screen_trans_dir_bits);
  int guard = 0;

  submodule_index = 6;
  while (submodule_index == 6 && guard++ < 128) {
    uint8 rv = OverworldScrollTransition();
    if (!(rv & 0xf)) {
      BYTE(overworld_screen_trans_dir_bits2) = dir;
      OverworldTransitionScrollAndLoadMap();
  ...
  if (byte_7E069C & 2) {
    for (guard = 0; guard < 128 && ((link_x_coord + d) & 0xfe) != kOverworld_Func8_tab[byte_7E069C]; guard++)
      link_x_coord += d;
    link_x_coord += d;
  } else {
    for (guard = 0; guard < 128 && ((link_y_coord + d) & 0xfe) != kOverworld_Func8_tab[byte_7E069C]; guard++)
      link_y_coord += d;
    link_y_coord += d;
  }
 ...
 * map, happens.  This mirrors Overworld_LoadAmbientOverlay(true) without
 * advancing submodule_index or forcing a fade-to-black. */
static void Overworld_RebuildScreenForInstantTransition() {
  uint16 bak1 = map16_load_src_off;
  uint16 bak2 = map16_load_dst_off;
  uint16 bak3 = map16_load_var2;
...
/*
 * Overworld_StartScrollTransition — Bumps to the next submodule, and
 * if the new transition direction bits are >= 4 (right or left edge,
 * which complete in one frame), runs the same-frame map load.
 */
...
/*
 * Overworld_EaseOffScrollTransition — Tail end of a screen transition.
 * For small (single-quadrant) maps, runs one final map-load tick so
 * tile data is consistent. Then waits 8 (or 9 for vertical edges)
 * subsubmodule frames before clearing the transition direction bits,
 * advancing to the next submodule, and disabling the follower (so
...
/*
 * Module09_0A_WalkFromExiting_FacingDown — Animates Link walking
 * downward out of a building/dungeon exit one pixel per frame for
 * `ow_countdown_transition` frames. When the timer expires, jumps
 * the player 3 px further (so they are clear of the doorway), seeds
 * link_y_vel = 3 for the next-frame walk, and resets to submodule 0
 * so normal player control resumes.
 */
...
/*
 * Module09_0B_WalkFromExiting_FacingUp — Mirror of the FacingDown
 * variant for upward-facing exits: walks Link 1 px north each frame
 * until the timer expires, then returns control.
 */
...
/*
 * Module09_09_OpenBigDoorFromExiting — Drives the "big door opening"
 * animation when Link walks back out of a major dungeon entrance
 * (e.g., Hyrule Castle gates). Runs the 32x32 door-tile update each
 * frame until step 3 of the door animation completes, then sets a
 * 36-frame walk-out countdown and advances to the walk-out submodule.
 */
...
/*
 * Overworld_DoMapUpdate32x32_B — Wrapper that runs the 32x32 door
 * map-tile update and clears the open/close counter. Used as the
 * bottom-half completion hook for door animations.
 */
...
/*
 * Module09_0C_OpenBigDoor — Door-opening submodule used when Link
 * walks INTO an entrance. Advances the door animation each frame
 * until step 3, then resets to submodule 0 so the dungeon-load
...
/*
 * Overworld_DoMapUpdate32x32_conditional — Throttled wrapper around
 * the 32x32 door update: most frames just bump the counter, every
 * 8th frame actually advances the door animation. This is what gives
 * the big doors their slow "creaking open" pacing.
 */
...
/*
 * Overworld_DoMapUpdate32x32 — Plot the four map16 tiles that make up
 * one frame of a 32x32 door animation, recording each tile in the
 * memorized-tile log so the change survives screen transitions.
 *
 * The four tile addresses are big_rock_starting_address + {0, 2,
 * 0x80, 0x82} — top-left, top-right, bottom-left, bottom-right of a
 * 2x2 map16 cell (each cell is 2 bytes wide; +0x80 advances one row).
 *
 * After plotting, sentinels the VRAM upload buffer with 0xffff,
 * advances door_animation_step_indicator (skip-ahead by 2 if we are
 * at the special frame 32), and schedules NMI BG VRAM upload mode 1.
 */
...
/*
 * Overworld_StartMosaicTransition — Three-state mosaic-fade transition
 * driver. Drives the SNES MOSAIC register (via ConditionalMosaicControl)
 * to dissolve the screen when entering or leaving a major area.
 *   case 0: optionally fade music to zero, reset transition props.
 *   case 1: ramp the palette filter (palette dim->normal).
 *   default (2+): force-blank, reload animated sprite tile group 0x1e
 *     when the screen index has the low 6 bits zero (a "world boundary"
 *     screen), enable color math + screen layers, and advance.
 */
...
/*
 * Overworld_LoadOverlays — Full overlay-load entry. Reinitializes
 * sprite slots + reloads every overworld sprite, clears Link's state
 * bits / throw state, queues the standard overworld ambient sfx, and
 * delegates to the inner overlay setup routine.
 */
...
/*
 * PreOverworld_LoadOverlays — Lightweight overlay-load entry that
 * skips the sprite-slot reset (caller has already prepared sprites).
 */
...
/*
 * Overworld_LoadOverlays2 — Inner overlay loader: snapshots the
 * "previous" map state (so a back-out cancels cleanly), clears the
 * subpixel scroll, then picks an `xv` overlay id from a large
 * dispatch over overworld_screen_index / dungeon_room_index and
 * loads it.
 *
 * The xv ids are kSecondaryOverlay-layer indices for things like
 * the master-sword-removed forest, Zora's Falls SFX, the Lost Woods
 * fog, etc. The fall-through `goto getout;` exits without loading
 * anything (used when an event flag has already disabled the overlay).
 */
...
/*
 * Module09_FadeBackInFromMosaic — Three-phase mosaic ramp-down used
 * when arriving on a new overworld screen. Phase 0 loads palettes and
 * sprite GFX; phase 1 drains the VRAM upload queue while ramping the
 * palette filter; phase 2 (default) restores ambient sfx + music for
 * the destination screen and hands off to submodule 8 (or warps back
 * to module 9 from module 11 with a 12-frame post-warp pause).
 */
...
/*
 * Overworld_Func1C — Variant of FadeBackInFromMosaic used during
 * special-area entries (no palette/sprite reload). Music is forced
 * to a fixed value depending on whether the destination is a "world
 * boundary" screen (low 6 bits zero -> ambient, else music 2).
 */
...
/*
 * OverworldMosaicTransition_LoadSpriteGraphicsAndSetMosaic — Helper
 * shared by both fade-back submodules. Reloads the new sprite GFX,
 * unblanks the screen, enables HDMA channel 0 (mosaic table), and
 * primes the palette-filter countdown so the dim->normal ramp begins
 * from the current mosaic level.
 */
...
/*
 * Overworld_Func22 — Final brightness ramp-up: bumps INIDISP_copy
 * each frame until it reaches 15 (full brightness), then resets the
 * submodule indices so normal gameplay can resume.
 */
...
/*
 * Overworld_Func18 — Wrapper that runs special-area entry + overlay
 * load while preserving the caller's main_module_index and bumping
 * the submodule index. Used when an entry trigger needs to enter a
 * special area but should return control to its own module afterward.
 */
...
/*
 * Overworld_Func19 — Companion to Overworld_Func18: runs the dungeon
 * "load and advance" routine instead of the special-area path while
 * preserving the caller's module/submodule pair.
 */
...
/*
 * Module09_MirrorWarp — Drives the Magic Mirror world-swap effect.
 * State machine via subsubmodule_index:
 *   0: Sanity-check (refuse on 0x80+ screens), kick off the music
 *      cue, toggle savegame_is_darkworld, swap the world bit on the
 *      current screen index, and set up the white-filter HDMA.
 *   1: Enable HDMA on channels 6+7 (the mirror-distortion lines).
 *   2: Build the increasingly-warped HDMA table (waving phase).
 *   3: Build the dewaving HDMA table (recoiling phase).
 *   default: Tear down and load the destination via
...
/*
 * MirrorWarp_FinalizeAndLoadDestination — Closes out the mirror warp:
 * disables HDMA window 0, resets the iris spotlight, clears the
 * palette filter, restores previously-loaded GFX sheets, swaps to the
 * destination song list, and sets the overworld music + ambient SFX
...
/*
 * Overworld_DrawScreenAtCurrentMirrorPosition — Renders one frame's
 * worth of overworld quadrants while preserving the current map16
 * load offsets. Used during the mirror-bonk recovery animation.
 * For small maps, retargets the load offsets to the special-area slot
 * (0x390) so partial overlay tiles draw correctly. After drawing,
 * restores the saved offsets and runs the mirror-bonk recovery if we
 * are in submodule 44.
 */
...
/*
 * MirrorWarp_LoadSpritesAndColors — Final substep of a mirror warp:
 *   - Pin countdown_for_blink to ~2.4s.
 *   - Decode the destination map16 -> map8 conversion (with the
 *     same special-area offset patch as DrawScreenAtCurrentMirrorPosition).
 *   - Load every relevant palette set (OW BG, sprites, special-OW).
 *   - Whitewash 6 palettes worth (96 colors) of sprite_main_buffer
 *     so transient sprites appear pure white during the warp flash;
 *     special case for screen 0x5b (Magic Cap) zeroes a couple slots.
 *   - Reset all sprites and reload the overworld sprite list.
 *   - Hand Link to the Mirror player-state, and (only in the light
 *     world) place a Mirror Portal sprite at his arrival point.
 */
...
/*
 * Overworld_Func2B — Master Sword acquisition palette animation hook.
 * Just delegates to the palette-animation routine (the cycling glow).
 */
...
/*
 * Overworld_WeathervaneExplosion — Stub. The original ROM had handler
 * code here for the bombable weathervane behind Link's house; the
 * reimplementation routes that effect through other systems.
 */
...
/*
 * Module09_2E_Whirlpool — 13-state machine that animates the
 * whirlpool teleport between connected pools.
 *   0      seed SFX, clear palette filter
 *   1-2    blue palette ramp (color drains away)
 *   3      latch destination, swap overlay
 *   4,6    schedule paired NMI subroutines (palette cache flush)
 *   5      load destination overlay/map
 *   7-8    aux GFX + transition GFX
 *   9      reload all palettes for the destination + sprite GFX
 *   10-11  blue palette restore (color returns)
 *   12     final unblank, set music + ambient SFX, return to gameplay
 */
...
/*
 * Overworld_Func2F — One-shot map16 patch at world offset 0x720,
 * setting that tile to id 0x212 and memorizing the change so it
 * persists across screen swaps. Used for a single scripted tile
 * mutation (the bombable wall by Link's house).
 */
...
/*
 * Module09_2A_RecoverFromDrowning — Submodule 0x2A entry. Phase 0
 * scrolls Link to the nearest non-water tile; subsequent phases
 * restore his position and resume gameplay (drowning was triggered
 * by entering water without flippers / cape).
 */
...
/*
 * Overworld_OperateCameraScroll — Per-frame camera advance/scroll
 * with parallax and special-overlay handling.
 *
...
/*
 * OverworldCameraBoundaryCheck — Inner-loop camera-shift helper.
 * For one direction (controlled by xa/ya), advance the camera by `vd`
 * pixels if there is still room before the area boundary, advance the
 * scroll-window cursors, and accumulate a 16-pixel "block crossed"
 * counter into overworld_unk1/3 (which sets transition direction bits
 * when the player has dragged the camera over a screen-edge boundary).
 *
 * Returns vd if the camera moved, 0 if it was already at the boundary.
 */
...
/*
 * OverworldScrollTransition — Per-frame step of an inter-screen scroll.
 * Drives BG2 (and BG1, except on the seam screens 0x1b/0x5b) by the
 * tabulated direction delta, and once enough frames have elapsed
 * (kOverworld_Func6B_Tab2[y]) starts dragging Link with the camera.
 *
 * When the BG2 offset reaches the target value, snaps Link's
 * coordinate to a multiple of 8, repositions the camera scroll
 * windows, sets new camera boundaries via Overworld_SetCameraBoundaries,
 * advances the submodule, and reinitializes sprite slots so the new
 * screen's enemies pop in clean.
 *
 * Returns the BG offset value after the step (caller compares against
 * the target to decide whether the transition is finished).
 */
...
/*
 * Overworld_FinalizeEntryOntoScreen — After a screen-edge transition
 * completes, walks Link a couple more pixels into the new screen so
 * he is clear of the seam, then resets the submodule indices and
 * applies the destination's music + ambient SFX. Called once per
 * frame until Link's coordinate matches the post-entry target in
 * kOverworld_Func8_tab.
 */
...
/*
 * Overworld_Func1F — Slow scripted walk-out used after special area
 * exits. Moves Link one pixel per frame in the direction encoded by
 * byte_7E069C until the ow_countdown_transition reaches zero, at
 * which point control reverts to the gameplay module.
 */
...
/*
 * ConditionalMosaicControl — Bumps the SNES MOSAIC level by one step
 * (0x10 = one pixel block) every other frame, indexed by the
 * palette-filter countdown's parity. The `| 7` enables mosaic on the
 * three relevant BG layers. BGMODE 9 == screen mode 1 with BG3 as the
 * top priority.
 */
...
/*
 * Overworld_ResetMosaic_alwaysIncrease — Same as ConditionalMosaicControl
 * but unconditionally bumps the mosaic level every call.
 */
...
/*
 * Overworld_SetSongList — Selects the overworld music/ambient table
 * based on game progress.
 *   sram_progress_indicator < 2  : pre-Master-Sword set, bank 0
 *   < 2 with sword_type < 2      : same with bank 0x40 ambient
 *   < 3                          : retrieved-Sword set, bank 0x80
 *   else                         : full-progress set, bank 0xc0
 * Final byte at offset 128 is the "track for module 11" override.
 */
...
/*
 * LoadOverworldFromDungeon — Set up overworld state when leaving a
 * dungeon. Two paths:
 *   - For dungeon rooms 0x100..0x180 (other than 0x104): restore
 *     the cached entry/exit state via LoadCachedEntranceProperties.
 *   - Otherwise: scan kExitDataRooms for the matching dungeon room
 *     id and reseed every relevant overworld state field
 *     (BG offsets, Link coordinates, scroll bounds, screen index,
 *     entrance values) from the kExitData_* tables.
 */
...
/*
 * Overworld_LoadNewScreenProperties — Generic post-load initializer
 * that runs after either LoadOverworldFromDungeon or a fresh world
 * load. Sets the tilemap mask, recomputes per-screen GFX/size,
 * reseeds the camera bounds, and zeroes Link's quadrant + Z state.
 */
...
/*
 * LoadCachedEntranceProperties — Restore every overworld scroll /
 * camera / area variable from its "_exit" mirror. The "_exit"
 * variants were saved when Link entered the dungeon, so this is the
 * inverse operation. Special cases:
 *   - dungeon_room_index < 0x124 nudges Link 16 px up (so he is not
 *     clipped into the door tile).
 *   - ow_entrance_value == 0xffff (no doorway) shifts him 0x20 down
 *     and orients him north (the warp-gate convention).
 */
...
/*
 * Overworld_EnterSpecialArea — Backup-and-replace pattern: snapshots
 * every relevant overworld variable into its "_spexit" mirror so the
 * upcoming special-overworld load can be undone later, then calls
 * LoadOverworldFromDungeon to bring in the new screen.
 *
 * After the load, applies special-area-specific overrides from the
 * kSpExit_* tables (sprite GFX, aux GFX, palette indices, scroll
 * extents, base offsets) keyed by the low 6/7 bits of dungeon_room_index.
 *
 * The 0x1010 special case routes the "Watergate from Lake Hylia" exit
 * to room 0x182 (Zora Falls) because the entry trigger lives in the
 * dungeon-room id space.
 */
...
/*
 * LoadOverworldFromSpecialOverworld — Restore-from-spexit: the
 * complement of Overworld_EnterSpecialArea. Copies every "_spexit"
 * mirror back to the live state, recomputes derived offsets,
 * reloads palettes, and re-runs Link's Z/swimming/quadrant init so
 * the player resumes on the previous overworld screen exactly where
 * they left it.
 */
...
/*
 * FluteMenu_LoadTransport — Bird-flute teleport: looks up the user's
 * selected destination index in birdtravel_var1[0], shifts the bit
 * pattern (so the cursor advances) and dispatches the actual load
 * via Overworld_LoadBirdTravelPos.
 */
...
/*
 * Overworld_LoadBirdTravelPos — Seed every overworld coordinate /
 * scroll / camera variable for bird-travel destination index `k`
 * from the kBirdTravel_* table set, then run the standard new-screen
 * setup: load properties, reset all sprites, reload overworld
 * sprites, reset torch background.
 */
...
/*
 * FluteMenu_LoadSelectedScreenPalettes — Reload the palette set for
 * the destination preview shown while the bird-travel menu is open.
 */
...
/*
 * FindPartnerWhirlpoolExit — Look up the matching paired whirlpool
 * for the current screen; the +9 offset slots whirlpool destinations
 * into the bird-travel index space (which whirlpool teleports reuse).
 */
...
/*
 * Overworld_LoadAmbientOverlay — Build the "ambient" tile overlay
 * for the current screen. For small maps it temporarily retargets
 * the map16 load offsets to the special-area slot 0x390 so partial
 * overlays compose correctly. When `load_map_data` is true, also
 * triggers the full quadrant + overlay redraw.
 *
 * Always runs Map16ToMap8 to materialize the BG3 character map into
 * WRAM 0x2000, then schedules NMI subroutine 4 (BG3 text upload).
 */
...
/*
 * Overworld_LoadAmbientOverlayFalse — Convenience shim that runs the
 * ambient overlay load WITHOUT drawing the quadrants (just rebuilds
 * the BG3 char map). Used when the caller has already drawn the map.
 */
...
/*
 * Overworld_LoadAndBuildScreen — Convenience shim that runs the full
 * ambient overlay load with both quadrant draw and BG3 build.
 */
...
/*
 * Module08_02_LoadAndAdvance — Submodule entry that loads the next
 * screen end-to-end and hands control off to module 16 (the post-
 * load animation module). Used during scripted game-flow transitions.
 */
...
/*
 * Overworld_DrawQuadrantsAndOverlays — Master "build the scene"
 * routine: decompresses + draws all four overworld quadrants, then
 * fills BG1 with the standard "blank" map16 tile (0xdc4) for the
 * 64-cell HUD strip area, draws an entrance doorway tile pair
 * (0xda4/0xda6 for normal entries, 0xdb4/0xdb5 for special) at
 * ow_entrance_value, and finally runs the per-screen overlay /
 * bomb-door post-pass.
 */
...
/*
 * TriggerAndFinishMapLoadStripe_Y — Fill the per-frame VRAM upload
 * packet stream (uvram.data) with N "vertical stripe" map16->map8
 * blocks. The leading 0x80 packet header tells HandleStripes14 the
 * stream uses the +32 (vertical) increment mode. Each iteration
 * pulls the source offset back by 0x80 (one column up) so the next
 * stripe lines up vertically; the dst variable is a 32-row stride
 * VRAM cursor managed by BufferAndBuildMap16Stripes_Y. Sentinels the
 * stream with 0xffff so the NMI handler knows when to stop.
 *
 * @n: number of vertical stripes to upload this frame
 */
...
/*
 * TriggerAndFinishMapLoadStripe_X — Horizontal-direction analog of
 * TriggerAndFinishMapLoadStripe_Y. The header 0x8040 selects the
 * horizontal stripe sub-mode (the 0x40 bit is the "X axis" marker
 * read by BufferAndBuildMap16Stripes_X), and each iteration pulls
 * the source back by 2 bytes (one column) and decrements the dst
 * column index modulo 32.
 */
...
/*
 * SomeTileMapChange — Lightweight redraw used during transitions:
 * decompress + draw all quadrants, fill the BG1 HUD strip with the
 * blank tile, run overlays/bomb-doors, and bump the submodule.
 */
...
/*
 * CreateInitialNewScreenMapToScroll — Dispatch table for "first frame
 * of a screen scroll": picks the correct stripe-builder based on
 * map size (small vs big) and transition direction (north / south /
 * east / west via overworld_screen_trans_dir_bits2 flag = 8/4/1/2).
 *
 * Each variant primes the map16 load offsets so the eight subsequent
 * scroll frames can incrementally upload one stripe per frame and
 * keep the off-screen edge of the new map in sync with Link's
...
/*
 * CreateInitialOWScreenView_Big_{North,South,West,East} — Variants
 * that pre-load the off-screen edge for big (4-quadrant) maps. Each
 * adjusts map16_load_src_off / var2 / dst_off to point at the row
 * (or column) the player is about to scroll into, then issues a
 * bulk Y or X stripe load to seed seven or eight stripes ahead.
 */
...
/*
 * CreateInitialOWScreenView_Small_{North,South,West,East} — Same as
 * the Big variants but for single-quadrant (small) maps. These also
 * stash the original load offsets in orange_blue_barrier_state plus
 * word_7EC174/76 so EaseOffScrollTransition can restore them at
...
/*
 * OverworldTransitionScrollAndLoadMap — Per-frame "tail" of a
 * scrolling screen change: emits one full map16 stripe in the
 * direction encoded by overworld_screen_trans_dir_bits2, sentinels
 * the packet stream with 0xffff, and (only if any data was actually
 * written) schedules NMI subroutine 3 to commit it.
 */
...
/*
 * BuildFullStripeDuringTransition_{North,South,West,East} — Append
 * one full stripe (32 tiles) to the VRAM packet stream at `dst` and
 * advance the load offsets for the next frame. North/south use the
 * vertical (0x80) header; east/west use the horizontal (0x8040)
 * header. Returns the new tail pointer.
 */
...
/*
 * OverworldHandleMapScroll — During an in-progress scroll, checks
 * whether the visible window has just exposed an as-yet-unloaded
 * stripe and, if so, emits exactly one stripe's worth of map16 data
 * to the VRAM packet stream.
 *
 * The diagonal cases 5 (south+east), 6 (south+west), 9 (north+east),
 * 10 (north+west) only consume the Y axis bit and leave the X bits
 * intact — Link can be moving diagonally, but only one axis worth of
 * stripes is uploaded per frame to keep the DMA budget bounded.
 *
 * After packing, sentinels and copies overworld_screen_trans_dir_bits2
 * to overworld_screen_transition so the scroll state machine can read
 * the post-tick value.
 */
...
/*
 * CheckForNewlyLoadedMapAreas_{North,South,West,East} — Edge-of-map
 * guards that early-return without emitting a stripe when there is
 * no new map data left to load (the camera reached the world edge).
 * Otherwise emit one stripe header + body and advance the offset
 * pointers.
 */
...
/*
 * BufferAndBuildMap16Stripes_X — Build one VRAM packet for an
 * x-axis (column) stripe transfer.
 *
 * Step 1: copy 32 map16 source words from `dung_bg2` into the
 * dung_replacement_tile_state ring buffer at offset map16_load_var2,
 * walking the source by 128 bytes per row (one quadrant column).
 * Tiles past 0x2000 are zeroed (off-map).
 *
 * Step 2: walk the buffer a second time, expanding each map16 cell
 * to its 4 map8 tile indices via GetMap16toMap8Table(). The packet
 * layout in `dst` interleaves two VRAM rows so HandleStripes14 can
 * commit them without a follow-up packet.
 *
 * Returns the new packet write cursor.
 */
...
/*
 * BufferAndBuildMap16Stripes_Y — Y-axis (row) counterpart of
 * BufferAndBuildMap16Stripes_X. Same two-pass design (gather 32
 * source map16 words into the ring buffer, then expand to map8) but
 * walks the source by 2 bytes (one map16 cell horizontally) per
 * iteration and the destination uses a 32-word stride between rows.
 */
...
/*
 * Overworld_DecompressAndDrawAllQuadrants — Decompress all four
 * quadrants of the current overworld screen (top-left, top-right,
 * bottom-left, bottom-right) into their fixed WRAM staging slots.
 * The quadrant indices are si + 0/1/8/9 (one column step + one row
 * step in the 8x8 area grid).
 */
...
/*
 * GetOverworldHibytes / GetOverworldLobytes — Asset accessors that
 * return the compressed-data pointer for screen `i`'s hi-byte and
 * lo-byte streams. The data is split because LZ-style compression
 * works better when high and low bytes are encoded independently.
 */
...
/*
 * Overworld_DecompressAndDrawOneQuadrant — Decompress one quadrant
 * of map32 ids and convert it into a 16x16 map16 grid in `dst`.
 *
 * Pipeline:
 *   1. Decompress hi byte stream into 0x14400, then interleave it
 *      into the odd bytes of 0x14000.
 *   2. Decompress lo byte stream the same way into the even bytes.
 *      Together they form 256 16-bit map32 ids.
 *   3. Walk the 16x16 grid, expanding each map32 id into a 2x2 cell
 *      of map16 tiles via Overworld_ParseMap32Definition. The +96
 *      after each row jumps to the next row of the output (which is
 *      32 cells wide; the next quadrant's data lives in those gaps).
 */
...
/*
 * Overworld_ParseMap32Definition — Expand a single map32 id into the
 * four map16 tile ids (a 2x2 cell). The expansion is cached: when the
 * upper bits of the id match map16_decode_last, the four
 * map16_decode_N[8] arrays are reused without re-fetching, which
 * speeds up the common case of consecutive identical map32 cells.
 *
 * The map32 -> map16 lookup is split across four parallel tables
 * (kMap32ToMap16_0..3); the (a >> 1) + (a >> 2) addressing arithmetic
 * unpacks the 6-byte-per-entry table layout.
 */
...
/*
 * OverworldLoad_LoadSubOverlayMap32 — Decompress one quadrant of the
 * SECONDARY/sub overlay (the per-screen "extra" map32 layer used by
 * special overlays) into WRAM 0x4000.
 */
...
/*
 * LoadOverworldOverlay — Run the sub-overlay map32 decompress, then
 * convert the resulting map16 grid into map8 character data at WRAM
 * 0x1000 (BG3 staging) via Map16ToMap8. Schedules NMI subroutine 4
 * (BG3 text upload) and advances the submodule.
 */
...
/*
 * Map16ToMap8 — Walk all 32 rows of the source map16 grid at `src`
 * and convert each row to map8 character data via
 * OverworldCopyMap16ToBuffer. The caller-supplied `r20` is OR'd into
 * the row's VRAM destination address (bit pattern that selects which
 * BG layer / page receives the data). Each row pulls map16_load_src_off
 * back by 0x80 (one row) and decrements var2.
 */
...
/*
 * OverworldCopyMap16ToBuffer — Copy one full row (32 cells) of map16
 * indices from `src` into the dung_bg2_attr_table BG3 character
 * staging buffer, expanded into 4 map8 ids per cell.
 *
 * `r20` is OR'd into the per-row VRAM destination address; `r14` is
 * the byte offset within the BG3 buffer; `r10` walks the per-row
 * VRAM-destination table at &word_7F4000.
 *
 * The (of >= 0x10) split implements the engine's "two pages of 16
 * rows" addressing: rows 16+ live at VRAM offset 0x800 instead of 0.
 */
...
/*
 * DecompressEnemyDamageSubclasses — Unpack the kEnemyDamageData blob
 * (one nibble per damage class) into the 0x1000-entry
 * enemy_damage_data byte array. Each compressed byte yields two
 * output entries (high nibble first). Used at boot to hydrate the
 * sprite damage lookup tables.
 */
...
/*
 * Decompress_bank02 — Decompress an ALttP-format LZ-style stream
 * from `src` into `dst`. Returns the number of decompressed bytes.
 *
 * Opcode layout: command byte's top 3 bits select the operation,
 * low 5 bits give length-1 (extended to 10 bits via a length byte
 * when the top 3 bits are all set):
 *   0x00 : direct copy of `len` bytes from src.
 *   0x20 : RLE — repeat the next byte `len` times.
 *   0x40 : alternating — interleave two bytes for `len` outputs.
 *   0x60 : incrementing — copy a byte that increments by 1 each step.
 *   0x80+: window copy — read 16-bit absolute offset and copy `len`
 *           bytes from earlier in the output buffer.
 *   0xff : end-of-stream sentinel.
 */
...
/*
 * Overworld_ReadTileAttribute — Look up the tile attribute byte
 * (kSomeTileAttr) at world position (x, y). Translates the world
 * coordinate into a map16 grid index by subtracting the area's base
 * offset and masking against the area's size mask, then folds the X
 * and Y components into a single 13-bit address (Y << 3 because each
 * row is 8 cells of 2-byte map16 entries = 16 bytes per row in the
 * mask space).
 */
...
/*
 * Overworld_SetFixedColAndScroll — Configure the SNES fixed color
 * (COLDATA) and the OW BG palette top-color slot per screen.
 *
 * Picks a 15-bit RGB seed `p` based on overworld_screen_index:
 *   0x80 + dungeon room 0x181 (Pyramid)  : enable subscreen, dark
 *     world tone (0x2A32) or light (0x2669).
 *   not 0x80/0x81, not Death Mountain    : same dark/light tones.
 *   else                                  : 0 (no override).
 *
 * `p` is written to both the main and aux palette buffers at index 0
 * and 32 (the "background" and "filter" slots), and a default
 * COLDATA triplet is set. For specific cyan/blue screens the
 * COLDATA is patched to 0x8c4c26 / 0x874a26.
 *
 * The trailing block (submodule != 4) sync-locks BG1 scroll to BG2
 * and applies a subtle parallax scroll on the 0x1b/0x5b (Lost
 * Woods) screens by deriving an extra Y offset from BG2HOFS.
 */
...
/*
 * Overworld_Memorize_Map16_Change — Append a single (pos, value)
 * map16 mutation to the persistent change log so the change survives
 * scrolling, mirror warps, etc. Skips two specific "ignored" tile
 * values (0xdc5, 0xdc9) which represent transient grass/decorations.
 */
...
/*
 * HandlePegPuzzles — Per-screen peg-block puzzle progression.
 *
 * Light-world Turtle Rock approach (screen 7): Tracks an ordered
 * sequence of three peg presses. The expected position is read from
 * kLwTurtleRockPegPositions; each correct hit plays SFX 0x2d and
 * advances word_7E04C8. Three correct hits unlock the entrance:
 * sets event flag 0x20 and jumps to submodule 47 (gate-opening).
 * Any out-of-order hit plays the dead-clunk SFX (0x3c) and resets
 * the counter to 0xffff (sentinel for "puzzle failed").
 *
 * Misery Mire approach (screen 0x62): Just counts up to 22 pegs
 * before opening the entrance with the standard 32x32 door update.
 */
...
/*
 * GanonTowerEntrance_Func1 — Two-phase Ganon's Tower entrance
 * cutscene. Phase 0 plays the chime SFX and starts the master-sword-
 * style palette glow; phase 1 ramps a blinding-white flash and (when
 * the screen is fully white) pins the palette filter at max so the
 * subsequent module can dissolve back from white.
 */
...
/*
 * Overworld_CheckSpecialSwitchArea — When Link is standing on a
 * special-switch tile (whirlpool, master-sword pedestal trigger,
 * etc.), kicks off a special-area entry: sets the destination
 * dungeon room id, the transition direction, and main module 11
 * (special-overworld load). The 4-entry tables are walked
 * highest-first so later overlapping entries take precedence.
 */
...
/*
 * Overworld_GetMap16OfLink_Mult8 — Returns a pointer into the
 * map16->map8 table at the cell Link is currently standing on. The
 * +8 X / +12 Y offsets snap to Link's collision center, the mask
 * step wraps around the area boundary, and the *4 stride matches
 * the 4-tile-per-cell layout.
 */
...
/*
 * Palette_AnimGetMasterSword — Two-phase entry to the master sword
 * "white-out and slowly return to color" cutscene.
 *   subsub == 0: kick off via Palette_AnimGetMasterSword2.
 *   subsub != 0: ramp the blinding-white filter; when the filter is
 *     fully saturated (darkening flag == 0xff), zero the master
 *     sword's 8-entry palette range and return to gameplay.
 */
...
/*
 * Palette_AnimGetMasterSword2 — Setup helper: backs up the live aux
 * palette into mapbak_palette, fills aux with pure white, copies the
 * BG color slot into the filter slot, and starts the bright-out
 * filter animation.
 */
...
/*
 * Palette_AnimGetMasterSword3 — Restore helper: when the filter has
 * fully run (countdown == 31, no further direction), restores the
 * backed-up aux palette and disables the subscreen.
 */
...
/*
 * Overworld_DwDeathMountainPaletteAnimation — Lightning-flash palette
 * cycle on Dark World Death Mountain screens (0x43/0x45/0x47).
 * Specific frame_counter ticks copy aux <- main (return-to-normal)
 * or main <- kDwPaletteAnim (light-up). Frame 36 also fires the
 * thunderclap SFX. The stride pattern (0x30/0x38/0x48/0x70/0x78)
 * targets the palette rows used by the dark mountain BG sky/cloud.
 */
...
/*
 * Overworld_LoadEventOverlay — Per-screen scripted tile patches that
 * apply when the screen's event-flag bit 0x20 is set (e.g., uncle's
 * body in Hyrule Castle exterior, the Master Sword glow, post-quake
 * cracks, dwarf blacksmith's broken house, etc.).
 *
 * Each `case` group corresponds to one or more overworld screen ids
 * sharing the same overlay definition. The XY(x,y) macro converts
 * (col, row) to a flat dung_bg2 index, and each assignment replaces
 * a single map16 cell with the post-event tile id.
 *
 * The shared `loc_8EF7B4` label is reused by several short overlays
 * that all draw the same 2x2 "cracked stone / sealed entrance" group
 * at different positions (passed in `x`).
 *
 * The huge case 59/123 block plots the entire Lost Woods foliage
 * overlay (stems, leaves, shadow groups). It is unrolled for speed —
 * the original ROM stored these as data tables but the conversion
 * inlined them.
 */
...
/*
 * Ancilla_TerminateWaterfallSplashes — On Zora's Waterfall (screen
 * 0x0f) only, kill any active waterfall-splash ancillas (type 0x41).
 * Other screens never spawn that ancilla so the early return is OK.
 */
...
/*
 * Overworld_GetPitDestination — Look up where Link should land when
 * he falls into a pit on the current overworld screen. Computes the
 * map16 cell index from his snapped (x & ~7, y & ~7) coordinate, then
 * scans kFallHole_Pos / kFallHole_Area for a matching entry. The
 * matching kFallHole_Entrances row provides which_entrance for the
 * destination dungeon room.
 *
 * Fallback: if no entry matches, drop into entrance 130 (Chris
 * Houlihan's room) in the light world. This is the secret debug-room
 * fallback that became famous in the SNES community.
 */
...
/*
 * Overworld_UseEntrance — Inspect the tile under Link's feet and,
 * when he is facing north, decide whether he is stepping into an
 * entrance:
 *   tile 0xe9                : standard small doorway -> draw open
 *                              door (DA4/DA6) and play SFX 21.
 *   tile 0x149 / 0x169       : big stone door (Hyrule Castle, etc.)
 *                              -> kick off the 32x32 door animation
 *                              with appropriate progress gating.
 *   anything else            : delegates to LookupInOwEntranceTab to
 *                              see if the tile is a dungeon entrance,
 *                              and if so checks tagalong / item-pose
 *                              constraints. If a tagalong is blocking,
 *                              triggers dialogue 5 ("you cannot bring
 *                              your friend in here").
 *
 * Successful entry sets main module 15 (entrance pre-load) and
 * stashes the previous module in saved_module_for_menu so the engine
 * can return on death/exit.
 */
...
/*
 * Overworld_ToolAndTileInteraction — Resolve what happens when Link
 * uses an item on the tile at (x, y).
 *
 * Returns the tile attribute to act on, or 0 if no interaction.
 *
 * Branches:
 *   - link_item_in_hand bit 1 (hammer in hand):
 *       attr 0x21b (peg block) -> SFX 17, advance peg puzzle, swap
 *                                tile to hammered version (0xdcb).
 *       else -> Overworld_PickHammerSfx (clunk SFX selection).
 *   - link_item_in_hand bit 6 (item not currently held -> shovel):
 *       Long whitelist of "shovelable" attrs (sand, soft dirt) ->
 *       reveal hidden secret if any (Overworld_RevealSecret) and
 *       swap to dug tile 0xdc9.
 *       attr 0x37e (thick grass) -> queue terrain-smash sprite +
 *       leaf poof, swap to mowed grass tile 0xdc5.
 *       attr 0x36 / 0x72a (bushes) -> standard or grassy bush, same
 *       reveal/swap path with the matching dust/leaf garnish.
 *
 * The shared `check_secret` and `memoize_getout` labels collapse the
...
/*
 * Overworld_PickHammerSfx — Pick the appropriate "hammer hits this"
 * SFX based on the underlying tile attribute. Maps four hard-tile
 * categories (50, 52, 54, 58) to SFX ids 26 / 17 / 5; anything else
 * stays silent.
 */
...
/*
 * Overworld_GetLinkMap16Coords — Compute the map16 cell index that
 * Link is "pointing at" for a tool action, derived from his facing
 * direction via kGetBestActionToPerformOnTile_x/y. The returned
 * point is also written into *xy so callers can use the snapped
 * coordinate for terrain-spawn placement.
 */
...
/*
 * Overworld_HandleLiftableTiles — Test whether the tile in front of
 * Link is liftable / smashable and, if so, dispatch the appropriate
 * pick-up. Three classes are recognized:
 *   small bushes / pots / signs (attr 0x36, 0x72a, 0x20f, 0x239,
 *     0x101) -> Overworld_LiftingSmallObj.
 *   rock piles (attr 0x36d..0x375 light-world, 0x23b..0x23e
 *     dark-world) -> SmashRockPile_fromLift.
 *   anything else -> just return the underlying tile attribute
 *     (no pick-up; caller decides what to do).
 */
...
/*
 * Overworld_LiftingSmallObj — Pick up a small object: reveal hidden
 * secret beneath it (rupee / heart / portal), substitute the
 * "after-lift" tile (`y`) into both the attribute and BG2 tilemap,
 * memoize the change, and queue an NMI VRAM upload. Returns the
 * tile attribute of the underlying ground so Link's collision math
 * snaps to the new walkable surface.
 */
...
/*
 * Overworld_SmashRockPile — Power-glove / Titan's-mitt entry that
 * smashes the rock-pile or bush in front of Link (or the cell one
 * row down when down_one_tile is true). Returns the post-smash
 * walkable attribute, or -1 if there was nothing to smash.
 */
...
/*
 * SmashRockPile_fromLift — Animate destruction of a 2x2 big-rock
 * pile. Rebases pos to the top-left of the 2x2 (using kBigRockTab1
 * to back out the partial offsets), kicks off the 32x32 door-update
 * machine (re-purposed for the rock crumble animation), and reveals
 * any underlying secret. A `secret == 0xffff` result is the special
 * "this rock pile was the screen's gate" trigger -- sets the global
 * "rock smashed" save flag, plays SFX 27, and extends the door-anim
 * counter to 80 so the gate fully opens.
 */
...
/*
 * Overworld_BombTiles32x32 — Apply a bomb's blast to a 3x3 grid of
 * map16 cells centered on (x-23, y-20). Each cell is processed by
 * Overworld_BombTile. word_7E0486/0488 capture the snapped origin
 * so subsequent VRAM patches know where the blast went.
 */
...
/*
 * Overworld_BombTile — Apply a single bomb cell of damage at (x, y).
 *
 * Behavior depends on the underlying tile attribute:
 *   0x36   - bush                 -> swap to 0xdc7, smash terrain idx 2
 *   0x72a  - special bush         -> swap to 0xdc8, smash terrain idx 4
 *   0x37e  - thick grass          -> swap to 0xdc5, smash terrain idx 3
 *   anything else                  -> jump to label_a (no destruction)
 *
 * The label_a fallback still calls Overworld_RevealSecret because a
 * blank-looking floor tile can be hiding a bombable wall: if the
 * secret returns 0xdb4 (the bomb-door tile), it draws a 2-cell
 * 0xdb4/0xdb5 pair at (pos, pos+2), memorizes the change, and sets
 * event flag bit 2 so the door persists.
 *
 * follower_indicator == 13 (a stone follower / bomb shop NPC) skips
 * straight to the secret-check path so the follower's tile never
 * gets accidentally cleared.
 */
...
/*
 * Overworld_AlterWeathervane — One-shot map mutation that turns the
 * weathervane behind Link's house into the post-bomb collapsed
 * version, exposing the secret entrance underneath. Sets the screen
 * 0x18 event flag and queues an NMI VRAM upload.
 */
...
/*
 * OpenGargoylesDomain — Plot the six map16 tiles that "open" the
 * Thieves' Town gargoyle entrance after the player throws a bottle.
 * Plays SFX 0x1b (revealed-secret chime) and sets event flag 0x20
 * for screen 0x58.
 */
...
/*
 * CreatePyramidHole — Plot the nine-tile 3x3 hole in the Pyramid of
 * Power roof revealed when Agahnim is defeated. Triggers an ambient
 * sfx update (the rumbling that plays after the hole appears),
 * sound_effect_2 = 3 (the breaking-stone SFX), and sets the screen
 * 0x5b event flag.
 */
...
/*
 * Overworld_RevealSecret — Walk the per-screen kOverworldSecrets
 * table to see if the map16 cell at `pos` hides a secret. Returns
 * the post-reveal map16 id (one of the four kTileBelow entries),
 * or 0 to indicate "no replacement needed". Also sets the
 * dung_secrets_unk1 byte to flag what KIND of reward (rupee, heart,
 * portal) should spawn underneath.
 *
 * The 0xffff sentinel terminates the per-screen secret list; reaching
 * it means the cell has no secret (jumps to `fail`).
 *
 * Special cases:
 *   - data == 0x84             : silent reveal (no chime).
 *   - screen 0x5b without a    : pyramid-of-power requires the
 *     specific tagalong            tagalong to be 13 to reveal.
 *   - data == 0x82 + bug-fix   : added by enhanced features set so
 *     features                     the Ice Temple portal chime plays.
 *
 * The trailing AdjustSecretForPowder hook lets the magic powder
 * upgrade certain secret rewards (e.g., turning a rupee into a fairy).
 */
...
/*
 * AdjustSecretForPowder — When magic powder is in Link's hand
 * (bit 0x40 of link_item_in_hand), force the secret reward kind to
 * 4 (the "fairy" upgrade). This is what causes the standard "throw
 * powder on the bush" event to upgrade ordinary secrets.
 */
...
/*
 * Overworld_DrawMap16_Persist — Combined "write to dung_bg2 +
 * upload to VRAM" wrapper. The persistence is implicit (the BG2
 * write keeps the change in WRAM); use Overworld_Memorize_Map16_Change
 * separately if the change must survive screen swaps.
 */
...
/*
 * Overworld_DrawMap16 — Append a single map16 cell (4 map8 tiles)
 * to the per-frame VRAM upload packet stream.
 *
 * Each cell becomes two stripes (top row and bottom row), each
 * encoded as: VRAM addr (big-endian via swap16), VMAIN code 0x300
 * (auto-increment, vertical 32 stride), then 2 map8 tile ids.
 * 0xffff sentinel after the second stripe.
 */
...
/*
 * Overworld_AlterTileHardcore — Same packet emission as
 * Overworld_DrawMap16 but ALSO writes the new value into dung_bg2.
 * Used for permanent map mutations that should not be revertible
 * even on screen reload (e.g., crystal-revealed tiles).
 */
...
/*
 * Overworld_FindMap16VRAMAddress — Convert a map16 buffer offset
 * (linear within the 64x64 tilemap) into a VRAM word address that
 * accounts for the four 32x32 tilemap pages: pages stack as
 * [TL=0, TR=0x400, BL=0x800, BR=0xc00], and within a page each row
 * is 32 words. The bit math:
 *   bit 5 of column   -> +0x400 (right page)
 *   bit 11 of address -> +0x800 (bottom page)
 *   low 5 bits        -> column inside page
 *   bits 7..10        -> row inside page (>>1 because each row is
 *                        2 bytes per cell in the source).
 */
...
/*
 * Overworld_AnimateEntrance — Top-level dispatcher for the five
 * scripted "dungeon entrance opens" cutscenes (Palace of Darkness,
 * Skull Woods, Misery Mire, Turtle Rock, Ganon's Tower).
 *
 * Sets the immobilization / unk1 / NMI-suppression flags so the
 * cutscene plays without player input or other module activity, then
 * dispatches into kOverworld_EntranceSequence[trigger - 1].
 */
...
/*
 * Overworld_AnimateEntrance_PoD — Palace of Darkness entrance.
 * Five-phase state machine driven by overworld_entrance_sequence_counter:
 *   0: Wait 64 frames, then plot the first 4-tile boom + sets the
 *      entrance event flag.
 *   1-3: Each phase waits 32 frames, plays an explosion, and plots
 *      the next layer of the gradually-opening doorway.
 *   4: Wait 32 more frames and play the entrance jingle.
 *
 * Each tile-plot is followed by `nmi_load_bg_from_vram = 1` so the
 * NMI handler picks up the change on the next frame.
 */
...
/*
 * Overworld_AnimateEntrance_Skull — Skull Woods (Dark Forest Palace)
 * entrance. Five phases that progressively plot the skull's eyes,
 * fangs, and entrance maw with SFX 0x16 (low rumble) at each phase.
 * Phase 4 calls OverworldEntrance_PlayJingle to finish.
 */
// Dark Forest Palace
...
/*
 * Overworld_AnimateEntrance_Mire — Misery Mire entrance, the most
 * elaborate of the five. The kMiseryMireEntranceBits LUT encodes
 * a per-frame TS_copy (subscreen enable) bit pattern that produces
 * the lightning-strike flicker. Phases 2+ also impose a 1-pixel
 * BG-shake (bg1_x_offset / bg1_y_offset alternating each frame).
 *
 * Subsequent phases plot the doors and flames after a 16-frame
 * delay (sub-phase advance + ambient SFX 7), then a 72-frame
 * stretch with the boom plotting tables unrolled below.
 */
...
/*
 * Overworld_AnimateEntrance_TurtleRock — 9-phase entrance:
 *   0-3: Drive a fixed-color filter from black to TR's red palette
 *        in 4 steps, each emitting a 0x10/0x14/0x18/0x1c brightness
 *        VRAM packet via the shared `common:` sentinel.
 *   4:   Black-out the boss palette rows (0x58/0x68) to set up the
 *        big tile-replacement pass.
 *   5:   Plot 16 tiles for the unsealed entrance, edit any 0x8aa
 *        placeholder tiles in the upload queue to 0x1e3, and
 *        configure subscreen + color math for the glow effect.
 *   6:   Pulse PaletteFilter_RestoreAdditive/Subtractive every 8
 *        frames for SFX 2 + the red glow ramp until turtlerock_ctr
 *        runs out, then advance to phase 7.
 *   7:   Continue the SFX pulse, then re-emit the TR tiles with
 *        normal color math (TS off, CGADSUB 0x20).
 *   8:   Final entrance jingle.
 */
...
/*
 * OverworldEntrance_PlayJingle — Common "entrance is now open" tail
 * for every dungeon-entrance cutscene. Plays SFX 27 (the chime),
 * clears every immobilization / NMI-suppression / shake flag, and
 * resets subsubmodule_index so normal gameplay resumes next frame.
 */
...
/*
 * OverworldEntrance_DrawManyTR — Bulk-plot the 16-cell Turtle Rock
 * entrance arch in a 4x4 grid (positions 0x99e..0xb24). The starting
 * map16 id 0xe78 increments through the next 16 ids so each cell
 * gets a unique tile.
 */
...
/*
 * Overworld_AnimateEntrance_GanonsTower — The most theatrical
 * entrance: 13 phases.
 *   0-1: GanonTowerEntrance_Func1 sets up the white-flash palette.
 *   2:   Three-cycle "white flash, fade back, repeat". After the
 *        third flash, queues ambient SFX 7 (low rumble).
 *   3-11: Per-phase tile plotting that grows the central pillar of
 *        light (each Overworld_DrawMap16_Persist call replaces a
 *        few tiles with the next progress sprite). OverworldEntrance_AdvanceAndBoom
 *        plays SFX 12+7 between phases for the explosion / boom.
 *   12:  Final entrance jingle, swap to GT music (track 13) and
 *        start the GT ambient SFX (9).
 */
...
/*
 * OverworldEntrance_AdvanceAndBoom — Step the entrance state machine
 * forward by one phase, reset the per-phase frame counter, and queue
 * the dual explosion SFX (12 + 7) heard between every phase boundary.
 */
