/*
 * variables.h — SNES WRAM Address-to-C-Variable Mapping Layer
 *
 * This file is the central bridge between the original SNES memory layout
 * and the C reimplementation. Every macro maps a named C variable to a
 * specific byte offset within g_ram[], a 128KB array that emulates the
 * SNES Work RAM (WRAM) at addresses $7E:0000-$7F:FFFF.
 *
 * The pattern `*(TYPE*)(g_ram + OFFSET)` dereferences a typed pointer into
 * the raw RAM buffer, allowing C code to read/write emulated SNES memory
 * as if it were a native variable. Array-style macros (without the leading
 * `*`) return pointers to arrays of values stored at consecutive offsets.
 *
 * Variable names were derived from reverse-engineering the original 65C816
 * assembly. Names prefixed with "byte_7E" or "word_7E" are unidentified
 * variables whose purpose has not yet been fully determined — the hex
 * suffix is the original SNES WRAM address.
 *
 * Many variables at the same offset serve different purposes depending on
 * the active game module (e.g., attract mode, file select, dungeon, overworld).
 * These are defined as separate macros mapping to the same g_ram offset,
 * which is intentional — the original SNES ROM reused the same RAM for
 * different subsystems that never run simultaneously.
 *
 * Architecture notes:
 *   - Offsets 0x0000-0x00FF: Zero-page variables (most frequently accessed)
 *   - Offsets 0x0100-0x01FF: Stack area
 *   - Offsets 0x0200-0x0FFF: Game state, sprites, ancillae, player data
 *   - Offsets 0x1000-0x1FFF: Tilemap upload buffers, intro/poly state
 *   - Offsets 0x2000-0x7FFF: BG tilemap data (BG1/BG2 each 8KB)
 *   - Offsets 0xC000-0xFFFF: Palette buffers, save data, extended state
 *   - Offsets 0x10000-0x1FFFF: Bank $7F — attribute tables, overlord data
 *
 * This file is included by virtually every .c file in the project.
 */
#ifndef ZELDA3_VARIABLES_H_
#define ZELDA3_VARIABLES_H_

/* ======================================================================
 * MODULE AND NMI CONTROL (0x10-0x1F)
 *
 * The game's main loop is driven by a module/submodule state machine.
 * main_module_index selects the active major mode (overworld, dungeon,
 * attract, file select, etc.), while submodule_index selects a sub-state
 * within that mode. NMI (Non-Maskable Interrupt) flags control what
 * gets uploaded to VRAM/CGRAM during the vertical blank period.
 * ====================================================================== */
...
/* ======================================================================
 * LINK POSITION AND MOVEMENT (0x20-0x6F)
 *
 * Core player character (Link) spatial state. Coordinates use 16-bit
 * values in pixel space; subpixel fields provide fractional precision
 * for smooth movement. Velocity values are signed 8-bit.
 * The SNES uses separate hi/lo bytes for coordinates in many contexts;
 * this reimplementation uses uint16 for direct 16-bit access.
 * ====================================================================== */
...
// Tile detection results — set by the tile collision system to report
// what type of tile Link is standing on or interacting with
...
/* ======================================================================
 * SCRATCH REGISTERS AND TEMPORARIES (0x72-0x78)
 *
 * General-purpose scratch variables used as temporaries within
 * subroutines. These mirror the original 65C816's use of zero-page
 * locations as fast-access scratch space.
 * ====================================================================== */
...
/* ======================================================================
 * MAP16 AND OVERWORLD SCREEN STATE (0x84-0x8F)
 *
 * Map16 is the SNES tile system where each 16x16 metatile is composed
 * of four 8x8 tiles. These variables track source/destination offsets
 * for tilemap loading and the current overworld screen.
 * ====================================================================== */
...
// OAM (Object Attribute Memory) allocation pointers — track current
// write position in the 128-entry sprite table and its extension table
#define oam_cur_ptr (*(uint16*)(g_ram+0x90))
#define oam_ext_cur_ptr (*(uint16*)(g_ram+0x92))
/* ======================================================================
 * PPU REGISTER SHADOW COPIES (0x94-0x9E)
 *
 * The SNES PPU registers are write-only, so the game maintains shadow
 * copies in RAM. These are written to the actual PPU registers during
 * NMI (vblank). Names match the official SNES register names with a
 * "_copy" suffix: BGMODE, MOSAIC, window mask selects (W12SEL, etc.),
 * color math settings (CGWSEL, CGADSUB), and fixed color data (COLDATA).
 * ====================================================================== */
...
/* ======================================================================
 * DUNGEON STATE (0xA0-0xBD)
 *
 * State variables for the dungeon (indoor) rendering system. Each dungeon
 * room has a header defining its layout, collision type, background
 * properties, and special tags (e.g., boss rooms, dark rooms). The
 * "quadrant" system divides large rooms into 256x256-pixel sections
 * for scrolling and layer management.
 * ====================================================================== */
...
/* ======================================================================
 * BG SCROLL OFFSET COPIES (0xE0-0xEE)
 *
 * Secondary copies of BG layer scroll offsets. The SNES has three
 * independently scrollable background layers (BG1, BG2, BG3). Multiple
 * copies exist because different systems (camera, transitions, HDMA
 * effects) need to save/restore scroll positions independently.
 * ====================================================================== */
...
/* ======================================================================
 * JOYPAD INPUT STATE (0xF0-0xFA)
 *
 * Buffered controller input. The SNES joypad register is split into
 * high byte (H: shoulder buttons, d-pad) and low byte (L: face buttons).
 * "filtered" versions contain only newly-pressed bits (edge detection),
 * while "last" versions retain the previous frame's held state.
 * ====================================================================== */
...
/* ======================================================================
 * LINK DMA AND GRAPHICS TRANSFER (0x100-0x10F)
 *
 * Variables controlling DMA transfers of Link's sprite graphics to VRAM.
 * The SNES uses DMA to bulk-copy tile data during vblank.
 * ====================================================================== */
...
// BG layer scroll offsets — primary copies used by the camera system
...
/* ======================================================================
 * SOUND AND MUSIC CONTROL (0x12C-0x136)
 *
 * Interface between the main CPU and the SPC700 sound processor.
 * Writing to music_control triggers track changes; sound_effect_1/2
 * queue SFX. The SPC700 reads these values via the APU I/O ports.
 * ====================================================================== */
...
/* ======================================================================
 * HUD AND MAP STATE (0x200-0x223)
 *
 * Variables for the heads-up display (heart meter, item selector,
 * rupee counter) and the dungeon map screen. The HUD is rendered
 * on BG3 and updated during NMI via buffered tile writes.
 * ====================================================================== */
...
/* ======================================================================
 * ANCILLA STATE (0x280-0x3F8)
 *
 * Ancillae are non-sprite interactive objects: boomerangs, arrows, bombs,
 * hookshot chain, magic powder, fire/ice rod shots, sword beams, etc.
 * The game supports up to 5 simultaneous ancillae (indices 0-4, with
 * some arrays extended to 10 for effects like door debris).
 *
 * Each ancilla has position, velocity, type, timer, direction, floor
 * level, and various type-specific state variables stored in parallel
 * arrays. Single-letter array names (A, B, G, H, etc.) are generic
 * state slots whose meaning varies per ancilla type.
 * ====================================================================== */
...
/* ======================================================================
 * TILE DETECTION AND PLAYER INTERACTION (0x2C0-0x37F)
 *
 * Results from the tile collision detection system. When Link moves,
 * the game checks which map16 tiles he overlaps and sets these flags
 * to indicate special tile types: staircases, pits, deep water, ice,
 * conveyor belts, thick grass, spikes, etc. The player movement
 * code reads these to determine movement restrictions and effects.
 * ====================================================================== */
...
// Item receipt state — controls the "Link holds item above head" animation
...
/* ======================================================================
 * PLAYER STATE MACHINE (0x300-0x37F)
 *
 * Extended player state variables governing Link's action state machine.
 * The player handler supports 31+ states (walking, dashing, swimming,
 * lifting, throwing, using items, taking damage, etc.). These variables
 * track timers, animation frames, spin attack progression, dash
 * countdowns, ledge jumps, and interaction flags.
 * ====================================================================== */
...
/* ======================================================================
 * DUNGEON ROOM OBJECTS AND DOOR STATE (0x400-0x4CA)
 *
 * Per-room dungeon state: which doors have been opened, savegame
 * state bits (chests opened, boss defeated), visited quadrants,
 * staircase tracking, torch timers, movable blocks, trap doors,
 * water gates, blast walls, and floor toggles. Many of these
 * arrays are indexed by room-relative object indices.
 * ====================================================================== */
...
/* ======================================================================
 * DUNGEON TILE REPLACEMENT AND PUSHABLE BLOCK ARRAYS (0x4F0-0x62F)
 *
 * The dungeon engine dynamically replaces tiles when doors open, chests
 * are unlocked, blocks are pushed, or switches are activated. These
 * parallel arrays store the tilemap positions, replacement tile values,
 * and state for each replaceable object in the current room.
 * ====================================================================== */
...
/* ======================================================================
 * CAMERA AND SCROLL BOUNDARIES (0x610-0x62F)
 *
 * Camera scroll limits define the visible area boundaries. The camera
 * system enforces these limits to prevent scrolling beyond room/screen
 * edges. Subpixel scroll offsets enable smooth per-pixel scrolling.
 * ====================================================================== */
...
// Mode 7 transformation center coordinates (for the overworld map view)
...
/* ======================================================================
 * SPOTLIGHT AND WATER HDMA EFFECTS (0x670-0x69F)
 *
 * Variables for HDMA-driven visual effects: the spotlight/iris effect
 * (used in dark rooms with the lantern) and water surface wave
 * distortion. HDMA (Horizontal DMA) modifies PPU registers on each
 * scanline to create per-scanline visual effects.
 * ====================================================================== */
...
/* ======================================================================
 * OAM BUFFERS (0x800-0xA20)
 *
 * The SNES OAM (Object Attribute Memory) holds 128 sprite entries plus
 * a 32-byte extension table. oam_buf (at 0x800) is the 512-byte main
 * table; extended_oam provides the extra high-X bit and size-select bit
 * per sprite. bytewise_extended_oam is an unpacked version used during
 * sprite building before being compressed into the 2-bit-per-sprite format.
 * ====================================================================== */
#define extended_oam ((uint8*)(g_ram+0xA00))
#define bytewise_extended_oam ((uint8*)(g_ram+0xA20))
#define byte_7E0AA0 (*(uint8*)(g_ram+0xAA0))
/* ======================================================================
 * GRAPHICS THEME AND PALETTE INDICES (0xAA1-0xABF)
 *
 * Indices selecting which tileset and palette combination to load for
 * the current area. The SNES has limited VRAM, so tilesets are swapped
 * as the player moves between areas. Palette indices control which
 * color sets are loaded into CGRAM for sprites and backgrounds.
 * ====================================================================== */
...
/* ======================================================================
 * DMA SOURCE ADDRESS TABLE (0xAC0-0xAF8)
 *
 * Source addresses for tile/palette DMA transfers to VRAM. Each slot
 * corresponds to a VRAM region (BG tiles, sprite tiles, animated tiles).
 * These are set up during area loading and triggered during NMI.
 * ====================================================================== */
...
/* ======================================================================
 * OVERLORD STATE (0xB00-0xB57)
 *
 * Overlords are room-level controllers that manage groups of sprites
 * or trigger environmental effects (e.g., pipe-crawling enemies,
 * falling tile patterns, cannon sequences). Each overlord has position,
 * type, and 3 general-purpose state variables. Up to 8 overlords can
 * be active simultaneously per room.
 * ====================================================================== */
...
/* ======================================================================
 * SPRITE SUPPLEMENTAL ARRAYS (0xB58-0xBEF)
 *
 * Extended per-sprite state that did not fit in the main sprite arrays.
 * Includes stun timers, collision flags, object priority, projectile
 * immunity flags, and generic state slots (sprite_N, sprite_unk2).
 * All arrays are indexed by sprite slot number (0-15).
 * ====================================================================== */
...
/* ======================================================================
 * ANCILLA CORE ARRAYS (0xBF0-0xC9F)
 *
 * Main parallel arrays for ancilla objects (projectiles, items, effects).
 * Each array has one entry per ancilla slot (up to 10 entries).
 * Position (y_lo/hi, x_lo/hi), velocity, subpixel precision, type ID,
 * step counter (animation phase), item-to-Link delivery flag, cooldown
 * timer, facing direction, floor level, and OAM sprite allocation index.
 * ====================================================================== */
...
/* ======================================================================
 * SPRITE CORE ARRAYS (0xD00-0xFA0)
 *
 * The main sprite system supports 16 simultaneous sprites (indices 0-15).
 * These parallel arrays store all per-sprite state in the struct-of-arrays
 * pattern used by the original SNES code (each array at a fixed WRAM
 * offset, 16 bytes apart).
 *
 * Key arrays:
 *   - y_lo/hi, x_lo/hi: 16-bit position split across two arrays
 *   - y_vel, x_vel: signed 8-bit velocity per axis
 *   - ai_state: current AI behavior state (type-specific FSM index)
 *   - sprite_state: lifecycle state (0=dead, active states vary)
 *   - sprite_type: enemy/NPC type ID (indexes into behavior tables)
 *   - health: hit points remaining
 *   - A-G: generic state slots whose meaning varies per sprite type
 *   - delay_main/aux1-4: countdown timers for AI state transitions
 *   - hit_timer: invincibility flash countdown after taking damage
 *   - flags2-5: per-sprite behavior flags (from ROM sprite data)
 *   - z, z_vel, z_subpos: vertical position for jumping/bouncing sprites
 * ====================================================================== */
...
/* ======================================================================
 * SPRITE COLLISION AND MISC (0xFA0-0xFFF)
 *
 * Sprite collision detection variables, repulse spark effect state,
 * sorting control, OAM region allocation, and various single-purpose
 * flags. is_in_dark_world at 0xFFF is a key gameplay flag that affects
 * which tileset, palette, music, and enemy set are active.
 * ====================================================================== */
...
/* ======================================================================
 * DOOR AND DUNGEON EXIT DATA (0x1800-0x19FF)
 *
 * Per-room door metadata: type/slot encoding, tilemap addresses for
 * door graphics, scroll direction through doors, and exit door lists.
 * The dungeon transition system uses these to animate door openings
 * and route the player between rooms.
 * ====================================================================== */
...
/* ======================================================================
 * TAGALONG (FOLLOWER) STATE (0x1A00-0x1AFF)
 *
 * Position history for NPCs that follow Link (Zelda, old man, etc.).
 * The tagalong stores a trail of 20 position snapshots so the follower
 * walks behind Link along his exact path with a configurable delay.
 * Bird travel coordinates store the 8 flute destinations.
 * ====================================================================== */
...
/* ======================================================================
 * TEXT AND DIALOGUE SYSTEM (0x1CD0-0x1CF4)
 *
 * State machine for the dialogue text rendering system. Controls the
 * message box position, text cursor, character-by-character rendering
 * speed, scroll behavior, and multi-choice selection state. The text
 * engine reads compressed dialogue data and renders it to a BG3 tilemap
 * region with variable-width font support.
 * ====================================================================== */
...
/* ======================================================================
 * ALT SPRITE SYSTEM (0x1D00-0x1DFF)
 *
 * Secondary sprite table used for environment sprites that coexist with
 * the main 16-slot sprite system. These handle decorative/ambient sprites
 * like birds, fish, and other background animations that should not
 * consume slots needed for enemies and NPCs.
 * ====================================================================== */
...
/* ======================================================================
 * INTRO SEQUENCE AND TRIFORCE ANIMATION (0x1E00-0x1F0C)
 *
 * State for the game's intro cinematic (rain scene, Zelda's telepathy)
 * and the Triforce reveal animation. The intro uses its own sprite
 * system with 8 slots for rain/lightning effects, independent of the
 * main game sprite tables.
 * ====================================================================== */
...
/* ======================================================================
 * POLYHEDRAL OBJECT RENDERER (0x1F01-0x1FFC)
 *
 * 3D polygon renderer used for the spinning Triforce model in the
 * intro/ending sequences. Implements rotation matrices (sin/cos of
 * angles a and b), vertex transformation, edge walking, and scanline
 * rasterization. The renderer supports flat-shaded convex polygons
 * with configurable vertex count and color modes. Rendered output
 * goes to a dedicated buffer for NMI upload to VRAM.
 * ====================================================================== */
...
/* ======================================================================
 * BG TILEMAP BUFFERS (0x2000-0x9FFF)
 *
 * Full 8KB tilemap buffers for BG1 and BG2. Each entry is a 16-bit
 * tile descriptor: 10-bit tile number + palette + priority + flip flags.
 * BG2 is typically the lower layer (floor/walls), BG1 is the upper
 * layer (foreground objects, upper walls in dungeons). In overworld
 * mode, overworld_tileattr overlaps the BG2 region for tile attribute
 * lookups (collision type, overlay flags).
 * ====================================================================== */
...
/* ======================================================================
 * EXTENDED STATE — BANK $7E UPPER / BANK $7F (0xC000+)
 *
 * The upper portion of WRAM contains lighting/palette state, area
 * exit snapshots, cached scroll positions for room transitions,
 * palette buffers, and various backup copies of state variables
 * needed for mode transitions (dungeon↔overworld, death/continue,
 * mirror warps, special exit handling).
 * ====================================================================== */
...
/* Special exit snapshot — saved state for returning to the overworld
 * from special entrance locations (e.g., after dying in a dungeon).
 * Contains the full set of camera, scroll, and position variables
 * needed to reconstruct the overworld view. */
...
// Normal exit snapshot — saved state for the most recent dungeon exit point
...
// Cached room state — snapshot for inter-room transitions within dungeons.
// Saved when entering a new room, restored if the transition is cancelled.
...
/* Map screen backup — state saved when opening the dungeon/overworld map
 * so it can be restored when the map is closed */
...
/* ======================================================================
 * PALETTE AND HUD TILE BUFFERS (0xC300-0xC87F)
 *
 * Double-buffered 512-byte (256-color) palette areas. aux_palette_buffer
 * holds dynamically loaded palettes; main_palette_buffer holds the active
 * palette uploaded to CGRAM during NMI. hud_tile_indices_buffer stores
 * the tilemap data for the HUD overlay on BG3.
 * ====================================================================== */
...
/* ======================================================================
 * SAVE DATA / SRAM MIRROR (0xF000-0xF4FF)
 *
 * This region mirrors the cartridge SRAM (battery-backed save memory).
 * save_dung_info stores per-room state (chests opened, boss defeated,
 * keys used) for all 296 dungeon rooms. save_ow_event_info stores
 * per-screen overworld events (diggable items found, bonk items, etc.).
 * ====================================================================== */
...
/* ======================================================================
 * LINK INVENTORY (0xF340-0xF37B)
 *
 * Link's item inventory, stored in SRAM at the same offsets as the
 * original game. Each item is a uint8: 0 = not owned, nonzero values
 * indicate item tier/variant (e.g., bow=1 normal, bow=2 silver arrows;
 * boomerang=1 blue, boomerang=2 red). Consumable counts (bombs, arrows,
 * rupees) and capacity upgrades are stored here alongside equipment
 * flags (pendants, crystals, abilities).
 * ====================================================================== */
...
/* ======================================================================
 * SAVE GAME PROGRESS AND FOLLOWER STATE (0xF3C5-0xF405)
 *
 * High-level game progression: which dungeons are cleared, whether
 * Link is in the Dark World, who is following Link (Zelda, old man,
 * etc.), and death/continue counters used for the ending score screen.
 * ====================================================================== */
...
// Memorized tile system — tracks tiles that were temporarily modified
// (e.g., by bombs, sword slashes, dashing into trees) so they can be
// restored when the player leaves and re-enters the area
...
/* ======================================================================
 * BANK $7F — OVERLAPPING SUBSYSTEM BUFFERS (0x10000+)
 *
 * Bank $7F (WRAM addresses $7F:0000-$7F:FFFF) is used as temporary
 * workspace by several subsystems that never run simultaneously.
 * The same physical RAM is aliased by multiple macro sets:
 *   - skullwoodsfire_*: Skull Woods dungeon fire trap animation
 *   - blastwall_*: Bombable/cracked wall destruction effect
 *   - breaktowerseal_*: Ganon's Tower barrier-breaking sequence
 *   - happiness_pond_*: Fairy fountain item upgrade animation
 *   - weathervane_*: Kakariko weathervane/flute boy scene
 *   - quake_/ether_/bombos_*: Medallion spell visual effects
 * This memory reuse is safe because these events are mutually exclusive.
 * ====================================================================== */
...
// Dungeon tile attribute tables — per-tile collision/behavior flags
// mirroring the BG tilemap layout (one attribute byte per metatile)
...
// Damage table loaded from ROM — per-enemy-type resistance/weakness data
// for each weapon type. 16 bytes per enemy type.
...
/* ======================================================================
 * GARNISH SYSTEM (0x1F800-0x1FA1B)
 *
 * Garnishes are lightweight visual effects attached to sprites: dust
 * puffs, water splashes, sparkles, small debris. They have their own
 * position/velocity/animation state and are independent of the main
 * sprite and ancilla systems. Up to 30 garnishes can be active.
 * ====================================================================== */
...
/* Moldorm/Swamola/Beamos segment arrays — multi-segment boss and enemy
 * position tracking. These enemies have articulated bodies where each
 * segment has its own x/y position stored in these arrays. */
...
/* ======================================================================
 * ATTRACT MODE / FILE SELECT OVERLAY VARIABLES
 *
 * These macros reuse the same low WRAM addresses as the main game
 * variables (link_y_coord, etc.) because attract mode and file select
 * run in entirely different game modules where the main game state
 * is not active. This is the same memory aliasing pattern the original
 * SNES ROM uses to minimize RAM consumption.
 * ====================================================================== */
...
/* ======================================================================
 * ADDITIONAL ALIASES AND UTILITY MAPPINGS
 *
 * 16-bit scratch register aliases, SRAM variable, messaging buffer,
 * and spell effect arrays (quake, ether, bombos) that overlay the
 * temporary workspace in bank $7F.
 * ====================================================================== */
...
/* ======================================================================
 * MISCELLANEOUS ALIASES AND LATE ADDITIONS
 *
 * Additional variable aliases added during development. Some share
 * offsets with earlier definitions (e.g., sprite_N_word is a 16-bit
 * view of the same data as sprite_N). vram_upload_* provides a
 * structured view of the VRAM upload buffer at 0x1000.
 * ====================================================================== */
...
// R10-R20: Direct-page register aliases used by the 65C816 CPU emulation
// bridge. These correspond to specific direct-page addresses that the
// original assembly code uses as pseudo-registers for subroutine arguments.
...
/* ======================================================================
 * STRUCT DEFINITIONS AND TYPED BUFFER VIEWS
 *
 * Structs providing structured access to regions of g_ram that contain
 * multi-field records rather than simple scalar values.
 * ====================================================================== */

/* MovableBlockData — Records which room a pushed block belongs to
 * and its tilemap position, for save/restore across room transitions. */
typedef struct MovableBlockData {
  uint16 room;      // Dungeon room index where this block was pushed
  uint16 tilemap;   // Tilemap offset of the block's current position
} MovableBlockData;

/* OamEntSigned — Same layout as OamEnt but with signed x/y coordinates,
 * used for sprite offset tables where relative positions can be negative. */
...
/* RoomBounds — Scroll boundaries for a dungeon room axis (Y or X).
 * a0/b0 define the scrollable range for quadrant 0, a1/b1 for quadrant 1.
 * The union allows both named and indexed access to the four values. */
...
/* OwScrollVars — Overworld scroll boundaries for an area. Defines the
 * pixel limits the camera can scroll to in each direction. Two instances
 * exist: ow_scroll_vars0 for the current area and ow_scroll_vars1 for
 * the adjacent area during screen transitions. */
...
/* ======================================================================
 * EXTERNAL DATA TABLE DECLARATIONS
 *
 * ROM data tables extracted from the original game and stored as C
 * arrays. These provide lookup data for room layouts, item properties,
 * overworld coordinates, and other constant game data.
 * ====================================================================== */
...
/* MirrorHdmaVars — State for the Magic Mirror's HDMA warp effect.
 * Controls the expanding/contracting circular distortion that plays
 * when Link uses the Magic Mirror to warp between worlds. The effect
 * is implemented via HDMA writes to BG scroll registers, creating a
 * wavy circular transition centered on Link's position. */
...
/* VRAM upload buffer structs — structured views of the g_ram region at
 * 0x1000 used to stage tile data before DMA transfer to VRAM during NMI.
 * UploadVram_32x32 represents a full 32x32 tile screen worth of data.
 * UploadVram_3 provides access to 4 data words after a 256-byte pad
 * (used for partial tilemap uploads). */
...
// The emulated SNES WRAM — 128KB (banks $7E-$7F) backing all the macros above
