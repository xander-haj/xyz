/*
 * dungeon.c — Dungeon Room Engine
 *
 * Implements the complete indoor/dungeon subsystem for Zelda: A Link
 * to the Past.  This file covers:
 *
 *   - Room loading and header parsing (320 rooms in a 16x20 grid)
 *   - Tilemap construction from three object-subtype layers
 *   - Door rendering, state machines, and 38 door types (shutters,
 *     locked, bombable, slashable, blast-wall, curtain, entrance, etc.)
 *   - Room tag routines (66 puzzle triggers: kill-all-enemies, torch
 *     puzzles, water gates, moving walls, crystal pegs, pressure plates)
 *   - Intra-room and inter-room screen transitions with scrolling,
 *     filter fades, mosaic effects, and camera quadrant management
 *   - Staircase system: spiral stairs, straight inter/intra-room stairs,
 *     pit falls, and warp-pad teleportation
 *   - Push-block mechanics with collision, pit detection, and savegame
 *     persistence via movable_block_datas
 *   - Water/flood effects (flood dam HDMA expansion, swamp drain/fill,
 *     water gate animated tile replacement)
 *   - Layer effects (scrolling floors for Trinexx, lightning for
 *     Agahnim 2, invisible floors, Ganon's torch-lit room)
 *   - Attribute table (0x2000-byte collision map) population for tiles,
 *     objects, doors, staircases, chests, and torches
 *   - HUD/palette updates, entrance initialization, and boss-room exits
 *
 * Coordinate system: each room occupies a 64x64 tile grid addressed
 * by the XY(x,y) macro (= y*64 + x).  The grid is split into four
 * quadrants (NW, NE, SW, SE) whose visibility is tracked per-room for
 * camera scrolling and transition logic.
 *
 * The file is organized roughly as:
 *   1. Static lookup tables and dispatch arrays
 *   2. Low-level tilemap drawing primitives
 *   3. Object subtype loaders (walls, floors, decorations, specials)
 *   4. Door rendering and attribute system
 *   5. Room tag (puzzle) routines
 *   6. Chest/push-block/tile interaction handlers
 *   7. Transition state machines (intra-room, inter-room, stairs, warp)
 *   8. Module entry points (Module_PreDungeon, Module07_Dungeon)
 *   9. Camera scrolling and quadrant management
 *  10. Entrance loading and layer effect handlers
 */
#include "zelda_rtl.h"
...
/* Return true when widescreen play should collapse the visible dungeon
 * transition waits.  The room loaders still run normally; only the scroll,
 * fade-wait, and doorway-walk presentation frames are fast-forwarded. */
static bool Dungeon_ShouldUseOpenWorldTransitions() {
  return (enhanced_features0 & (kFeatures0_ExtendScreen64 | kFeatures0_WidescreenVisualFixes)) != 0;
}

/* Room IDs for each dungeon's boss room.  Indexed by dungeon number
 * (0 = Eastern Palace boss room 200, 1 = Desert Palace room 51, etc.).
 * Used to trigger boss-fight exits and post-boss transitions. */
// todo: move to config
...
/* Boss-to-exit room mapping: kDungeonExit_From holds the boss room ID
 * and kDungeonExit_To holds the destination room after the boss is
 * defeated.  Parallel arrays indexed by dungeon number. */
...
/* Offsets into the room object tile data for subtype-1 objects.
 * Subtype 1 covers variable-length room geometry: walls, floors, pits,
 * railing, water edges, and other repeating structures.  Each entry
 * is a byte offset into the compressed room object tile definitions
 * that LoadType1ObjectSubtype1() indexes via the object ID (0x00-0xF7). */
...
/* Offsets for subtype-2 objects: fixed-size decorative and structural
 * elements such as wall corners, S-bends, statues, star switches,
 * torches, chests, staircases, and water structures.  Indexed by
 * subtype-2 object ID. */
...
/* Offsets for subtype-3 objects: special/complex room elements such as
 * water faces, cane-ride platforms, prison cells, rupee floors,
 * staircases, Agahnim's altar/windows, pots, bomb-able floors, pipes,
 * and the Triforce pedestal.  Indexed by subtype-3 object ID. */
...
/* Door tile source data tables (4 directional variants).
 * Each table maps a door type index to a byte offset into the door
 * tile graphics data.  kDoorTypeSrcData is for north-facing doors,
 * kDoorTypeSrcData2 for south, kDoorTypeSrcData3 for west,
 * kDoorTypeSrcData4 for east.  Door types include: open, locked,
 * bombable, shutter, key-door, big-key-door, curtain, entrance, etc. */
...
/* Tilemap byte offsets for each door position in the four cardinal
 * directions.  Each array holds 12 entries (3 positions per quadrant,
 * 4 quadrants) mapping a door slot to the starting tilemap address
 * where the door tiles are written.  Used by the door rendering
 * system to place door graphics at the correct screen location. */
...
/* Spiral staircase direction multipliers for camera offset computation.
 * Paired values control horizontal and vertical movement per step. */
static const int8 kSpiralTab1[] = { 0, 1, 1, -1, 1, 1, 1, 1 };
/* Floor-level offsets for pit/teleport transitions.  When Link falls
 * through a pit, these tables determine how many dungeon floors down
 * he drops based on the current level indicator. */
static const int8 kTeleportPitLevel1[] = { 0, 1, 1 };
static const int8 kTeleportPitLevel2[] = { 0, 0, 1 };
/* Remaps raw door type codes to normalized rendering indices.  Many
 * door types share the same visual appearance (e.g., several locked
 * variants map to the same tile set).  Zero entries mean "no remap
 * needed" (passthrough or unused). */
...
/* Pixel offsets for Link's position when entering/exiting staircases.
 * First 10 entries are for upward staircases, next 10 for downward.
 * Values encode the Y-pixel displacement to place Link at the correct
 * landing spot relative to the staircase tile. */
static const int8 kStaircaseTab2[] = {
  12, 32, 48, 56, 72, -44, -40, -64, -64, -88, 12, 24, 40, 48, 64, -28,
  -40, -56, -64, -80,
};
/* Sub-pixel X adjustments for staircase alignment by direction. */
static const int8 kStaircaseTab3[] = { 4, -4, 4, -4 };
/* Link's Y-coordinate landing positions for each staircase variant. */
static const int8 kStaircaseTab4[] = { 52, 52, 59, 58 };
/* Camera Y-scroll offsets applied during staircase transitions. */
static const int8 kStaircaseTab5[] = { 32, -64, 32, -32 };
/* Moving wall tile counts and pixel distances for the four wall sizes.
 * Sizes0 = number of tile rows in the wall segment.
 * Sizes1 = total pixel travel distance for the moving wall. */
static const uint8 kMovingWall_Sizes0[4] = { 5, 7, 11, 15 };
static const uint8 kMovingWall_Sizes1[4] = { 8, 16, 24, 32 };
/* Water gate tile expansion layout.  Groups of 3 bytes encode
 * (position, tile_a, tile_b) for each expansion step.  The 0xFF
 * sentinel terminates the list.  Controls the animated opening
 * sequence of water gates in rooms like the Swamp Palace. */
...
/* Bitmasks for the 6 chest slots per room.  OR'd into the room's
 * persistent save data to track which chests have been opened. */
static const uint16 kChestOpenMasks[] = { 0x100, 0x200, 0x400, 0x800, 0x1000, 0x2000 };
/* Per-layout quadrant visibility flags.  Each room layout (0-31) has
 * a bitmask where bits 3-0 represent NW/NE/SW/SE quadrants.  A set
 * bit means that quadrant is part of the room's visible area. */
...
/* Maps a quadrant index (0-15) to the bitmask that should be set in
 * the room's visited-quadrants tracker when that quadrant is entered.
 * Multi-bit values represent merged quadrant rooms (e.g., 0xC = both
 * NW+NE visible in a wide horizontal layout). */
static const uint8 kQuadrantVisitingFlags[] = { 8, 4, 2, 1, 0xC, 0xC, 3, 3, 0xA, 5, 0xA, 5, 0xF, 0xF, 0xF, 0xF };
/* Convert tile (x, y) to a linear tilemap index.  The room is laid
 * out as a 64-tile-wide grid, so each row is 64 entries. */
#define XY(x, y) ((y)*64+(x))
/* Item IDs awarded from the 8 chests in the minigame room.
 * Arranged so the prize distribution matches the original game. */
static const uint8 kDungeon_MinigameChestPrizes1[8] = {
  0x40, 0x41, 0x34, 0x42, 0x43, 0x44, 0x27, 0x17
};
/* Extended prize table for the rupee chest minigame (32 entries).
 * Contains rupee item IDs (0x34=green, 0x41=blue, 0x46=red, 0x47=big)
 * shuffled to create a randomized-feeling reward distribution. */
...
/* Pixel offsets from Link's position to test each of the 4 directions
 * for a liftable tile.  The query checks the attribute map at
 * (link_x + _x[dir], link_y + _y[dir]). */
static const int8 kDungeon_QueryIfTileLiftable_x[4] = { 7, 7, -3, 16 };
static const int8 kDungeon_QueryIfTileLiftable_y[4] = { 3, 24, 14, 14 };
/* Return values indicating the liftable tile type found at each
 * attribute value (0x70=light pot, 0x50=heavy pot, 0x54=sign, etc.).
 * The high/low bytes encode the tile pair for 2-tile-wide objects. */
static const uint16 kDungeon_QueryIfTileLiftable_rv[16] = { 0x5252, 0x5050, 0x5454, 0x0, 0x2323 };
/* Tilemap destination offsets for blast-wall (exploding wall) debris.
 * Used to clear the tile rows affected by a north-facing bomb wall. */
static const uint16 kDoor_BlastWallUp_Dsts[] = { 0xd8a, 0xdaa, 0xdca, 0x2b6, 0xab6, 0x12b6 };
/* Flags indicating which door slots in the adjacent (scrolled-off)
 * room have been opened/modified during transitions. */
#define adjacent_doors_flags (*(uint16*)(g_ram+0x1100))
/* Array of door state words for the adjacent room, mirroring the
 * current room's door_type_and_slot[] during inter-room transitions. */
#define adjacent_doors ((uint16*)(g_ram+0x1110))
/* Dungeon palette configuration for each dungeon area.  Each entry
 * specifies {main_palette, sprite_palette, bg_palette, aux_palette}
 * indices used when loading the room's color data.  The 41 entries
 * cover every distinct dungeon/cave area in the game. */
...
/* BG1 and BG2 VRAM upload source offsets for room object tiles.
 * These tables encode (low_byte, high_byte, bank) triplets mapping
 * each of the 11 tile-upload slots to a VRAM source address.
 * Not referenced by game logic; retained for binary-exact ROM
...
/* VRAM upload source offsets for the 16 BG tilemap quadrant transfers.
 * Each entry is the byte offset into the room tilemap buffer to begin
 * copying from when uploading that quadrant's tile data to VRAM. */
...
/* Destination slot indices (1-16) for the quadrant VRAM uploads.
 * Controls which NMI upload slot receives each tilemap chunk. */
static const uint8 kUploadBgDsts[] = { 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 4, 8, 12, 16 };
/* Collision attribute pairs written into the attribute table for each
 * door type.  High byte = left/top tile, low byte = right/bottom tile.
 * Values like 0x80 = solid wall, 0x84 = locked door, 0x8E = big-key
 * door, 0x88 = bombable, 0x82 = shutter, 0x01 = open passage. */
...
/* Layer-effect handler dispatch table.  Each dungeon room can specify
 * a BG layer effect (index 0-7) that runs every frame.  Entries:
 *   0-1: Nothing (static background)
 *   2: Scroll (floor scrolls under Link, e.g. conveyor belts)
 *   3: WaterRapids (animated water current in Swamp Palace)
 *   4: Trinexx (scrolling floor in the Trinexx boss room)
 *   5: Agahnim2 (lightning flashes in Ganon's Tower)
 *   6: InvisibleFloor (floor appears only when Link steps on it)
 *   7: Ganon (torch-dependent room visibility) */
...
/* Signed pixel distances for push-block movement in each direction.
 * Index 0=up (-256 sub-pixels = -1 tile), 1=down (+256), 2=left (-4),
 * 3=right (+4).  Applied incrementally each frame during the slide. */
static const int16 kPushBlockMoveDistances[] = { -0x100, 0x100, -0x4, 0x4 };
/* Room tag routine dispatch table (66 entries).  Each dungeon room has
 * a "tag" byte in its header that selects one of these handlers to run
 * every frame.  Tags implement room-specific puzzle logic:
 *   0x00: no-op (most rooms)
 *   0x01-0x08, 0x29-0x30: directional/quadrant/room kill-triggers
 *   0x09-0x0A: quadrant/room-wide triggers
 *   0x14: prize-triggered double door
 *   0x15-0x16: switch-hold / switch-toggle doors
 *   0x17-0x19: water off / water on / water gate
 *   0x1B: unused tag
 *   0x1C-0x1D: moving wall east/west
 *   0x1E-0x1F: moving wall + torch check
 *   0x20: switch-triggered exploding wall
 *   0x21-0x22: pit holes (kill-all / chest variant)
 *   0x24: holes type 2
 *   0x25: get heart for prize
 *   0x26: kill-room block
 *   0x27: trigger chest
 *   0x28: pull-switch exploding wall
 *   0x33: torch puzzle door
 *   0x38: Agahnim battle tag
 *   0x3C: push block for chest
 *   0x3D: Ganon's door
...
/* Door opening/closing animation tile source offsets, one per frame
 * (5 frames) for each cardinal direction.  These point into the door
 * animation tileset and are cycled through by the shutter/opening
...
/* Intra-room (sub-tile) transition state machine — 8 steps.
 * Handles scrolling between quadrants within a single room without
 * loading new room data.  Sequence: prep → fade filter → reset
 * shutters → scroll camera → find landing → state5 → filter →
 * trigger shutters. */
...
/* Inter-room (super-tile) transition state machine — 16 steps.
 * Handles moving between adjacent dungeon rooms.  Involves loading
 * the new room's data, applying palette filters, syncing BG layers,
 * handling dark-room lighting, and scrolling the camera to the new
 * room's starting position. */
...
/* Falling/down-floor transition state machine — 18 steps.
 * Triggered when Link falls through a pit or descends a floor.
 * Handles music change, room reset, palette/BG updates, sprite
 * graphics reload, fade-in, landing animation, and room caching. */
...
/* Flood dam / water gate animation state machine — 6 steps.
 * Manages the Swamp Palace flood sequence: prepare tiles → three
 * frames of main expansion → expand boundaries → fill remaining. */
...
/* Per-direction screen offset deltas applied when repositioning Link after a
 * spiral staircase transition.  Indexed by facing direction (0=up,1=down,
 * 2=left,3=right).  X is the pixel nudge left/right; Y is up/down. */
static const int8 kSpiralStaircaseX[] = { -28, -28, 24, 24 };
static const int8 kSpiralStaircaseY[] = { 16, -10, -10, -32 };
/* Spiral staircase inter-room transition — 20-step state machine.
 * Steps 0-6 are shared with the fat-staircase transition (init, music,
 * filter, room load, BG update), steps 7-10 fade BG layers in pairs,
 * steps 11-17 animate the scroll and grayscale, step 18 sets boss music,
 * step 19 caches the destination room entry properties. */
...
/* Landing wipe spotlight — 2-step state machine used when Link falls
 * through a pit and the screen fades in via an expanding circle wipe. */
static PlayerHandlerFunc *const kDungeon_Submodule_F[2] = {
  &Module07_0F_00_InitSpotlight,
  &Module07_0F_01_OperateSpotlight,
};
/* South-facing (upward-travelling) intra-room straight staircase — 2 steps.
 * Step 0: init positions.  Step 1: advance Link one stair tile per frame. */
static PlayerHandlerFunc *const kDungeon_StraightStaircase[2] = {
  &Module07_10_00_InitStairs,
  &Module07_10_01_ClimbStairs,
};
/* North-facing (downward-travelling) intra-room straight staircase — 2 steps. */
static PlayerHandlerFunc *const kDungeon_StraightStaircaseDown[2] = {
  &Module07_08_00_InitStairs,
  &Module07_08_01_ClimbStairs,
};
/* Inter-room straight staircase transition — 19-step state machine.
 * Covers the full sequence: fade out, load destination room, filter BG
 * chars, reset sprites, animate the stair scroll, load sprite GFX, scroll
 * camera, prep destination, fade back in, set the new song, and cache the
 * room entry properties. */
...
/* Warp-pad teleportation transition — 15-step state machine.
 * Applies a mosaic effect on entry, loads the destination room, syncs
 * room properties and builds the overlay, runs three fade cycles, then
 * fades back in from the warp and finalises the room entry cache. */
...
/* Top-level dungeon submodule dispatch table — 31 entries indexed by
 * dung_submodule_index (set by puzzle triggers, transition initiators, and
 * the module-07 entry point).  Index 0 is normal player control; indices
 * 1-26 cover every transition and puzzle type in the dungeon. */
...
/* 24-frame animated tile sequence for the dungeon water/lava surface
 * (the rippling pattern on Swamp Palace water and lava pool floors).
 * Each byte is a CHR offset (0x5D = flat, 0x5E = mid-ripple, 0x5F = peak).
 * Cycled every 4 frames by the NMI animated-tile routine. */
...
/* Convert a dsto tile-map index into a pointer into the active BG tile
 * buffer in WRAM.  dung_line_ptrs_row0 is 0x4000 for the upper layer and
 * 0x5000 for the lower layer; multiplying d by 2 converts from word index
 * to byte offset within g_ram. */
uint16 *DstoPtr(uint16 d) {
  return (uint16 *)&(g_ram[dung_line_ptrs_row0 + d * 2]);
}

/* Fill n consecutive tile words in dst with the single tile word src[0].
 * Used for horizontal runs of identical ceiling, floor, or carpet tiles. */
void Object_Fill_Nx1(int n, const uint16 *src, uint16 *dst) {
  int t = src[0];
  do *dst++ = t; while (--n);
}

/* Copy a 4-wide, 5-tall block of tile words from src into the BG tile map
 * at dst.  Each row in the tile map is 64 words wide (XY(0,1) = +64),
 * so dst is advanced by XY(0,1) between rows.  src is packed linearly. */
...
/* Copy an 8-word (4 wide × 2 tall) tile block from src into both the BG1
 * and BG2 tile maps at dsto simultaneously.  Used for two-layer water
 * ladder tiles where both layers must display the same graphic. */
...
/* Draw one row of a scalable chest-platform object into dung_bg2.
 * src is a 3-entry array: src[0] = left cap tile, src[3] = fill tile,
 * src[6] = right cap tile. dung_draw_width_indicator controls how many
 * fill tiles are placed between the caps.  Writes a second independent
 * row 7 tiles below using src[9], src[15], src[18] for the lower rail.
 * Called once per row from the outer DC loop in LoadType1ObjectSubtype1. */
...
/* Draw a square pit hole of size (dung_draw_width_indicator × dung_draw_width_indicator)
 * into the BG tile map.  The interior is filled with a uniform tile, then the
 * top/bottom edges and left/right edges are overdrawn with distinct border tiles
 * sourced from two fixed data offsets (0x63c for top/bottom, 0x648 for sides).
 * Object_SizeAtoAplus15(4) expands the raw encoded size to the actual tile count.
 * Used for the A4 hole object (square pit, scalable in both dimensions). */
...
/* Dispatch a subtype-1 (variable-length repeating) room object to its draw routine.
 * idx: the object ID byte from the room data (0x00-0xF7).
 * dst: pointer into the active BG1 tile map at the object's upper-left tile.
 * dsto: word index into the combined BG tile buffer (used for BothBgs writes).
 *
 * Subtype-1 objects repeat a 2–4 tile template horizontally or vertically for
 * a count determined by dung_draw_width_indicator and dung_draw_height_indicator.
 * These are set by one of four size helpers before the inner loop runs:
 *   RoomDraw_GetObjectSize_1to16()   — width/height 1-16 tiles (most objects)
 *   RoomDraw_GetObjectSize_1to15or26() — 1-15 normal, else 26 (walls)
 *   RoomDraw_GetObjectSize_1to15or32() — 1-15 normal, else 32 (ceiling)
 *   Object_SizeAtoAplus15(n)         — fixed width, height from header
 *
 * Special cases handled here include:
 *   0x22/0x5F/0x69/0x8A: rail objects that skip the end cap if a rail
 *     already occupies the first tile (prevents double-cap overlap).
 *   0xCD/0xCE: moving wall objects that check dung_savegame_state_bits
 *     to decide whether to draw the wall in its open or closed position.
 *   0xD8/0xDA: water/HDMA region objects that configure water_hdma_var*
 *     for the lava/water HDMA effect and conditionally flood the region.
 *   0xC4/0xDB and the fill_floor group: fill a rectangular area with the
 *     room's primary or secondary floor tile pattern.
 *   IDs not listed here assert(0) — they are reserved or unused. */
...
/* Write n columns of a 3-tall tile strip into both dung_bg1 and dung_bg2
 * simultaneously.  src contains n*3 tile words in column-major order
 * (three tiles per column).  Used for S-bend wall corners (subtype-2,
 * cases 0x14-0x17) that must appear on both BG layers. */
...
/* Dispatch a subtype-2 (fixed-size decorative/structural corner) room object.
 * idx: object ID (0x00-0x3F).  dst: BG1 tile map pointer.  dsto: BG word index.
 *
 * Unlike subtype-1, these objects have no repeating dimension — each ID maps to
 * a single fixed drawing routine.  Key groups:
 *   0x00-0x07 / 0x1C,0x24,0x25,0x29: outer wall corners (HIGH layer) — 4×4 tiles on BG1.
 *   0x08-0x0F: outer wall corners (LOW layer) — written to both BG1 and BG2.
 *   0x10-0x13: N-axis S-bend (3 columns, BothBgs).
 *   0x14-0x17: W-axis S-bend (4 rows, BothBgs).
 *   0x1E: star tile (pressure switch) in OFF state.
 *   0x1F: star tile in ON state — registers the tile position in star_shaped_switches_tile[]
 *         so the switch handler can toggle it at runtime.
 *   0x20: pre-lit torch — increments dung_num_lit_torches.
 *   0x2D: floor staircase UP — records position in dung_inter_starcases[] and advances
 *         all staircase counter fields so subsequent stairs use the next slot.
 *   0x2E/0x2F: floor staircase DOWN — similar, using the southdown counters.
 *   0x31-0x36: in-room and water ladder stair markers that write positions to
 *              dung_stairs_table_1 and update the various water/activated ladder counts.
 *   0x37: water gate large — draws either the open or closed variant depending on
 *         dung_savegame_state_bits & 0x800; also calls RoomTag_OperateWaterFlooring.
 *   0x38-0x3B: spiral staircase door sprites that stamp the BG2 high-priority bit
 *              on adjacent tiles (dung_bg2[dsto±1] |= 0x2000) so they render above Link.
 *   0x3C: sanctuary wall — writes a 6-row repeating pattern with horizontal flips.
 *   0x3F: dwarf smithy hole — writes a fixed 8×7 BG2 tile block. */
...
/* Register one bombable floor tile in the replacement-tile system and draw its
 * initial (intact) appearance.
 * a: initial dung_replacement_tile_state value for this slot (encodes type/state).
 * src: pointer to the 4 tile words displayed when the floor is intact (2×2 block).
 * src_below: pointer to the 4 replacement tiles shown after the floor is blown away
 *            (stored in replacement_tilemap_UL/LL/UR/LR[i]).
 * dst: BG tile map pointer for the intact tile draw.
 * dsto: word index used as the key for future replacement lookups
 *       (dung_object_tilemap_pos[i] = dsto*2 | layer_flag).
 * The slot index i is derived from dung_misc_objs_index, which is advanced by 2
 * after each call so each bombable floor occupies its own entry. */
...
/* Dispatch a subtype-3 (interactive/special) room object.
 * idx: object ID (0x00-0x7E).  dst: BG1 pointer.  dsto: BG word index.
 *
 * Subtype-3 objects are single fixed items that have runtime interactions or
 * persistent state tied to dung_savegame_state_bits.  Key entries:
 *   0x00: water face (dam gate) — open or closed depending on water tag state.
 *   0x01/0x02: waterfall face variants — always drawn open.
 *   0x12: rupee floor — skipped entirely if the floor has already been swept
 *         (dung_savegame_state_bits & 0x1000), otherwise draws 3 columns of rupees.
 *   0x15: Kholdstare shell (BG2 ice-block layer) — suppressed after boss death.
 *   0x16: single hammer peg.
 *   0x18: cell lock (big-key door) — draws a locked grate or clears if opened.
 *   0x19: chest — checks big-key/boss conditions and open-state bits to determine
 *         which of the open/closed chest tiles to draw; registers in dung_chest_locations[].
 *   0x1B/0x1C/0x1D: intra-room stair markers writing to dung_stairs_table_1/2.
 *   0x1E-0x21/0x26-0x29: inter-room straight stair markers (up/down, upper/lower layer)
 *         registering in dung_inter_starcases[] and advancing multiple counter fields.
 *   0x33: submerged staircase — conditionally draws as water-blocked or normal.
 *   0x35/0x36: active/inactive water ladders.
 *   Most other cases draw a simple 2×2 tile block via RoomDraw_Rightwards2x2. */
...
/* Adjust the camera scroll bounds by one screen quadrant (0x100 pixels)
 * in the A (primary) axis direction.  Used to widen the visible scroll
 * range when transitioning into the next column of quadrants. */
void RoomBounds_AddA(RoomBounds *r) {
  r->a0 += 0x100;
  r->a1 += 0x100;
}

/* Shift both B (secondary) bounds forward by 0x200 pixels (two quadrants).
 * Used for fat-staircase inter-room transitions that span two columns. */
void RoomBounds_AddB(RoomBounds *r) {
  r->b0 += 0x200;
  r->b1 += 0x200;
}

/* Contract the B bounds backward by 0x200 — inverse of RoomBounds_AddB. */
void RoomBounds_SubB(RoomBounds *r) {
  r->b0 -= 0x200;
  r->b1 -= 0x200;
}

/* Contract the A bounds backward by 0x100 — inverse of RoomBounds_AddA. */
void RoomBounds_SubA(RoomBounds *r) {
  r->a0 -= 0x100;
  r->a1 -= 0x100;
}

/* Initiate a westward inter-room screen transition.
 * Flips link_quadrant_x, adjusts the camera boundary, saves the current room
 * data, and sets submodule_index = 1 (scroll phase).  If Link has fully crossed
 * into the next quadrant (link_quadrant_x nonzero after the flip), also:
 *   - Shifts room_bounds_x back by 0x200 to track the new room's scroll range.
 *   - Saves dungeon_room_index_prev, then decrements dungeon_room_index by 1
 *     (rooms are stored in row-major order; adjacent rooms differ by ±1 or ±0x10).
 *   - If the tile under Link is a teleport tile (0x89 family), loads the travel
 *     destination from dung_hdr_travel_destinations[3] instead.
 *   - Toggles link_is_on_lower_level if room_transitioning_flags & 1 (layer swap).
 *   - Toggles cur_palace_index_x2 if room_transitioning_flags & 2 (palace swap).
 *   - Sets submodule_index = 2 (load new room phase).
 * Resets room_transitioning_flags and recomputes quadrant_fullsize_y for the new layout. */
...
/* Variant of Dungeon_StartInterRoomTrans_Left that nudges Link 8 px west
 * before starting the transition.  Used when the tile detector fires a
 * frame early due to a sub-pixel boundary, preventing Link from appearing
 * to teleport forward when the camera snaps to the new room. */
void Dung_StartInterRoomTrans_Left_Plus() {
  link_x_coord -= 8;
  Dungeon_StartInterRoomTrans_Left();
}

/* Initiate a northward inter-room screen transition.
 * Flips link_quadrant_y (XOR 2 because Y quadrants are bit 1), adjusts camera,
 * saves current room, and starts the scroll.  Special exit paths:
 *   tile 0x8E: Link is stepping on the dungeon-exit tile → Dung_HandleExitToOverworld.
 *   room 0: the top row of the dungeon grid has no room above → save keys and exit
 *           to the overworld (module 25).
 * If crossing into the next quadrant: decrements dungeon_room_index by 0x10
 * (rooms are laid out in a 16-wide grid, so the room above is 16 indices earlier).
 * Layer/palace swap handling is the same as the Left variant. */
...
/* Initiate a southward inter-room screen transition.
 * Mirror of StartInterRoomTrans_Up: flips link_quadrant_y (XOR 2),
 * adjusts quadrant, saves current room data, and starts the downward scroll.
 * Special exit path:
 *   tile 0x8E: dungeon-exit tile under Link → Dung_HandleExitToOverworld.
 * On quadrant boundary crossing: increments dungeon_room_index by 16
 * (one row lower in the 16-wide grid). Same level/palace-swap logic as Up. */
...
/* Write a 2×2 tile block to VRAM upload queue and both tile/attr shadow tables.
 * pos: word index into overworld_tileattr / dung_bg2_attr_table (row-major, stride 64).
 * t0/t1 = left column top/bottom tiles; t2/t3 = right column top/bottom tiles.
 * attr is written uniformly to all four cells in dung_bg2_attr_table.
 * Tiles are packed into vram_upload_data in the engine's DMA format:
 *   [vram_addr, size=0x100, tile_word] × 4 entries, terminated with 0xFFFF.
 * Sets nmi_load_bg_from_vram to schedule the VRAM upload on the next NMI. */
...
/* Convert a word-index in the 64-wide tile-map array into a SNES VRAM byte address,
 * with bytes swapped (for the DMA packet format that the NMI handler expects).
 * Bit manipulation maps the linear pos into the SNES BG tile-map layout:
 *   bits [5:0]  → tile X within a 32-tile row
 *   bits [12:7] → tile Y (row number)
 *   bit  6      → selects the right half of a 64-wide virtual map (VRAM page bit)
 * The swap16 call converts the result to little-endian for the upload buffer. */
uint16 Dungeon_MapVramAddr(uint16 pos) {
  pos *= 2;
  return swap16(((pos & 0x40) << 4) | ((pos & 0x303f) >> 1) | ((pos & 0xf80) >> 2));
}

/* Same as Dungeon_MapVramAddr but without the byte-swap.
 * Used when the caller needs the raw VRAM offset (e.g. for door tilemap addresses
 * that are stored in dung_door_tilemap_address[] as plain indices). */
uint16 Dungeon_MapVramAddrNoSwap(uint16 pos) {
  pos *= 2;
  return ((pos & 0x40) << 4) | ((pos & 0x303f) >> 1) | ((pos & 0xf80) >> 2);
}

/* Entrance-door stubs — these four functions exist as placeholders in the original
 * SNES ROM dispatch table for the four directions but are never validly called
 * in the reimplementation. Each asserts false because RoomDraw_FlagDoorsAndGetFinalType
 * would receive an incorrect dsto value if they were invoked (see NOTE comments). */
...
/* Draw a 4-column door strip and resolve its final type.
 * Calls RoomDraw_FlagDoorsAndGetFinalType with is_vertical=1 to record the door slot
 * and obtain the resolved door type (bit 8 set → already drawn, early out).
 * Special type promotions before drawing:
 *   kDoorType_1E / kDoorType_36 → kDoorType_Regular  (iron-bar doors open normally)
 *   kDoorType_38               → kDoorType_ShuttersTwoWay  (two-way shutter)
 * In both promotion cases, door_type_and_slot[i] is updated so the runtime open/close
 * logic sees the corrected type. Copies 3 tile words per column for 4 columns from
 * kDoorTypeSrcData2 into the BG tile map at dsto. */
...
/* Return a pointer to the door-info word stream for the given room index.
 * kDungeonRoomDoorOffs[room] is a byte offset into the packed kDungeonRoom blob.
 * The resulting uint16* is walked by the door-draw routines to read door type/position pairs. */
const uint16 *GetRoomDoorInfo(int room) {
  return (uint16 *)(kDungeonRoom + kDungeonRoomDoorOffs[room]);
}

/* Return a pointer to the 14-byte room header for the given room index.
 * Header encodes: collision layer, floor tile, tag IDs, palette, bg2 attribute, etc. */
const uint8 *GetRoomHeaderPtr(int room) {
  return kDungeonRoomHeaders + kDungeonRoomHeadersOffs[room];
}

/* Return the default (empty/unmodified) room layout byte stream for layout index i.
 * Used when the room has no saved state to overlay (fresh or pre-save rooms). */
const uint8 *GetDefaultRoomLayout(int i) {
  return kDungeonRoomDefault + kDungeonRoomDefaultOffs[i];
}

/* Return the full room layout byte stream for room index i from kDungeonRoom.
 * Contains packed object data (subtype + position) that LoadType1/2/3 consume. */
const uint8 *GetDungeonRoomLayout(int i) {
  return kDungeonRoom + kDungeonRoomOffs[i];
}

/* Write a packed 16-bit attribute word as two consecutive bytes into dung_bg1_attr_table.
 * j is the byte index; low byte → j, high byte → j+1. Used when setting palette/priority
 * attributes for a pair of horizontally adjacent BG1 tiles. */
static inline void WriteAttr1(int j, uint16 attr) {
  dung_bg1_attr_table[j + 0] = attr;
  dung_bg1_attr_table[j + 1] = attr >> 8;
}

/* Same as WriteAttr1 but targets dung_bg2_attr_table (collision / BG2 attribute layer). */
static inline void WriteAttr2(int j, uint16 attr) {
  dung_bg2_attr_table[j + 0] = attr;
  dung_bg2_attr_table[j + 1] = attr >> 8;
}

/* Tag routine shared by room tags 0x22 and 0x3B (movable-block overlay triggers).
 * Fires only if the room's savegame state has bit 0x100 set (block already pushed).
 * On activation: clears the tag, queues overlay j to load (dung_overlay_to_load),
 * plays the dungeon-shutter SFX (0x1B), and switches to submodule 3 (overlay load). */
...
/* Draw OAM sprite for one pushed block (slot i).
 * Allocates 4 bytes from OAM region B, converts the block's world coordinates
 * (pushedblocks_y_lo/hi, pushedblocks_x_lo/hi) to screen space by subtracting
 * the current BG2 scroll offsets, and writes a 16×16 OAM entry (tile 12, attr 0x20).
 * The ext byte (g_ram[oam_ext_cur_ptr]) = 2 marks it as a large sprite.
 * Only renders when pushedblocks_some_index < 3 (at most two blocks visible). */
...
/* Draw the left half of a door into dung_bg2 using a 3-column × 4-row tile block.
 * src: word offset into the tile source table; door: index into dung_door_tilemap_address[].
 * Each iteration writes 4 vertically stacked tiles in one column then advances
 * dst by 1 (next column) and src by 4 (next column's tile group). */
...
/* Draw the right half of a door into dung_bg2 using a 3-column × 4-row tile block.
 * Same structure as Object_Draw_DoorLeft_3x4 but offsets columns by +1 via XY(1,*). */
...
/* Run one frame of the locked-door-opening animation.
 * skip_anim: if true, jump immediately to the final open state (frame 16).
 * Animation steps:
 *   frame 4  — set dung_door_opened bits via kUpperBitmasks, ctr = 4
 *   frame 12 — same bit-set, ctr = 4 (second threshold)
 *   frame 16 — final open: call Dungeon_LoadToggleDoorAttr_OtherEntry, optionally
 *              DrawCompletelyOpenDoor for stair-mask locked types (0xF0–0xFF range),
 *              then return to submodule 0.
 * On every frame: queues a DMA packet for the animated door strip via
 * DrawDoorOpening_Step1 + Dungeon_PrepOverlayDma_nextPrep, plays SFX 21 (key click),
 * and sets nmi_copy_packets_flag to fire the upload on the next NMI. */
...
/* Return the palette info record for dungeon palette index idx.
 * kDungPalinfos contains color data and metadata for each dungeon color set. */
const DungPalInfo *GetDungPalInfo(int idx) {
  return &kDungPalinfos[idx];
}

/* Return the teleport message ID for the given room index.
 * Used by falling-into-pit teleport logic to show the room-name message. */
uint16 Dungeon_GetTeleMsg(int room) {
  return kDungeonRoomTeleMsg[room];
}

/* Return true if the current dungeon room is in the hardcoded list of pit rooms
 * where falling deals direct damage to Link (e.g. spike pits, death drops).
 * Scans kDungeonPitsHurtPlayer[] (a short list) from the end for dungeon_room_index. */
...
/* Package the next BG2 quadrant of the newly loaded room into the VRAM upload buffer.
 * Called incrementally during inter-room scroll: each call handles one 32×32-tile
 * quadrant (one quarter of a room). ofs selects the source and destination via
 * kUploadBgSrcs/kUploadBgDsts using the current overworld_screen_transition nibble
 * plus dung_cur_quadrant_upload. Tiles are laid out as UploadVram_32x32 structs
 * (2×2 tile groups) at g_ram[0x1000]. Sets nmi_subroutine_index=1 and
 * nmi_disable_core_updates=1 so the NMI handler will DMA this buffer to VRAM. */
...
/* Prepare one BG1 quadrant for VRAM upload during a room transition.
 * In theory this should handle water-flood rooms (tag 25) differently, but
 * that case is asserted unreachable here — see comment inside. */
...
/* Package one BG1 quadrant into the VRAM upload buffer (non-water variant).
 * Identical structure to Dungeon_PrepareNextRoomQuadrantUpload but reads from
 * dung_bg1[] instead of dung_bg2[], and offsets the destination address by +0x10
 * (BG1 VRAM base is 0x10 pages above BG2 in the SNES video layout).
 * Called every frame during scroll transitions until all 4 quadrants are uploaded. */
...
/* Reposition BG1 relative to BG2 to simulate a lantern light cone in dark rooms.
 * Only active when hdr_dungeon_dark_with_lantern is set and not in submodule 20.
 * The cone direction is determined by link_direction_facing and is_standing_in_doorway:
 *   - In a doorway: index is biased by Link's sub-tile position (X or Y offset within
 *     the doorway cell) to smoothly pan the cone as Link walks through the opening.
 *   - Open room: index = raw facing direction (0=up,1=down,2=left,3=right).
 * For indices 0-1 (vertical): adjusts BG1HOFS_copy2 based on Link's X, then clamps
 *   BG1VOFS_copy2 within [0, kOrientLampBgTab4[idx]].
 * For indices 2-3 (horizontal): adjusts BG1VOFS_copy2 based on Link's Y, then clamps
 *   BG1HOFS_copy2 within [0, kOrientLampBgTab4[idx]].
 * The net effect is that BG1 (the black overlay with a hole) tracks Link's lantern. */
...
/* Handle the game-state bookkeeping when Link defeats a dungeon boss.
 * Saves death counters, dungeon keys, marks boss room cleared (bit 0x8000 in
 * dung_savegame_state_bits), and flags all four room quadrants in save RAM.
 * Maps the current room to a post-boss exit room via kDungeonExit_From/To, then
 * transitions based on the destination room:
 *   0x20 → Zelda rescue cutscene (module 21): flips world, forces sword-up pose,
 *           updates sram_progress_indicator and save_ow_event_info.
 *   0x0D → Agahnim 2 / GT finale (module 24): sets up OW map and fades CGADSUB.
 *   j ≥ 3 → Crystal boss exit (module 22): plays ending music (0xF1) and transitions.
 *   else  → Standard boss exit (module 19): normal overworld return. */
...
/* Flush the current-dungeon death counter to per-palace SRAM.
 * cur_palace_index_x2 >> 1 selects which palace slot in deaths_per_palace[] to write.
 * death_save_counter is reset to 0 unless the palace index is 8 (GT, which tracks
 * deaths across multiple re-entries). */
...
/* Full room initialization — called whenever Link enters a new dungeon room.
 * Sequence:
 *   1. Dungeon_LoadHeader() — parse room header bytes into dung_hdr_* fields.
 *   2. Zero out all per-room dynamic state: stair slot counters, door arrays,
 *      torch timers, replacement tile states, object metadata arrays, chest counts,
 *      floor-movement fields, HDMA variables, etc.
 *   3. Load the room's object layout via GetDungeonRoomLayout and drive the
 *      object draw loop (RoomDraw_DrawAllObjects).
 * The blanket zeroing ensures no data bleeds from the previous room across a
 * transition (important because the slot arrays are sized for worst-case rooms). */
...
/* Iterate through all packed object records in level_data and dispatch to draw routines.
 * Two-pass structure:
 *   Pass 1 (objects): reads 2-byte position+subtype word + 1-byte ID triples until
 *     0xFFF0 (end-of-objects sentinel) or 0xFFFF (end-of-stream).
 *     Each triple is dispatched by RoomData_DrawObject.
 *   Pass 2 (doors): reads 2-byte door records until 0xFFFF.
 *     Each record is dispatched by RoomData_DrawObject_Door.
 * dung_draw_width_indicator and dung_draw_height_indicator are reset to 0 before
 * each object so subtype-1 routines start with a clean size state. */
...
/* Decode a packed door record and dispatch to the appropriate direction draw function.
 * Bit layout of the 16-bit record a:
 *   [15:8] → door_type (kDoorType_* constant)
 *   [7:4]  → position (which of the 4 possible slots in that wall)
 *   [1:0]  → direction: 0=North, 1=South, 2=West, 3=East */
...
/* Decode a 3-byte packed object record and dispatch to the appropriate draw routine.
 * Bits in r0 (bytes 0-1) and idx (byte 2):
 *   Normal objects (idx < 0xF8):
 *     r0[1:0]  → dung_draw_width_indicator (repeating width)
 *     r0[9:8]  → dung_draw_height_indicator (repeating height)
 *     r0[7:2]  → X tile position (>>2 after masking low 8 bits)
 *     r0[15:10]→ Y tile position
 *     idx      → object ID passed to LoadType1ObjectSubtype1
 *   Subtype-3 objects (idx >= 0xF8 — interactive/special):
 *     idx is re-encoded: (idx & 7) << 4 | height << 2 | width → LoadType1ObjectSubtype3
 *   Subtype-2 objects (r0 & 0xFC == 0xFC — corner/fixed-size decoration):
 *     Different bit packing: x = (r0&3)<<4 | (r0>>12)&F; y = ((r0>>8)&F)<<2 | (idx>>6)
 *     Lower 6 bits of idx → passed to LoadType1ObjectSubtype2 */
...
/* Draw the floor tile layers for the current room.
 * Reads the first byte of level_data: high nibble = floor-1 tile index (BG2),
 * low nibble = floor-2 tile index (BG1, shifted left 4 to align with table).
 * Each nibble selects a 4×2-tile pattern from the floor tile table; that pattern
 * is tiled across all four 32×32-tile quadrants of the room by RoomDraw_FloorChunks.
 * Switches dung_line_ptrs_row0 between the BG2 and BG1 offset tables around each call. */
...
/* Tile a 4×2 floor pattern across the entire room (4 quadrants × 8 rows of blocks).
 * kDungeon_QuadrantOffsets gives the tile-map word offset for each quadrant corner.
 * Inner loop: 8 calls to RoomDraw_A_Many32x32Blocks (one per row of 4×4 super-tiles),
 * advancing dst by XY(0,4) each time (4 tile rows down). */
...
/* Write n consecutive 4×4-tile blocks from src into the BG tile map at dst.
 * Each block is two passes of a 4×2 blit (8 tiles per pass = rows 0-1, then 2-3),
 * then dst advances 4 columns right and 4 rows back to begin the next block. */
...
/* Draw n consecutive 1-wide × 3-tall tile columns, advancing rightward each step.
 * Each column writes 3 tiles vertically (rows 0-2) from src, then dst += 1 column,
 * src += 3. Used for vertical repeating wall strips (e.g. side rails). */
...
/* Check whether a moving-wall room tag has already been triggered.
 * Tags 0x1C–0x1F in dung_hdr_tag[0] or [1] indicate a blast-wall or push-wall room.
 * If the corresponding savegame bit (0x1000 >> i) is set, the wall was already moved:
 *   clear the collision, tag, and bg2_properties and return false (skip re-drawing).
 * Otherwise return true so the caller proceeds with the wall-present layout.
 * Also zeros the sub-pixel and floor-move-flags on entry. */
...
/* Initialize the moving-wall replacement tile buffer before the wall slides.
 * Fills all 64 entries of moving_wall_arr1 with tile 0x1EC (transparent/empty).
 * moving_wall_var1 encodes the VRAM address of the wall's column: X within 32-tile
 * row = dsto & 0x1F; page bit = (dsto & 0x20) ? 0x400 : 0; plus base 0x1000. */
...
/* Draw a horizontally repeating 1-high object strip of width (dung_draw_width_indicator+2).
 * Layout: first tile from src[0], then src[1]+src[2] repeated for each interior step,
 * then src[3] as the final cap. dst advances by 2 tiles per inner iteration. */
...
/* Copy a 4-wide × 4-tall tile block from src into dst (used for water surface objects).
 * Writes 4 tiles per row, advancing dst by one row (XY(0,1)) after each. */
...
/* Draw a fixed 4-wide × 4-tall tile block. Delegates to RoomDraw_Object_Nx4(4,...). */
void RoomDraw_4x4(const uint16 *src, uint16 *dst) {  // 8197ed
  RoomDraw_Object_Nx4(4, src, dst);
}

/* Draw an N-wide × 4-tall tile block column by column.
 * Each iteration writes 4 tiles vertically into one column of dst then advances
 * src by 4 and dst by 1 column. Used for fixed-size rectangular room objects. */
...
/* Draw an N-wide × 4-tall tile block into BOTH dung_bg1 and dung_bg2 simultaneously.
 * Used for objects that appear on both layers (e.g. water/lava surface tiles that
 * must be present on BG1 for collision and BG2 for rendering). dsto is a word index. */
...
/* Draw a single 2-wide × 2-tall tile block at dst from src.
 * Tile order: [left-top, left-bottom, right-top, right-bottom] in src. */
...
/* Draw a 3-wide × 2-tall tile block at dst.
 * Reads 6 tiles from src in row-major order (top row then bottom row). */
...
/* Draw n rows of a 4-wide water-surface object, advancing dst downward each row.
 * Each row reads 4 tiles from src and writes them linearly to dst (consecutive words).
 * Used for the HDMA water region geometry tiles (e.g. trench walls, underwater ledges). */
...
/* Draw an 8-row × n-column decoration block into dung_bg2 at dsto.
 * The layer page bit is selected by dung_line_ptrs_row0: if it equals 0x4000 (upper BG),
 * add 0x1000 to dsto; otherwise use dsto directly (lower BG). Advances dst by 64 tiles
 * (one full tile-map row) and src by n tiles after each of the 8 rows. */
...
/* Draw a 12×12-tile lamp-cone overlay into dung_bg1 (the darkness mask layer).
 * a: byte offset into dung_bg1 (divided by 2 to get word index).
 * y: source table offset in SrcPtr tile ROM.
 * Writes 12 rows of 12 tiles each, skipping 52 words between rows to align with
 * the 64-tile-wide tile map stride. Used by lantern dark rooms. */
...
/* Draw Agahnim's Tower throne-room window and pillar decorations into dung_bg2.
 * Five separate draw passes covering the room's architectural details:
 *   Pass 1 (src 0x1BF2): three sets of 6-wide×4-tall window frames at columns 7, 13, 19.
 *   Pass 2 (src 0x1C22): 5 diagonal curtain tiles on both sides (bit 0x4000 = H-flip).
 *   Pass 3 (src 0x1C2C): vertical column pairs at rows 11, 17, 23 — both sides mirrored.
 *   Pass 4 (src 0x1C5C): 6-wide arch tiles at rows 9-10 for two interior columns.
 *   Pass 5 (src 0x1C74): 2-wide column detail at rows 14 and 20.
 *   Pass 6 (src 0x1C8C): 5-tall column cap at column 7, rows 9-13.
 * Finally OR's 0x2000 (priority bit) onto four tiles at XY(14,28..29) for the
 * foreground floor strip in front of Agahnim's platform. */
...
/*
 * RoomDraw_FortuneTellerRoom - Draws the fortune teller's room interior decoration.
 * @dsto: Destination offset into the BG2 tilemap buffer.
 *
 * Renders the fortune teller's table, curtains, and surrounding decor using
 * horizontally mirrored tile pairs (XOR 0x4000 flips the tile horizontally on SNES).
 * The room has a symmetric layout, so tiles are placed in mirrored pairs.
 */
...
/*
 * Object_Draw8x8 - Draws an 8x8 tile block composed of four 4x4 sub-blocks.
 * @src: Source tile data pointer; each 4x4 sub-block is 16 words apart.
 * @dst: Destination tilemap pointer.
 *
 * Assembles a large object from four quadrants, each drawn as a 4x4 tile block.
 */
...
/*
 * RoomDraw_Door_North - Dispatches drawing of a north-facing door based on its type.
 * @type: The door type constant (e.g., kDoorType_Regular, kDoorType_LgExplosion).
 * @pos_enum: Position enumeration index (0-11); values >= 6 indicate a door that
 *            spans into an adjacent room quadrant (requiring a mirrored south-side draw).
 *
 * Each door type has distinct visual representation and gameplay behavior. This function
 * routes to the appropriate specialized drawing routine. Special cases include exploding
 * walls, curtain doors (slashable), entrance doors, and stair-mask locked doors.
 */
...
/* Draw a stair-mask locked door on the north wall (types kDoorType_StairMaskLocked0–3).
 * Records the door's direction (0=north), tilemap address, and type in the slot arrays.
 * If the door is already open (bit set in dung_door_opened_incl_adjacent), skip drawing.
 * Types 0-1 (StairMaskLocked0/1) → RoomDraw_OneSidedShutters_North (shutter appearance).
 * Types 2-3 (StairMaskLocked2/3) → resolve via RoomDraw_FlagDoorsAndGetFinalType then
 *   blit 4 columns × 3 rows of tiles into dung_bg1 at dsto, then call Door_PrioritizeCurDoor. */
...
/* Set the high-priority flag (bit 0x2000) on the tilemap address of the most recently
 * registered door slot. Causes the door tiles to render above sprites on the SNES.
 * Called for waterfall-tunnel doors and stair-mask locked doors that overlap sprites. */
void Door_PrioritizeCurDoor() {  // 81a8fa
  dung_door_tilemap_address[(dung_cur_door_idx >> 1) - 1] |= 0x2000;
}

/* Draw a north-facing door that may span two room quadrants.
 * pos_enum ≥ 6 means the door straddles the N-S quadrant boundary: in that case,
 * temporarily OR dung_cur_door_idx with 0x10 and also draw the mirrored south-side
 * portion via RoomDraw_CheckIfLowerLayerDoors_Y using kDoorPositionToTilemapOffs_Down.
 * Then always draw the primary north-side shutter via RoomDraw_OneSidedShutters_North. */
...
/* Draw a north-wall door's 4-column × 3-row tile strip (one-sided shutter variant).
 * Calls RoomDraw_FlagDoorsAndGetFinalType to register the slot; returns early if bit 8
 * set (already drawn or open). Type promotions before drawing:
 *   54 (kDoorType_1E) → kDoorType_ShuttersTwoWay
 *   56 (kDoorType_38) → kDoorType_Regular
 * Then blits 4 columns from kDoorTypeSrcData[t>>1] into DstoPtr(dsto), 3 tiles per column. */
...
/* Draw a door on the south wall.
 * Mirrors RoomDraw_Door_North with south-specific tile offsets (kDoorPositionToTilemapOffs_Down).
 * Handles all the same special types but with south-appropriate draw helpers:
 *   kDoorType_EntranceLarge  → 10-wide BG decor block at dsto + XY(-3,-4).
 *   kDoorType_EntranceLarge2 → same decor but forces page 0x1000, then copies row with
 *     high-priority bit (0x2000) from BG1→BG2 at the entrance threshold row.
 *   kDoorType_EntranceCave/Cave2 → 4×4 cave-mouth tiles at dsto.
 *   Others follow the same type dispatch as North. */
...
/* Draw a west-wall door, handling quadrant-spanning and type promotion.
 * pos_enum ≥ 6: also draws the mirrored east-side half (dung_cur_door_idx | 0x10).
 * Type promotions: kDoorType_36 → ShuttersTwoWay; kDoorType_38 → Regular.
 * Blits 3 rows × 4 tiles per row from kDoorTypeSrcData3 into DstoPtr(dsto). */
...
/* Draw a door on the east wall.
 * Uses kDoorPositionToTilemapOffs_Right for the base tilemap offset.
 * Dispatches on type: PlayerBgChange, EntranceDoor, ThroneRoom, or normal/high-range.
 * High-range types (≥ kDoorType_RegularDoor33) → RoomDraw_OneSidedLowerShutters_East. */
...
/* Draw a normal east-wall door, applying high-priority and waterfall-tunnel special cases.
 * kDoorType_Regular2: mark column to the right of dsto as high-priority (over sprites).
 * kDoorType_WaterfallTunnel: draw + prioritize door slot.
 * All cases ultimately delegate to RoomDraw_OneSidedShutters_East. */
...
/* Draw an east-wall door's 3-row × 4-tile-per-row strip (one-sided shutter variant).
 * Registers the slot via RoomDraw_FlagDoorsAndGetFinalType (direction=3=East).
 * Type promotions: kDoorType_36 → Regular; kDoorType_38 → ShuttersTwoWay.
 * Writes tiles starting at DstoPtr(dsto)+1 (one column right of door base) from
 * kDoorTypeSrcData4, advancing dst one column and src 4 tiles per row. */
...
/* Draw a north-wall curtain door (kDoorType_Slashable).
 * If already-open flag returned by RoomDraw_FlagDoorsAndGetFinalType has bit 8 set,
 * draws the "open curtain" tiles (src 0x78a). Otherwise draws the closed curtain tiles
 * from kDoorTypeSrcData. Both are 4×4 blocks. */
...
/* Set up and optionally draw a bombable exploding-wall door on the north wall.
 * Always registers the door slot at kDoor_BlastWallUp_Dsts[pos_enum] + 10.
 * If the door bit is NOT set in dung_door_opened_incl_adjacent → just record direction
 * and slot (wall still present), return.
 * If already blown open:
 *   - Determines which tag slot to clear (slot 0 unless tag 0x20/0x25/0x28 is active).
 *   - Sets quadrant_fullsize_y=2 and dung_blastwall_flag_y=1 (expand camera vertically).
 *   - Draws two wall segments: upper frame (kDoorTypeSrcData2[42]) and lower opening
 *     (kDoorTypeSrcData[42]) via RoomDraw_ExplodingWallSegment.
 *   - OR's dung_unk2 with 0x200 to signal the blast-wall was already used. */
...
/* Draw one segment of an exploded wall (frame or fill) into dung_bg2.
 * First 12 tiles from src are drawn as a column via RoomDraw_ExplodingWallColumn.
 * Then src[12] is a fill tile repeated 3 rows tall for 18 columns (the blown-open
 * interior). dung_draw_width_indicator is temporarily set to 18 for the fill loop. */
...
/* Draw a high-range south-wall door (shutter-trap variants) with split BG1/BG2 writes.
 * Type promotions: ShutterTrapUR → Shutter; ShutterTrapDL → RegularDoor33.
 * Rows 1-2 → dung_bg1; row 3 → dung_bg2 (so the threshold tile is on the visible layer).
 * Calls RoomDraw_MakeDoorHighPriority_South and Door_PrioritizeCurDoor for render order. */
...
/* Draw a high-range west-wall door (shutter-trap variants) with split BG1/BG2 writes.
 * pos_enum ≥ 6: mirror-draw the east side first (with dung_cur_door_idx | 0x10).
 * Type promotions: ShutterTrapUR → Shutter; ShutterTrapDL → RegularDoor33.
 * Column 0 → dung_bg2; columns 1-2 → dung_bg1 (so the wall face is on the collision layer).
 * Calls RoomDraw_MakeDoorHighPriority_West and Door_PrioritizeCurDoor. */
...
/* Draw a high-range east-wall door (shutter-trap variants) with split BG1/BG2 writes.
 * Type promotions: ShutterTrapUR → RegularDoor33; ShutterTrapDL → Shutter.
 * Columns 0-1 → dung_bg1[+1]; column 2 → dung_bg2[+1] (rightmost tile is on BG2).
 * Calls RoomDraw_MakeDoorHighPriority_East and Door_PrioritizeCurDoor. */
...
/* Set bit 0x2000 (high tile priority) on the 4-wide north door strip in dung_bg2.
 * Masks dsto to the start of its row (& 0xF07F>>1) then OR's 0x2000 into 4 tiles per row
 * and advances by XY(0,1) until dsto wraps back to the original value (one full quadrant row). */
...
/* Set bit 0x2000 on all rows of the south door strip until the column bits (& 0x7C0) hit 0.
 * OR's 0x2000 into 4 consecutive tiles per row, advancing by XY(0,1) each time. */
...
/* Set bit 0x2000 on all columns of the west door strip until wrapping back to dsto_org.
 * Masks dsto to column 0 of the door row (& 0xFFE0), then OR's 0x2000 into 4 rows
 * per column advancing by XY(1,0) until back to the masked start position. */
...
/* Set bit 0x2000 on all east-door columns until the column index (dsto & 0x1F) reaches 0.
 * OR's 0x2000 into 4 rows per column, advancing rightward by XY(1,0) each step. */
...
/* Set bit 0x2000 (high priority) on a 4-wide × 7-row strip of dung_bg2 tiles.
 * Used for kDoorType_Regular2 north/south doors where the door must render above sprites.
 * Iterates 7 rows downward from dsto, OR'ing 4 consecutive tiles per row. */
...
/* Set bit 0x2000 on a 5-column × 4-row strip of dung_bg2 tiles.
 * Used for kDoorType_Regular2 west/east doors. Iterates 5 columns rightward,
 * OR'ing 4 rows per column. */
...
/* Draw a vertical column of 4×2 tile blocks downward with a variable row-spacing.
 * Each iteration writes 8 tiles (4 wide × 2 rows), then advances dst by increment
 * (instead of a fixed XY(0,2)), allowing arbitrary vertical gaps between blocks.
 * Repeats for dung_draw_width_indicator iterations. Used by staircase-rail objects. */
...
/* Draw a 1-wide × 5-tall tile column (2×2 plus 1 extra row) at dst from src.
 * Used for pillar-top decorations or doorpost elements where an extra cap tile is needed. */
...
/* Draw a 1-wide × 4-tall tile column at dst (shelf/ledge end cap). Returns dst unchanged. */
...
/* Draw a 1-wide × 3-tall tile column at dst (interior bar/rail segment). Returns dst. */
...
/* Register and draw a 2×2 replaceable tile object (e.g. gray block, pushable stone).
 * Allocates slot i = dung_misc_objs_index >> 1, records:
 *   dung_replacement_tile_state[i] = a (encodes the block type/state)
 *   dung_object_pos_in_objdata[i]  = current stream offset (to re-draw on reload)
 *   dung_object_tilemap_pos[i]     = byte address | page bit (0x2000 if upper BG)
 *   replacement_tilemap_UL/LL/UR/LR[i] = the original tiles under the block
 * Then draws the 2×2 block via RoomDraw_Rightwards2x2. */
...
/* Register and draw a single pot (2×2 destructible decoration).
 * Same slot-registration pattern as DrawBigGraySegment with state 0x1111.
 * Hardcodes the "under the pot" tiles to the floor tile words (0x0D0E/0D1E/4D0E/4D1E).
 * In the dark world, uses alternate pot tile source (0xE92). */
...
/* Draw a 4×4 bombable floor tile cluster (4 independent 2×2 bomb-floor sub-tiles).
 * Special case: room 101 with savegame bit 0x1000 set — floor already bombed,
 *   draw an open hole (Object_Hole at src 0x5AA) and return.
 * Otherwise draws four 2×2 quadrants (NW/NE/SW/SE) via Object_BombableFloorHelper,
 * each with its own replacement-tile state code (0x3030/3131/3232/3333). */
...
/* Register and draw a single hammer peg (2×2 interactive object, driven-down by hammer).
 * Slot registration with state 0x4040 (peg type marker).
 * Hardcodes replacement tiles to 0x19D8/... (the recessed floor tile set). */
...
/* Register and draw a lightable torch (2×2 animated sprite object).
 * Slot index = dung_index_of_torches >> 1; records dsto_x2 and the data stream slot.
 * If dsto_x2 has bit 0x8000 set the torch starts lit (src 0xECA); otherwise unlit (0xEC2).
 * Tracks dung_num_lit_torches (capped at 3) for the all-torches-lit door trigger. */
...
/* Parse the 14-byte room header and populate all dung_hdr_* variables.
 * Also determines dung_loade_bgoffs_h/v_copy (BG2 scroll snap for the new room):
 *   submodule 0 (normal entry): align to 0x1FF-masked current scroll.
 *   submodule 21 or 6–17 (stairs/fall): add 0x20 (half-tile nudge) then align.
 *   others (walking transition): align on horizontal or vertical axis based on
 *     link_direction and a ±256 kAdjustment offset.
 * Header byte layout (hdr_ptr[]):
 *   [0]  bits[7:5]=bg2_properties, bits[4:2]=collision, bit[0]=dark_with_lantern
 *   [1]  palette index → dpi (DungPalInfo)
 *   [2]  aux_tile_theme_index (selects VRAM graphics set)
 *   [3]  sprite_graphics_index (+0x40 bias)
 *   [4]  collision_2 (second collision layer type)
 *   [5-6] tag[0..1] (room event triggers)
 *   [7]  hole_teleporter_plane (bits 0-1) + staircase_plane[0-2] (bits 2-7)
 *   [8]  staircase_plane[3] (bits 0-1)
 *   [9-13] travel_destinations[0..4] (stair/teleport exit room indices)
 * After reading the header, loads save state from save_dung_info[room]:
 *   bits [15:12] = door-opened flags → dung_door_opened (and _incl_adjacent | 0xF00)
 *   bits [11:4]  = state bits → dung_savegame_state_bits (shifted left 4)
 *   bits [3:0]   = visited quadrants → dung_quadrants_visited
 * Reads room door tilemap addresses from GetRoomDoorInfo, then checks all four
 * adjacent rooms (left/right/up/down) for already-open doors via
 * Dungeon_CheckAdjacentRoomsForOpenDoors. */
...
/* Propagate open-door state from an adjacent room into dung_door_opened_incl_adjacent.
 * idx: starting index into kLookup[] (0=south-neighbor, 6=north-neighbor, 12=east, 18=west).
 * Loads the adjacent room's door info, then for each door in that room checks whether
 * its position code (low byte) matches one of the 6 kLookup entries starting at idx.
 * On a match: looks up the "reverse" position via kLookup2 and searches the current
 * room's dung_door_tilemap_address array for a door at that position.
 * If found and open in the adjacent room (adjacent_doors_flags bit set):
 *   - Trapdoor types (k=0x44 or 0x18): only propagate if coming from prev room AND
 *     clear dung_flag_trapdoors_down (trapdoor is already raised).
 *   - Other doors: set kUpperBitmasks[j] in dung_door_opened_incl_adjacent. */
...
/* Load the door list for an adjacent room into adjacent_doors[].
 * Also computes adjacent_doors_flags: starts with that room's save-state open-door bits
 * OR'd with 0xF00, then additionally sets a flag bit for each door whose type is
 * kDoorType_EntranceLarge (0x4000 high byte) or a basic open type (< 0x200 high byte). */
...
/* Write collision attribute bytes into dung_bg2_attr_table (and dung_bg1_attr_table)
 * for every staircase and interactive object registered during room load.
 * Processes multiple ordered ranges of dung_inter_starcases[], incrementing the
 * "type counter" t (starting 0x3030, +0x101 per stair) as it walks through:
 *   inter-room up-north stairs (attr at XY+1,1; XY+1,2 cleared)
 *   wall up-north spiral stairs (attrs 0x5E5E for steps, t for landing)
 *   wall up-north spiral stairs variant 2 (0x5F5F)
 *   inter-room up-north straight stairs (0x3838 top, 0 bottom)
 *   inter-room up-south straight stairs (0 top, t mid, 0x3939 bottom)
 *   inter-room south-down stairs (t at XY+1,2; 0x2626 at XY+1,3) [t reset to 0x3434]
 *   wall down-north spiral stairs (0x5E5E) and variant 2 (0x5F5F)
 *   inter-room down-north straight stairs (0x3838, t, 0)
 *   inter-room down-south straight stairs (0, t, 0x3939)
 * Then writes attributes for in-room staircases (from dung_stairs_table_1):
 *   type 0 (up-north): 0x1F1F; type 1 (south-down): 0x1E1E; type 2 (pseudo-up): 0x1D1D.
 * Also writes 0x3B3B collision for all star-shaped floor switches. */
...
/* Apply collision attributes to all registered door slots (dung_door_tilemap_address != 0).
 * Then calls Dungeon_LoadSingleDoorTileAttribute (handles tile-level attr for water/pit
 * doors) and ChangeDoorToSwitch (converts switch-triggered doors). Advances overworld_map_state. */
...
/* Write collision attribute bytes for one door slot k into dung_bg2_attr_table.
 * Two paths based on type:
 *   alpha (open-pass types: Regular, Entrance, ExitToOw, EntranceLarge, EntranceCave):
 *     → write "passable" attributes allowing Link to walk through.
 *   beta (semi-solid types: EntranceLarge2, Cave2, Type4, Regular2, WaterfallTunnel,
 *         RegularDoor33, WarpRoomDoor):
 *     → write attributes that block Link on both sides but mark the door area.
 *   LgExplosion → returns immediately (blast-wall handles its own collision).
 *   High-range closed shutters → write "blocked" attributes (collision walls).
 * Direction (dung_door_direction[k]) selects among N/S/W/E attribute patterns.
 * When the door is open (bit in dung_door_opened_incl_adjacent): uses open-door attrs. */
...
/* Write collision attributes for an exploded blast-wall opening.
 * Direction bit: 0 (north/south) → 12 rows × 22 columns: edges get 0x102/0x201,
 *   interior gets 0x0 (open air), advancing one row per iteration.
 * Direction bit: 2 (east/west) → 5 column-pairs × 22 rows: edges get 0x101/0x202,
 *   interior gets 0x0, advancing two columns per iteration.
 * The non-zero edge attrs mark the wall frame for the collision layer. */
...
/* Stub: would convert floor-switch-controlled doors to their triggered state.
 * In this reimplementation dung_unk5 is always 0 so this is a no-op. */
void ChangeDoorToSwitch() {  // 81c1ba
  assert(dung_unk5 == 0);
}

/* Toggle the low bit of all crystal-peg collision attributes in both BG tables.
 * Crystal pegs use attribute 0x66 or 0x67; XOR with 1 flips between raised (0x66)
 * and lowered (0x67) state for all 0x1000 attribute entries.
 * Called when Link swings the hammer to flip all pegs simultaneously. */
...
/* Execute both room tag routines (dung_hdr_tag[0] and [1]) each frame.
 * First calls Dungeon_DetectStaircase (which may change submodule_index).
 * Bug-fix mode: if a staircase was detected (submodule != 0), skip the tag routines
 * this frame to prevent Link's coordinates from going out of sync with tag spawning.
 * g_ram[14] is set to the tag slot index (0 or 1) before each tag call.
 * flag_skip_call_tag_routines gates the entire function (set by cutscene handlers). */
...
/* Tag 0x00: null tag — no room event active (default). */
void Dung_TagRoutine_0x00(int k) {  // 81c328
}

/* Check whether Link is standing on a staircase tile and trigger the transition.
 * Only runs when link_direction has a moving-up/down bit (bits 2–3 of direction).
 * Computes the attribute-table index from Link's coordinates:
 *   pos = (Y + kBuggyLookup[k>>1]) quantized to 0x1F8 → shifted into row bits
 *         OR X quantized to column bits, OR 0x1000 if on lower level.
 * The lookup table kBuggyLookup is named "buggy" because some entries were
 * inherited from the original SNES code with off-by-one Y offsets. */
...
/* Tag trigger: fires only if Link is in the NW quadrant (X < 0x100 AND Y < 0x100). */
void RoomTag_NorthWestTrigger(int k) {  // 81c432
  if (!(link_x_coord & 0x100) && !(link_y_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2A: fires only if Link is in the NE quadrant (X >= 0x100 AND Y < 0x100). */
void Dung_TagRoutine_0x2A(int k) {  // 81c438
  if ((link_x_coord & 0x100) && !(link_y_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2B: fires only if Link is in the SW quadrant (X < 0x100 AND Y >= 0x100). */
void Dung_TagRoutine_0x2B(int k) {  // 81c43e
  if (!(link_x_coord & 0x100) && (link_y_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2C: fires only if Link is in the SE quadrant (X >= 0x100 AND Y >= 0x100). */
void Dung_TagRoutine_0x2C(int k) {  // 81c444
  if ((link_x_coord & 0x100) && (link_y_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2D: fires if Link is in the west half of the room (X < 0x100). */
void Dung_TagRoutine_0x2D(int k) {  // 81c44a
  if (!(link_x_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2E: fires if Link is in the east half (X >= 0x100). */
void Dung_TagRoutine_0x2E(int k) {  // 81c450
  if (link_x_coord & 0x100)
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x2F: fires if Link is in the north half (Y < 0x100). */
void Dung_TagRoutine_0x2F(int k) {  // 81c456
  if (!(link_y_coord & 0x100))
    RoomTag_QuadrantTrigger(k);
}

/* Tag 0x30: fires if Link is in the south half (Y >= 0x100). */
void Dung_TagRoutine_0x30(int k) {  // 81c45c
  if (link_y_coord & 0x100)
    RoomTag_QuadrantTrigger(k);
}

/* Core quadrant/position-gated room tag trigger.
 * tag ≥ 0x29: "chest reveal" type — wait for all sprites cleared, then
 *   call RoomTag_OperateChestReveal (spawns items from a chest or raises floor).
 * tag 0x0B–0x28: "trapdoor toggle" type — if current trapdoor state differs from
 *   the block-was-pushed flag, flip dung_flag_trapdoors_down, play SFX 37, and start
 *   submodule 5 (door animation) from the beginning.
 * tag < 0x0B: wait for sprites clear, then call Dung_TagRoutine_TrapdoorsUp. */
...
/* Raise the room's trapdoors (open the shutter doors) if they are currently down.
 * Clears dung_flag_trapdoors_down, resets the door animation counters, plays
 * the shutter SFX (0x1B = door creak), and switches to submodule 5. */
...
/* Room-clear trigger: waits for ALL sprites in the room to die before firing.
 * tag == 10: raise trapdoors (all enemies killed → doors open).
 * other tags: call RoomTag_OperateChestReveal (spawn reward). */
...
/* Re-killable boss room clear: when the room is clear, unblock the menu and
 * clear tag[1]. Used for rooms where the boss can respawn (Agahnim variants). */
...
/* Open doors after a water/block puzzle is solved: fires when
 * dung_flag_statechange_waterpuzzle is set AND trapdoors are down.
 * Raises the doors via submodule 5. Used in Swamp Palace water rooms. */
...
/* Boss room: open the exit door once the palace prize is already collected.
 * In the dark world checks link_has_crystals; in the light world link_which_pendants.
 * kDungeonCrystalPendantBit selects the bit for the current palace. If set, raises
 * the trapdoors and clears the tag so it doesn't re-trigger. */
// Used for bosses
...
/* Open/close the room doors based on how many torches are currently lit.
 * Counts dung_object_tilemap_pos[] entries with bit 0x8000 set (= torch is lit).
 * If fewer than 4 torches are lit → doors stay/go down (down=true).
 * If state changed, trigger door animation (submodule 5) with shutter SFX. */
...
/* Floor-switch triggered exploding wall: checks shutter state via RoomTag_MaybeCheckShutters.
 * If shutters are in the right state, fires Dung_TagRoutine_BlastWallStuff. */
...
/* Pull-switch triggered exploding wall: only fires if dung_flag_statechange_waterpuzzle
 * is set (meaning the lever/switch was just pulled this frame). */
...
/* Initiate a blast-wall explosion sequence.
 * Searches door_type_and_slot[] for the entry with type 0x30 (blast wall),
 * records its slot in dung_unk_blast_walls_3.
 * Determines which sub-section of the blast wall to detonate based on Link's
 * quadrant (Y-quadrant for N/S walls; X-quadrant for E/W walls):
 *   i = (Y>>8 & 1 + 1) * 2  for vertical walls
 *   i = X>>8 & 1             for horizontal walls
 * kBlastWall_Tab0[i] = bomb animation type; kBlastWall_Tab1[i] = offset into door.
 * Packs the world position of the explosion into messaging_buf (H and V components),
 * plays SFX 27 (bomb), sets the blast-wall-in-progress flag, and spawns the
 * AncillaAdd_BlastWall ancilla to animate the explosion. */
...
/* Spawn the boss's Heart Container or Crystal prize after the kill.
 * Only runs if the boss has been defeated (dung_savegame_state_bits bit 0x8000) AND
 * the palace prize has not already been collected (crystal/pendant bit not yet set).
 * Calls Ancilla_SpawnFallingPrize with the prize type from kBossFinishedFallingItem
 * (indexed by palace number). If spawning fails (out of ancilla slots), keeps retrying
 * next frame. On success (or if prize already collected), clears the tag. */
// Used for bosses
...
/* Post-Agahnim-1 boss room tag: triggers the after-battle sequence.
 * Fires when save_ow_event_info[0x5B] bit 0x20 is NOT set (Agahnim not yet defeated)
 * AND dung_savegame_state_bits bit 0x8000 is set (boss just died this run).
 * Reverts any translucency palette swap from the fight, clears the tag, and calls
 * PrepareDungeonExitFromBossFight to transition to the cutscene/world-change. */
...
/* Ganon's room: unlock the exit once all sprites are gone (or flagged non-blocking).
 * Checks that no sprite is in "active" state (state=4) or alive without the "ignore
 * for door" flag (sprite_flags4 bit 6). If all clear and Link isn't falling:
 *   immobilize Link (flag_is_link_immobilized=26), switch to submodule 26 (Ganon death
 *   ceremony), force sword-up pose, clear inputs, set R16=0x364 (ceremony timer). */
...
/* Play the shutter-open SFX and clear the tag when Link enters the SE quadrant
 * AND the room is clear of sprites. Used as a "kill switch" for leftover room tags
 * in rooms where a block event should auto-resolve once the area is clear. */
...
/* Reveal a chest after a pushable block was moved: waits for BG VRAM upload to finish
 * (nmi_load_bg_from_vram == 0) and for the block-pushed flag, then calls
 * RoomTag_OperateChestReveal to animate the chest appearing. */
void RoomTag_PushBlockForChest(int k) {  // 81c7c2
  if (!nmi_load_bg_from_vram && dung_flag_movable_block_was_pushed)
    RoomTag_OperateChestReveal(k);
}

/* Shutter-triggered chest reveal: waits for countdown_for_blink == 0 and for the
 * shutters to be in the correct state (via RoomTag_MaybeCheckShutters), then reveals. */
...
/* Animate chest lids flipping open and queue VRAM uploads for all registered chests.
 * Clears the tag, resets the VRAM upload offset, then iterates dung_chest_locations[]:
 *   - Writes collision attributes (attr starting 0x5858, +0x101 per chest).
 *   - Copies the open-chest tiles (src 0x149C) into dung_bg2 at the chest position.
 *   - Queues a VRAM DMA packet in the upload buffer for each tile group.
 * After all chests: sets nmi_load_bg_from_vram=1 to schedule the VRAM update. */
...
/* Shake BG1 relative to BG2 while the moving wall is in motion.
 * Alternates bg1_x_offset between -1 and +1 each frame (via frame_counter & 1),
 * with bg1_y_offset = -bg1_x_offset (diagonal shake).
 * Clears both offsets if the tag was already cleared (wall finished moving). */
...
/* Animate a moving wall sliding westward (toward Link or away).
 * Each frame:
 *   - If not yet moving (dung_floor_move_flags == 0): check torch/switch conditions.
 *   - If moving: shake BG1, accumulate velocity from MovingWall_MoveALittle().
 * Adjusts dung_floor_x_offs and BG1HOFS_copy2 each frame.
 * Completion check: when x_offs >= kMovingWall_Tab0[slot], play SFX, unfreeze Link,
 *   clear the tag, and schedule an NMI VRAM column update. */
...
/* Check whether the moving wall's trigger condition is met (torches or switch).
 * If dung_flag_statechange_waterpuzzle is set → switch triggered, proceed.
 * Otherwise count lit torches (bit 0x8000 in dung_object_tilemap_pos[]): if < 4, return.
 * On trigger: increment dung_floor_move_flags (starts the wall moving), clear the
 * puzzle flag, set savegame state bit (0x1000 >> k), play "rumble" ambient SFX (7),
 * and freeze Link (flag_is_link_immobilized=1). */
...
/* Compute the integer pixel-per-frame speed for the moving wall.
 * Accumulates 0x22 into a sub-pixel counter each frame; the high byte is the
 * integer velocity (≈ 0.13 pixels/frame → produces ~1px every 8 frames). */
...
/* Advance the gigantic-wall slot counter and handle the "second half" of the wall.
 * Returns moving_wall_var2 (current slot index). If tag < 0x20, the wall has crossed
 * into the second section: disables collision (dung_hdr_collision=0), sets TM_copy=0x16
 * (show BG1 for the gap), and bumps the slot index by 8. */
...
/* Drain water from a Swamp Palace water room when the drain puzzle is solved.
 * Fires when dung_flag_statechange_waterpuzzle is set (valve was just pulled).
 * Setup: reconfigures HDMA window registers for the drained state, sets mosaic
 * to max (31) for a "drain flash", queues palette update, sets submodule 11.
 * Draws the drain-bed tile block into dung_bg2 at the computed water-surface position
 * and queues a VRAM DMA packet. Plays shutter SFX (0x1B) + water-drain SFX (0x2E).
 * Sets savegame state bit 0x800 (water drained permanently for this room). */
...
/* Fill water into a Swamp Palace water room when the fill puzzle is solved.
 * Fires when dung_flag_statechange_waterpuzzle is set.
 * Sets submodule 12 (water-fill animation), plays water-fill SFX (0x2F and 0x1B),
 * sets BYTE(dung_floor_y_offs)=1 to trigger the rising-water HDMA animation,
 * and marks savegame bit 0x800 (water-level change saved). */
...
/* Water-gate tag: manages the vertical water gate that reveals a passage when lowered.
 * Returns immediately if bit 0x800 is set (gate already used) or puzzle flag is clear.
 * Otherwise animates the gate lowering into the water. */
...
/* Tags 0x37/0x39/0x3A: star-switch overlay selectors — call Dung_TagRoutine_Func2
 * with the overlay index for the respective switch target state. */
...
/* Core star-switch overlay selector.
 * av: initial overlay index for this switch type.
 * On first call: if dung_overlay_to_load is 0, initializes it to av.
 * Checks whether Link is standing on a floor switch (RoomTag_CheckForPressedSwitch).
 * If yes, adds the switch's attribute offset (yv) to av; if the result differs from
 * the current overlay, loads the new overlay (submodule 3), plays bomb SFX (27),
 * toggles byte_7E04BC, and restores the star tile CHR data. */
...
/* Tag 0x22 variant: after a block push, load overlay 0 (the base room layout). */
void RoomTag_ChestHoles0(int k) {  // 81cc5b
  Dung_TagRoutine_0x22_0x3B(k, 0x0);
}

/* Tag 0x3B variant: after a block push, load overlay 0x12 (alternate layout). */
void Dung_TagRoutine_0x3B(int k) {  // 81cc62
  Dung_TagRoutine_0x22_0x3B(k, 0x12);
}

/* Floor-switch triggered hole overlay: if Link is pressing a switch (attr 0x2323/3A3A/3B3B),
 * clears the tag, queues overlay 5, and switches to submodule 3 (overlay load)
 * with shutter SFX 0x1B. Used in rooms where a switch opens a pit/hole. */
...
/* Draw the water-gate floor tiles into dung_bg1 based on kWatergateLayout.
 * Parses the layout table (same format as room object data) and fills 4×4-tile blocks
 * with the water-surface tile pattern (src 0x110) using the same nested width/height
 * loop as the main object draw routine. Used when the Swamp Palace water gate is raised. */
...
/* Check whether Link is standing on a shutter-type floor tile (attr 0x2323 or 0x2424).
 * Returns false if Link is immobilized or in an auxiliary state.
 * Probes four adjacent tile-map positions around Link's feet (using RoomTag_GetTilemapCoords
 * + offsets of +64 for rows). When a matching attr is found, also checks the row below
 * it to confirm a continuous 2-row shutter strip; on success, writes attr to *attr_out,
 * saves the position in word_7E04B6, and returns true. */
...
/* Convert Link's world coordinates to a dung_bg2_attr_table word index.
 * X: (link_x_coord - 1) & 0x1F8 >> 3  → tile column (0–63)
 * Y: (link_y_coord + 14) & 0x1F8 << 3  → tile row offset (stride 64)
 * Adds 0x1000 for the lower BG level. The +14 Y bias centers the probe on Link's feet. */
int RoomTag_GetTilemapCoords() {  // 81cda5
  return ((link_x_coord - 1) & 0x1f8) >> 3 | ((link_y_coord + 14) & 0x1f8) << 3 | (link_is_on_lower_level ? 0x1000 : 0);

}

/* Check whether Link is pressing a floor switch (attr 0x2323, 0x3A3A, or 0x3B3B).
 * Same probe pattern as RoomTag_MaybeCheckShutters but adds the star-shaped-switch
 * and crystal-switch attr codes (0x3A3A/3B3B). Writes found attr to *y_out. */
...
/* Check if a bomb explosion at (x,y) hits any destructible dungeon tile.
 * In overworld mode (module != 7): delegates to Overworld_BombTiles32x32.
 * In dungeon mode: reads dung_bg2_attr_table at the explosion center and two
 * neighboring positions.  Dispatch on attr value:
 *   0x62: bombable floor (e.g. Thieves' Attic) → set room state bit, draw hole,
 *     play SFX. Room 0x65 specifically records the bomb in savegame state.
 *   0xF0–0xFF: door-type dispatch — only breaks walls of type kDoorType_BreakableWall,
 *     0x2A, or 0x2E; otherwise returns. If matched: clears the door tile to open passage,
 *     records the wall as open in savegame/dung_door_opened, spawns door-debris ancillae,
 *     plays SFX 27, starts submodule 9 (wall-break explosion). */
...
/* Draw the blast-wall explosion frame into dung_bg2 at dsto.
 * Uses src 0x31EA: draws the two edge "crumble" pairs via
 * ClearExplodingWallFromTileMap_ClearOnePair at dsto and dsto+25,
 * then fills the interior (dung_unk_blast_walls_2 - 1 columns × 12 rows)
 * with the solid "hole" tile (src[24]) to represent the blown-open gap. */
...
/* Animate all shutter doors opening or closing over 8 frames.
 * door_animation_step_indicator is incremented each call:
 *   frame 4: set door_open_closed_counter (0 if opening, 4 if closing).
 *   frame 8: flip dung_door_opened_incl_adjacent bit for each shutter,
 *             play open (SFX 21) or close (SFX 22), update collision attributes.
 * Each eligible shutter door (type Shutter or ShuttersTwoWay) that changed state
 * is redrawn via DrawShutterDoorSteps and queued for VRAM DMA.
 * After frame 16: returns to submodule 0 and clears nmi_copy_packets_flag. */
...
/* Skip the door-open animation and immediately show the cracked door as fully open.
 * Wraps Dungeon_OpeningLockedDoor_Combined(true) — skip_anim=true jumps to frame 16. */
void OpenCrackedDoor() {  // 81d469
  Dungeon_OpeningLockedDoor_Combined(true);
}

/* Reload collision attributes for a door that just changed state (open/close).
 * Calls Dungeon_LoadSingleDoorAttribute (rewrites the attr table for this door slot)
 * and then Dungeon_LoadSingleDoorTileAttribute (applies toggle-floor / toggle-palace
 * passable bits for the whole room). Used after any door state transition. */
void Dungeon_LoadToggleDoorAttr_OtherEntry(int door) {  // 81d51c
  Dungeon_LoadSingleDoorAttribute(door);
  Dungeon_LoadSingleDoorTileAttribute();
}

/* Apply the passable-tile-attribute overlay bits for toggle-floor and toggle-palace tiles.
 * For each entry in dung_toggle_floor_pos[]: looks up existing attr in bg2 or bg1 table
 * (based on which table has an 0x8x attr) and ORs in 0x1010 (marks tile as passable-up).
 * For each entry in dung_toggle_palace_pos[]: same but ORs in 0x2020 (palace-level flag).
 * Called after any door toggle to keep floor passability in sync with door state. */
...
/* Rewrite staircase collision attributes for the stair-mask-locked door open state.
 * Iterates dung_inter_starcases[] matching the same stair-type ranges as
 * Dungeon_LoadObjectAttribute, but clears the middle rows (XY+1,2 and XY+1,3) to 0
 * so that the passage behind the door is treated as open air.
 * Only processes spiral-stair variants (0x5E5E / 0x5F5F); straight-stair ranges are
 * iterated but have no writes (the loop bodies are empty). */
...
/* Check whether Link is facing a liftable tile (pots, blocks, etc.) and return its type.
 * Calculates the tile position in front of Link using direction-indexed offset tables.
 * Checks dung_bg2_attr_table at that position: attr must be in range 0x70–0x7F
 * (the liftable object attr range). Then looks up dung_replacement_tile_state[attr&0xF]:
 *   state 0 → tile already lifted/missing, return 0xFFFF (not liftable).
 *   state 0x2020 (hammer-peg type) → return 0x55.
 *   other → return kDungeon_QueryIfTileLiftable_rv[state & 0xF] (object-type code). */
...
/* Process all active pushable-block objects each frame.
 * Iterates dung_misc_objs_index from 0 down to dung_index_of_torches_start:
 *   state 1: Start of slide — restore the floor tile under the block (RoomDraw_16x16Single),
 *     update tilemap position by kPushBlockMoveDistances[direction], → state 2.
 *   state 2: Mid-slide — call PushBlock_Slide to move the block sprite each frame.
 *     When PushBlock_Slide sets state to 3: check for pit landing (PushBlock_CheckForPit)
 *     then advance to state 4.
 *   state 4: Falling — call PushBlock_HandleFalling for the fall animation. */
...
/* Restore the floor tiles under a pushable block to their pre-push state.
 * Reads the saved tile data from replacement_tilemap_UL/LL/UR/LR[index] and
 * writes it back via Dungeon_Store2x2 (schedules an NMI VRAM upload).
 * The attribute for the restored tile is derived from the lower-right tile word. */
...
/* Resolve where a pushed block landed and update state accordingly.
 * Toggles dung_flag_movable_block_was_pushed (unless the block's tilemap pos bit 0x4000
 * is clear — meaning it's a "free" block that doesn't count).
 * Checks dung_bg2_attr_table at the new position:
 *   0x20 (pit): plays SFX 0x20, updates movable_block_datas for the new room
 *     (dung_hdr_travel_destinations[0] = the pit's target room).
 *   0x23 (floor switch): marks related_to_trapdoors_somehow, sets state 4 (falling).
 *   other: draws the landed block tiles (2×2 stone block at 0x922/0x932/0x923/0x933,
 *     attr 0x27) via Dungeon_Store2x2, sets state 0xFFFF (done). */
...
/* Lift the tile in front of Link and replace it with floor tiles.
 * Computes the world coords and tilemap index of the tile in front of Link,
 * reads the attr to find the replacement-tile slot, then restores the floor
 * underneath it (via Dungeon_Store2x2 with the saved replacement tiles).
 * Returns the object-type code and sets pt->x/pt->y to the tile's world coords.
 * Also stores the world coords in R16/R18 for the caller (spawn position). */
...
/* Convert the current misc-object's tilemap position to world coordinates.
 * Reads dung_object_tilemap_pos[dung_misc_objs_index>>1] and extracts:
 *   X = (link_x_coord & 0xFE00) | ((pos & 0x007E) << 2)  (column * 4 pixels, page from Link)
 *   Y = (link_y_coord & 0xFE00) | ((pos & 0x1F80) >> 4)  (row * 4 pixels, page from Link)
 * Writes result to *pt. Used by ManipBlock routines to get world spawn position. */
...
/* Look up the item hidden under a lifted/smashed pot or tile and reveal it.
 * pos4: tilemap word-index of the tile; pos6: same used to find attr-table slot.
 * Searches kDungeonSecrets[room] table for a matching position.
 * If found, reads the "data" byte:
 *   0       → nothing hidden, return.
 *   1–0x7F  → small pickup (rupee, key, etc.): sets dung_secrets_unk1 bits.
 *             Bit 8 (data==8) marks the secret as already revealed (permanent reveal).
 *             Others track per-room via pots_revealed_in_room bitmask (one-shot).
 *   0x80–0x87 → special: updates replacement tile data to hole tiles (src 0x5BA)
 *             and plays SFX 0x1B, enabling the bombable-floor effect at this position.
 *   0x88    → special: sets replacement tiles to dark-world hole tile set. */
...
/* Write a "common" 2×2 tile group to the dungeon BG at world position (x,y).
 * v selects the tile type from kPrepSpriteInducedDma_Srcs (rupees, holes, blocks, etc.).
 * v == 8: also draws a second 2×2 group at x+16 (for wider objects like large rupees).
 * Sets nmi_load_bg_from_vram to queue the VRAM upload this frame. */
...
/* Build a VRAM DMA packet for one 2×2 tile group at world position (x,y).
 * v selects the tile source from kPrepSpriteInducedDma_Srcs (indexed as v>>1).
 * Computes the tilemap word-index (pos), writes VRAM addresses for all four cells,
 * updates dung_bg2[] shadow and dung_bg2_attr_table[] with the new tile data,
 * and appends the 6-entry DMA packet (addr, size, tile) × 4 + 0xFFFF sentinel
 * to vram_upload_data at vram_upload_offset. */
...
/* Remove the embedded rupee tile at (x,y) when Link picks it up.
 * Replaces both tile slots at (pos, pos+1) in dung_bg2 with tile 0x190F (empty floor).
 * Writes the corresponding attr from the tile's attributes_for_tile entry.
 * Queues a 2-entry VRAM DMA packet (two 2×1 tile blits with size 0x100) and
 * sets nmi_load_bg_from_vram. Sets dung_savegame_state_bits bit 0x1000 to persist
 * the collected state across room re-entries. */
...
/* Open a chest and determine the item inside.
 * tile: the collision attribute byte at Link's position (0x58–0x63).
 * Mini-game chests (tile == 0x63) delegate to OpenMiniGameChest.
 * chest_idx = tile - 0x58 selects the chest from dung_chest_locations.
 * Reads the chest's saved location and checks whether it's already open
 * (kChestOpenMasks bit set in the room's save data). If so, returns 0 (empty).
 * Otherwise marks it open, draws the open-chest tile overlay, and returns the
 * item code from the room's chest-item table.
 * NOTE: does not return exactly like the original ROM (see comment). */
...
/* Animate the big chest opening and update its tile map.
 * Draws the 4×3 open-big-chest tile block (src 0x14C4) into dung_bg2,
 * queues a VRAM DMA via Dungeon_PrepOverlayDma_nextPrep, sets attr 0x2727 (open/empty)
 * for all 6 cell pairs, plays chest-open SFX (14), sets nmi_copy_packets_flag,
 * and sets byte_7E0B9E=1 (big chest opened flag used by save logic). */
...
/* Open a rupee-chest mini-game chest and return the random prize.
 * Checks minigame_credits: 0 → "no credits" message (0x163); 255 → "max wins" (0x162).
 * Decrements credits, then finds the 2×2 chest tile pair near Link by scanning
 * up to 2 adjacent columns for attr 0x6363.
 * Draws the open-chest top tiles (src 0x14A4) — note a Zelda bug causes the draw
 * to land 2 rows too low (pos_wrong), while the VRAM upload correctly targets pos.
 * Prize selection:
 *   room 0x00: 4-bit RNG table (kDungeon_RupeeChestMinigamePrizes[0..15])
 *   room 0x18: 4-bit RNG + 0x10 offset into same table
 *   other:     3-bit RNG, avoids repeating the last prize (some_menu_ctr). */
...
/* Build the VRAM address for a chest stripe DMA packet.
 * pos: base address offset; y: chest slot index (selects from dung_chest_locations).
 * Adds the chest's tilemap position to pos, then applies the same bit-shuffle as
 * Dungeon_MapVramAddrNoSwap but with swap16 — used for chest-reveal stripe DMA packets. */
uint16 RoomTag_BuildChestStripes(uint16 pos, uint16 y) {  // 81ef0f
  pos += dung_chest_locations[y >> 1];
  return swap16(((pos & 0x40) << 4) | ((pos & 0x303f) >> 1) | ((pos & 0xf80) >> 2));
}

/* Apply collision attributes for the "water drained" state in Swamp Palace.
 * Reconfigures graphics: enables mode-7 blending (CGWSEL=2, CGADSUB=0x32),
 * clears the color-math window registers, disables collision layer 0, clears TMW.
 * Then sets all wet-staircase slots in both table_1 (north-up) and table_2 (south-up)
 * to attr 0x1D1D (the "dry stairs" collision code). Queues a CGRAM update and
 * advances subsubmodule_index. */
...
/* Prepare one BG1 quadrant for VRAM upload during the water-fill animation.
 * Calls WaterFlood_BuildOneQuadrantForVRAM per step and advances
 * dung_cur_quadrant_upload and subsubmodule_index.
 * After 6 steps (all 4 quadrants + 2 extra for BG1/BG2): resets to submodule 0. */
...
/* Draw the rising water column in dung_bg2 and queue VRAM DMA for 4 columns.
 * dsto = (word_7E047C >> 1) + XY(0,2) — the vertical start of the water surface.
 * Writes depth rows of 4 tiles each from src into dung_bg2 going downward.
 * Then builds 4 VRAM upload entries (one per column): each entry is a 5-row strip
 * using size=0x980 (9.5 rows * 2 bytes = enough for depth rows). Terminates with 0xFFFF
 * and sets nmi_load_bg_from_vram. Used by the rising-water HDMA effect. */
...
/* Apply collision attributes for the "water filled" state in Swamp Palace.
 * Clears TMW_copy, then sets water-staircase attrs:
 *   dung_stairs_table_1 (pseudo-up-north wet stairs): top row = 0x003/0x300/0xA03/0x30A
 *     (water-entry codes), rows 1-2 = 0x808 (underwater swim attr) in both tables.
 *   dung_stairs_table_2 (south-up wet stairs): bottom row = water-entry, rows 0-2 = 0x808.
 * Returns to submodule 0 and clears nmi_boolean and subsubmodule_index. */
...
/* Animate the water gate rising from the dam — expands the flood area by one step.
 * Increments watergate_var1 each frame; every 16 frames redraws the water surface
 * tiles from kWatergateSrcs1 (4-stage tile progression) and queues VRAM DMA
 * for the 3 gate columns. Adjusts HDMA water-level variables each frame.
 * At watergate_var1 == 64: signals completion via subsubmodule_index++. */
...
/* Ganon's fight: extinguish a torch and enable translucency mode simultaneously.
 * Calls Palette_AssertTranslucencySwap (activates color-math blending), sets the
 * torch counter to 0xC0, then delegates to Dungeon_ExtinguishTorch. */
...
/* Ganon's fight: extinguish a torch (sets byte_7E0333 = 193 = 0xC1).
 * Delegates to Dungeon_ExtinguishTorch for the actual tile update. */
void Ganon_ExtinguishTorch() {  // 81f4a1
  byte_7E0333 = 193;
  Dungeon_ExtinguishTorch();
}

/* Extinguish one torch: clear its "lit" flag, restore its unlit tile graphic,
 * and decrement the lit-torch count (dung_num_lit_torches).
 * byte_7E0333 & 0xF selects the torch slot index.
 * Clears bit 0x8000 from dung_object_tilemap_pos (marks unlit), then updates
 * dung_torch_data to restore the unlit position.
 * Redraws the unlit tile (src 0xEC2) via RoomDraw_AdjustTorchLightingChange.
 * If this is a "lights-out" room: updates the room brightness level:
 *   0 lit → TS_copy=1 (full darkness)
 *   else → set overworld_fixed_color_plusminus from kLitTorchesColorPlus table,
 *           start submodule 10 (screen fade). */
...
/* Set the 5-column strip of wall tiles around the staircase to high priority (bit 0x2000).
 * Called when Link begins descending a spiral staircase so the wall tiles render in
 * front of Link's sprite. Saves the staircase position in word_7E048C.
 * Queues two VRAM DMA packets (the two halves of the 5-column strip) and sets
...
/* Clear the high-priority (bit 0x2000) flag from the 5-column staircase wall strip.
 * Called when Link exits a spiral staircase so the wall tiles no longer overlap sprites.
 * Reads the saved position from word_7E048C and queues two VRAM DMA packets. */
...
/* Build the VRAM upload list for the blast-wall explosion animation frame.
 * Reads the current dung_bg2 column data at dsto and packages it into uvram.data[]
 * as pairs of tall DMA transfers (5 rows each, size = r6|0xA00 and |0xE00).
 * r10 = number of columns in the current "stripe" segment (3 or more).
 * If dung_unk_blast_walls_2 >= 6: skips a gap between the two wall halves using
 * kBlastWall_Tab2 to compute the column jump distance.
 * The direction bit (dung_door_direction & 2) selects horizontal vs vertical stripe.
 * Terminates the upload list with 0xFFFF. */
...
/* Apply a packed room overlay (alternate layout for switched/puzzle states).
 * Reads 3-byte records: [0]=X position bits, [1]=Y position bits, [2]=tile type.
 * Sentinel 0xFFFF ends the stream. Special types:
 *   0xA4: writes 4-tile horizontal row at row+1 from the inline tile ID.
 *   others: calls the appropriate object draw routine based on type byte.
 * Used after switch triggers (Dung_TagRoutine_Func2, RoomTag_Holes2, etc.)
 * to redraw parts of the room with alternate geometry. */
...
/* Draw a north-wall door in its fully open or fully closed state (no animation frame).
 * Uses kDoorTypeSrcData[GetDoorGraphicsIndex(door,r4_door)>>1] as the tile source. */
...
/* Draw a north-wall door tile strip: 4 columns × 3 rows (rows 0-2) into dung_bg2. */
...
/* Draw the south door using the same slot as door index (door == r4_door). */
void GetDoorDrawDataIndex_South_clean_door_index(int door) {  // 81fb0b
  GetDoorDrawDataIndex_South(door, door);
}

/* Prepare south door DMA step: if the door is in the upper half of the room
 * (pos < kDoorPositionToTilemapOffs_Down[9]), also draws and queues the matching
 * north-side twin (door ^ 8) at pos + 0x500 (or +0x800 for high-range types).
 * Then draws the south side and returns dma_ptr. */
...
/* Select and draw the south door's current animation frame.
 * door_open_closed_counter: 0 or 4 → fully open/closed (DrawDoorToTileMap_South).
 * Mid-animation: computes animation frame index x:
 *   +4 for high-range door types (≥ 0x42); +2 for two-way/single shutters.
 *   Then blits kDoorAnimDownSrc[x>>1] via Object_Draw_DoorDown_4x3. */
...
/* Draw a south-wall door in its final open or closed state. Uses kDoorTypeSrcData2. */
...
/* Draw a south-wall door tile strip: 4 columns × 3 rows (rows 1-3) into dung_bg2. */
...
/* Draw the west door using the same slot as door index (door == r4_door). */
void GetDoorDrawDataIndex_West_clean_door_index(int door) {  // 81fbc2
  GetDoorDrawDataIndex_West(door, door);
}

/* Prepare west door DMA step: if the door is in the right column of the room
 * (pos >= kDoorPositionToTilemapOffs_Left[6]), also draws the matching east-side twin
 * (door ^ 8) at pos - 16 (or -28 for high-range types). Then draws the west side. */
...
/* Select and draw the west door's current animation frame (same logic as South variant).
 * Uses kDoorAnimLeftSrc and Object_Draw_DoorLeft_3x4 for mid-animation frames. */
...
/* Draw a west-wall door in its final open or closed state. Uses kDoorTypeSrcData3. */
...
/* Draw the east door using the same slot as door index (door == r4_door). */
void GetDoorDrawDataIndex_East_clean_door_index(int door) {  // 81fc80
  GetDoorDrawDataIndex_East(door, door);
}

/* Prepare east door DMA step: if the door is in the left column of the room
 * (pos < kDoorPositionToTilemapOffs_Right[6]), also draws the west-side twin
 * (door ^ 8) at pos + 16 (or +28 for high-range types). Then draws the east side. */
...
/* Select and draw the east door's current animation frame (same logic as West variant).
 * Uses kDoorAnimRightSrc and Object_Draw_DoorRight_3x4 for mid-animation frames. */
...
/* Draw an east-wall door in its final open or closed state. Uses kDoorTypeSrcData4. */
...
/* Step the fixed-color brightness toward overworld_fixed_color_plusminus by ±1 per frame.
 * COLDATA_copy0 & 0x1F is the current brightness; if already at target, do nothing.
 * Otherwise nudge it by 1 in the correct direction and apply via
 * Dungeon_ApproachFixedColor_variable. Used for smooth dark-room torch fade. */
...
/* Apply a fixed-color brightness level a to all three SNES color-math channels.
 * COLDATA_copy0 = a | 0x20 (red), COLDATA_copy1 = a | 0x40 (green), copy2 = a | 0x80 (blue).
 * The channel-select bits (0x20/0x40/0x80) ensure CGRAM writes target each channel. */
...
/* Module initialization sequence: runs once when entering any dungeon.
 * Execution order:
 *   1. Play ambient SFX 5 (dungeon wind), reset room indices and save state.
 *   2. Dungeon_LoadEntrance() — parse entrance data (room index, Link position, direction).
 *   3. Set link_num_keys from per-dungeon key counts (0xFF = infinite for special entrances).
 *   4. Rebuild HUD, reset torch/lantern state.
 *   5. Dungeon_LoadAndDrawRoom() — full room draw (floors + objects + doors).
 *   6. Dungeon_LoadCustomTileAttr() — apply any custom per-room tile attributes.
 *   7. Decompress animated tile CHR data, load attribute table, load palettes.
 *   8. Compute dung_loade_bgoffs_h/v_copy from dungeon_room_index for scroll snap.
 *   9. Special case: room 0x104 with sram_progress_flags bit 4 → disable lantern dark.
 *  10. SetAndSaveVisitedQuadrantFlags(), set CGWSEL/CGADSUB (mode-7 blending for BG).
 *  11. Set overworld_fixed_color_plusminus from dung_num_lit_torches or full-bright.
 *  12. Start palette fade (palette_filter_countdown=0x1F, darkening_or_lightening=2).
 *  13. Reset Link state (speed, buttons), reset sprite subsystems.
 *  14. Flag flag_skip_call_tag_routines so tags don't fire on first frame.
 *  15. If game is in the intro sequence (sram_progress_indicator==0, !bit4):
 *        force bright COLDATA, disable dark, and call Link_TuckIntoBed (Link asleep).
 *  16. Switch to main_module_index=7 (dungeon), submodule_index=15 (load submodule).
 *  17. Load dungeon music bank and set ambient SFX. */
...
/* Set the ambient dungeon SFX based on location and progress.
 * Only runs for the first two save-state stages (sram_progress_indicator < 2).
 * Default: SFX 5 (dungeon wind). If on the upper floor (dung_cur_floor >= 0)
 * and not in rooms 2 or 18 (Link's house / intro room): switch to SFX 3 (cave drip). */
...
/* Load overworld songs if the music type has been marked for switching.
 * flag_which_music_type != 0 signals a pending overworld music reload;
 * clears the flag and calls LoadOverworldSongs(). */
...
/* Main dungeon module update loop — called every frame while in a dungeon.
 * Sequence:
 *   1. Dungeon_HandleLayerEffect() — HDMA layer effects (water, dark rooms, etc.).
 *   2. kDungeonSubmodules[submodule_index]() — dispatch the active submodule handler.
 *   3. Bug-fix: if a Somaria block on a door caused a module switch, skip the rest.
 *   4. Run Dungeon_PushBlock_Handler() for all active push-block objects.
 *   5. If still in submodule 0 (exploration state):
 *      - Load CHR half-slot graphics rotation.
 *      - Dungeon_HandleCamera() — scroll and quadrant tracking.
 *      - Dungeon_HandleRoomTags() — execute room event routines.
 *      - Sprite_HandlePushedBlocks() — OAM for all pushed blocks.
 *      - Dungeon_HandleShootingGallery() — target-shooting mini-game.
 *      - Dungeon_HandleInvisibleFloor() — invisible floor detection.
 *   6. OrientLampLightCone() — adjust BG1 for lantern dark rooms.
 *   7. Dungeon_HandleToggleFloor() — update toggle-floor states. */
...
/* Check whether Link has reached a room edge and trigger a scroll transition.
 * Checks Y velocity → top/bottom edges (dir 3=north, 2=south);
 * Checks X velocity → left/right edges (dir 1=west, 0=east).
 * Edge thresholds: top Y < 4, bottom Y >= 476, left X < 8, right X >= 489.
 * If Link_CheckForEdgeScreenTransition returns false (no special-case override),
 * calls Dungeon_HandleEdgeTransitionMovement to start the appropriate trans direction,
 * then sets submodule_index=2 (scroll) if still in module 7. */
...
/* Clamp Link's direction to the transition axis and start the room scroll.
 * kLimitDirectionOnOneAxis masks out the perpendicular movement bits:
 *   east/west (dir 0/1): keep bits 0-1 (vertical) only (& 0x3)
 *   south/north (dir 2/3): keep bits 2-3 (horizontal) only (& 0xC)
 * Then dispatches to the appropriate StartInterRoomTrans function. */
...
/* Submodule 0: normal player control.
 * Handles the three pause-menu inputs: Start (game menu), X (dungeon map), Select (swap
 * menu).  If none are pressed and Link is not locked, routes item-switching inputs through
 * Hud_HandleItemSwitchInputs() then calls Link_Main() for movement and combat. */
...
/* Submodule 1: sub-tile intra-room transition (Link walking through a doorway
 * within the same screen quadrant).  Saves previous coordinates, runs Link's
 * walk animation, then dispatches the current step via
...
/* Intra-room transition step 0: open all shutter doors (set trapdoors_down=0,
 * run OperateShutterDoors with step 7), then start the palette brightness ramp
 * (palette_filter_countdown=31, mosaic_target_level=0) and advance the step counter. */
...
/* Intra-room transition step 1: clear screen-darkening state, set mosaic to 31
 * (full wipe effect), and clear the Somaria-block-switch and water-puzzle flags
 * so stale state from the previous room does not carry over. */
...
/* Intra-room transition step 2: apply dark-room palette filter.
 * If the room has no lantern requirement (dung_want_lights_out == 0), skip
 * directly to the next step.  Otherwise run ApplyPaletteFilter_bounce() twice
 * each frame to accelerate the fade. */
...
/* Intra-room transition step 3: finalize room entry, then close shutter doors if
 * any enemies remain.  Calls ResetThenCacheRoomEntryProperties() to flush scroll
 * state, then sets trapdoors_down=1 and switches to submodule 5 (ControlShutters)
 * if the trapdoors were not already down. */
...
/* Submodule 2: inter-room scroll transition (moving between quadrants or rooms
 * within the same dungeon floor).  Once step >= 1, runs the incremental attribute
 * loader; once step >= 7, also pumps the VRAM upload queue.  Saves Link's coords,
 * runs his animation, then dispatches via kDungeon_InterRoomTrans[subsubmodule_index](). */
void Module07_02_SupertileTransition() {  // 828a26
  ...
  if (!Dungeon_ShouldUseOpenWorldTransitions())
    Link_HandleMovingAnimation_FullLongEntry();
  kDungeon_InterRoomTrans[subsubmodule_index]();
}

/* Inter-room scroll step 0: preserve the dark-with-lantern flag across the
 * transition reset (ResetTransitionPropsAndAdvanceSubmodule clears it), then
 * restore it so the next room correctly inherits the lighting state. */
...
/* Inter-room scroll step 1: load and draw the new room.
 * Calls Dungeon_LoadRoom(), resets star-tile graphics, loads aux GFX,
 * resets all sprites, copies dungeon_room_index to dungeon_room_index2,
 * and mirrors BG scroll offsets unless the dark-lantern flag is still set.
 * Clears the lantern flag after use so the next step runs in normal light. */
...
/* Inter-room scroll step 2: synchronize room geometry and graphics.
 * Clears TS_copy for dark/no-overlay rooms, adjusts quadrant layout,
 * loads the new sprite GFX set, mirrors BG1/BG2 scroll offsets, starts
 * building the water-flood quadrant for VRAM, then advances to step 3. */
...
/* Shared inter-room scroll step: if the room uses darkness/lighting,
 * apply one palette filter bounce before calling the common notDarkRoom
 * helper (which builds the next water quadrant and advances the step). */
...
/* Spiral-staircase shared step: apply one palette filter bounce (for dark-room
 * fading), then build the next water-flood quadrant for VRAM upload. */
...
/* Common inter-room transition helper: build the next BG water-flood quadrant
 * for VRAM upload and advance subsubmodule_index by 1. */
void Dungeon_InterRoomTrans_notDarkRoom() {  // 828ab3
  WaterFlood_BuildOneQuadrantForVRAM();
  subsubmodule_index++;
}

/* Shared inter-room scroll step: like State10, applies a dark-room palette
 * filter bounce then delegates to State4 (PrepareNextRoomQuadrantUpload + advance). */
...
/* Spiral-staircase shared step: apply a dark-room palette filter bounce,
 * package the next room quadrant for VRAM upload, and advance the step. */
...
/* Common inter-room transition helper: package the next room quadrant for VRAM
 * upload via Dungeon_PrepareNextRoomQuadrantUpload() and advance the step. */
void Dungeon_InterRoomTrans_State4() {  // 828ac8
  Dungeon_PrepareNextRoomQuadrantUpload();
  subsubmodule_index++;
}

/* Inter-room scroll step: wait for the screen-scroll to finish (overworld_map_state==5),
 * compute the final landing position via SubtileTransitionCalculateLanding(),
 * optionally apply a dark-room filter bounce, then reset the torch background
 * and player state to complete the transition. */
...
/* Staircase transition: advance the step counter and then reset the torch
 * background and player state (same final step as State12 for stair paths). */
void Dungeon_Staircase14() {  // 828aed
  subsubmodule_index++;
  Dungeon_ResetTorchBackgroundAndPlayer();
}

/* Restore the HUD torch background layer and configure TM/TS based on the
 * room's BG2 properties (kSpiralTab1 decides whether BG1 or BG2 is the
 * main layer, and whether the overlay layer is active). */
...
/* Cancel any active ancilla interactives and, if Link was running (dash),
 * zero out his auxiliary state, incapacitation timer, Z velocity, and run
 * flag.  The TurnWhileDashing enhanced feature suppresses the run-cancel
 * so momentum is preserved across the transition in that mode. */
...
/* Sync BG1 scroll to BG2 and configure TM/TS for the destination room.
 * Special-cases rooms 54 and 56 (Turtle Rock entrance) to skip the layer
 * flip.  Delegates to DungeonTransition_RunFiltering() to apply the
 * fixed-color torch brightness and handle translucency + palette. */
...
/* Apply post-transition color filtering for dark or torch-lit rooms.
 * If the room uses darkness, sets overworld_fixed_color_plusminus from
 * kLitTorchesColorPlus (indexed by number of lit torches), then drives
 * the fixed-color brightness toward that target and zeroes the mosaic.
 * Always calls Dungeon_HandleTranslucencyAndPalette() to sync CGRAM. */
...
/* Inter-room scroll step: fade-filter gate.  If the room is dark, run
 * ApplyPaletteFilter_bounce() up to twice per frame until the countdown
 * reaches zero.  In a lit room, skip immediately to the next step. */
...
/* Inter-room scroll final step: cache room entry state and close shutter
 * doors if enemies are present.  Skips the shutter close for room 172
 * (Agahnim's Tower pre-boss hallway) unless the save flags show the
 * boss has already been fought.  Calls Dungeon_PlayMusicIfDefeated(). */
...
/* After a room transition, check whether the destination is a boss room and
 * set music accordingly.  Room 18 (Ganon) → 0x14, room 2 (Agahnim2) → 0x10.
 * For all other boss rooms (kBossRooms list), plays track 0x15 (boss music)
 * only when sprites are NOT all cleared (battle in progress). */
...
/* Submodule 3: apply a room overlay (alternate tile layout for a solved puzzle).
 * Reads the overlay byte stream from kDungeonRoomOverlay[kDungeonRoomOverlayOffs[...]],
 * draws it into dung_bg2 via Dungeon_DrawRoomOverlay(), schedules DMA packets for
 * each changed tile pair, signals the NMI copy, and returns to submodule 0. */
...
/* Submodule 4: run one frame of the locked-door opening animation. */
void Module07_04_UnlockDoor() {  // 828c0a
  Dungeon_OpeningLockedDoor_Combined(false);
}

/* Submodule 5: run one frame of shutter-door open/close animation.
 * Delegates directly to OperateShutterDoors(). */
void Module07_05_ControlShutters() {  // 828c0f
  OperateShutterDoors();
}

/* Submodule 6: "fat" (wide) inter-room staircase transition (e.g. Hyrule Castle
 * tower stairways).  Runs attribute/VRAM uploads from step 3/13 onward.
 * From step 13, also drives Link's movement: counts staircase_var1 down,
 * boosts speed at 16 remaining, forces link_direction based on which_staircase_index
 * bit 2 (up vs down), calls Link_HandleVelocity() and Dungeon_HandleCamera().
 * The 17-step state machine is dispatched via the switch table below. */
...
/* Fat-staircase init step: optionally stop music for Eastern/Desert Palace
 * entrance stairs (rooms 7/23 without pendant), set the walk-timer
 * (staircase_var1 = 106 for down, 88 for up), reset overworld_map_state,
 * and advance via ResetTransitionPropsAndAdvanceSubmodule(). */
...
/* Clear overworld_map_state then run the full transition property reset.
 * Wrapper that ensures the interface flags are zeroed before calling
 * ResetTransitionPropsAndAdvanceSubmodule(). */
...
/* Core transition reset: clear mosaic, palette filter, and mosaic target;
 * zero torch count; if the room is dark, configure CGWSEL/CGADSUB for
 * lantern-mode color math (CGWSEL=0x02, CGADSUB=0xB3), then clear the
 * dark flag, cancel any running state on Link, reload palette caches, and
 * increment subsubmodule_index to move to the next transition step. */
...
/* Load the destination room for a special staircase transition.
 * Adjusts Link's world coordinates for the room offset difference
 * (Dungeon_AdjustAfterSpiralStairs), then fully loads and draws the new room,
 * resets star-tile graphics, loads custom tile attributes, syncs room index,
 * reinitializes the follower, and advances the step. */
...
/* Load the new room's sprite GFX set, reset all sprite slots, then run the
 * filtering step (DungeonTransition_RunFiltering) to sync color math. */
...
/* Set up BG layer and camera for the "fat" staircase room-to-room scroll.
 * Mirrors BG1→BG2 offsets, adjusts quadrant layout, sets TM/TS based on
 * room BG2 properties.  Updates dung_cur_floor (−1 for down, +1 for up),
 * sets staircase_var1 walk-timer (32 down / 48 up), plays footstep SFX
 * and the "step" ambient sound, then calls notDarkRoom to queue the
 * water-quadrant upload and advance the step. */
...
/* Reset all transition-state counters to zero (overworld_map_state,
 * subsubmodule_index, overworld_screen_transition, submodule_index,
 * water-puzzle flag, movable-block push flag) and cache the current
 * camera scroll bounds via CacheCameraProperties(). */
...
/* Kick off an NMI subroutine 9 (BG CHR slots 3+4 update) to load the
 * destination room's main tile-set graphics, and advance the step. */
...
/* Kick off NMI subroutine 10 (BG CHR slots 5+6 update) to load the
 * destination room's secondary/animated tile-set graphics, and advance the step. */
void DungeonTransition_TriggerBGC56UpdateAndAdvance() {  // 828e1d
  nmi_subroutine_index = nmi_disable_core_updates = 10;
  subsubmodule_index += 1;
}

/* Submodule 7: falling / pit-fall transition (Link drops into a lower room).
 * From step 6 onward, pumps the VRAM upload queue, runs the attribute loader,
 * and incrementally applies grayscale.  Dispatches each frame to
...
/* Falling transition step 0: stop music when entering the Hera/Ice/Mire bosses
 * (rooms 0x10, 7, 0x17), then run the interface+property reset. */
...
/* Falling transition step 6: mirror BG1 scroll to BG2, adjust quadrant layout,
 * set TM/TS from room properties, then build one water-flood quadrant for VRAM
...
/* Falling transition step 15: fade in palette.  While darkening_or_lightening_screen
 * is non-zero, wait.  Once faded in, snap Link's Y position to the current tile row,
 * play boss music if needed, and decrement dung_cur_floor (unless this is a special
 * pit room 0x89/0x4f which keeps the floor, or 0xA7 which locks it to floor 1). */
...
/* After changing floors: set hud_floor_changed_timer=1 to trigger the HUD
 * floor-indicator blink, play the "floor change" sound (SFX2 = 36),
 * then mark and save the visited quadrant flags. */
...
/* Falling transition step 16: run the landing-from-pit handler each frame.
 * Once the landing is complete (submodule_index cleared by handler), switch
 * back to submodule 7, step 17, and reload the CHR half-slot. */
...
/* Falling transition step 17: wait for the screen-scroll to finish
 * (overworld_map_state == 5), then cache room entry properties and
 * set the boss music via Dungeon_PlayMusicIfDefeated(), reload CHR slot. */
...
/* Straight intra-room stair step 0: configure the walk timer and SFX.
 * Walking north (link_direction & 8): timer = 0x38 frames, SFX1 = 23 (step up),
 *   also clears the lower-level mirror and conditionally lower-level flags.
 * Walking south (link_direction & 4): timer = 0x3c frames, SFX1 = 25 (step down).
 * Sets link_speed_modifier = 1 then advances the step. */
...
/* Straight intra-room stair step 1: wait until staircase_var1 reaches zero
 * (walk animation completed), then set the lower-level flags for south-facing
 * stairs (walking downward), reset transition counters, and save quadrant flags. */
...
/* South-facing straight stair step 0: same structure as 08_00 but toggles
 * (XOR) link_is_on_lower_level and its mirror when heading south (downward),
 * setting 0x3c / SFX 25 for down or 0x38 / SFX 23 for up. */
...
/* South-facing straight stair step 1: wait for staircase_var1 to reach zero,
 * then toggle lower-level flags for a north-direction exit (walking upward),
 * reset transition counters, and save quadrant flags. */
...
/* Submodule 9: run one frame of the bomb-cracked-door opening sequence. */
...
/* Submodule 0x0B: drain a Swamp Palace water pool.
 * Step 0: slide the HDMA water-window using kTurnOffWater_Tab0 (pattern
 *   −1,−1,−1,+1 cycling every 8 frames) until var2 == var4 then sets drained attrs.
 * Step 1: fill all of dung_bg1 with the "dry floor" tile from SrcPtr(0x1e0),
 *   reset dung_cur_quadrant_upload, and advance.
 * Steps 2-5: upload the modified tilemap one quadrant per step. */
...
/* Submodule 0x0C: fill a Swamp Palace water room (flood sequence).
 * Steps 0-3: prepare the BG1 tilemap with flood water tiles (one quadrant each).
 * Steps 4-8: every 4 frames advance the flood depth; updates water HDMA vars
 *   to animate the water "vomit" (rising surface) using kAdjustWaterVomit tables.
 * Step 9: configure the window-mask / color-math registers for the water
 *   translucency effect, then advance.
 * Step 10: animate the water surface rising using kTurnOnWater_Tab0/1; finishes
 *   when the surface reaches the calculated room height (r0 = 0x688 - BG2VOFS - 0x24).
 * Step 11: slide the HDMA window to final position using kTurnOnWater_Tab2,
 *   applying additional vomit steps at distances 0 and 8 from var4. */
...
/* Submodule 0x0D: animate the flood-dam water gate.
 * Each frame calls FloodDam_PrepFloodHDMA() to update the HDMA water tables,
 * then dispatches to kWatergateFuncs[subsubmodule_index]() for the current step. */
void Module07_0D_FloodDam() {  // 82904f
  FloodDam_PrepFloodHDMA();
  kWatergateFuncs[subsubmodule_index]();
}

/* Submodule 0x0E: spiral staircase inter-room transition.
 * From step 7 onward, pumps the VRAM upload queue and the attribute loader.
 * Each frame calls HandleLinkOnSpiralStairs() to drive Link's animated walk
 * along the spiral, then dispatches via kDungeon_SpiralStaircase[subsubmodule_index](). */
...
/* Apply two palette-filter bounces per frame (fast double-fade) and run
 * one step of the incremental grayscale effect.  Used at step 14 of the
 * fat-staircase and spiral-staircase transitions to fade to grey quickly. */
...
/* Spiral staircase step 2: count down staircase_var1 (waiting for the animation
 * to play out).  While var1 >= 9, apply up to two palette-filter bounces.
 * When var1 reaches 0, set link_visibility_status = tagalong_var5 = 12 to hide
 * Link during the inter-room crossfade. */
...
/* Sync all BG scroll/layer registers after a spiral-staircase room change.
 * Temporarily adjusts Link's Y by ±48 to compute the correct wall-priority
 * region (SpiralStairs_MakeNearbyWallsHighPriority_Exiting), then restores it.
 * Mirrors BG1→BG2 offsets, adjusts quadrant layout, sets TM/TS, updates
 * dung_cur_floor (−1 for down / +1 for up), sets the walk timer to 24,
 * plays the floor-change blip, and begins the next water-quadrant upload. */
...
/* Spiral staircase: reset Link's dynamic state, set staircase_var1 = 0x38,
 * advance the step, then immediately check/start boss music.  Used mid-transition
 * so the music switches as soon as Link steps onto the new floor. */
...
/* Start boss music for the current room if appropriate.
 * Room 16 → track 0x1C (Ganon fight), room 7 → 0x15 (Eastern boss),
 * room 23 without pendant-1 → 0x11 (Desert boss pre-pendant) — but only
 * if the boss track isn't already suppressed (music_unk1 != 0xF1) and
 * the first pendant hasn't been collected. */
...
/* Spiral staircase step 17: keep moving Link to the landing spot each frame
 * via SpiralStairs_FindLandingSpot(), counting down staircase_var1.
 * When it hits zero, reset it (10 for down / 24 for up) and advance the step. */
...
/* Spiral staircase step 18: continue sliding Link to the landing spot.
 * When staircase_var1 hits zero, advance the step and clear overworld_map_state
 * to signal the transition is ready to finalize. */
...
/* Spiral staircase step 0: set high-priority wall tiles around the staircase
 * opening (SpiralStairs_MakeNearbyWallsHighPriority_Entering), then if Link
 * is on the lower layer, swap TM/TS so the lower background is the main plane
 * and mark link_is_on_lower_level = 3 (both mirror and real bits). */
...
/* Spiral staircase step 19 (final): restore the correct layer flags for the
 * destination floor using kTeleportPitLevel1/2[cur_staircase_plane], swap TM/TS
 * back to upper-layer mode, optionally clear wall high-priority for upward exits,
 * sync dungeon_room_index2, then cache room entry state to finish the transition. */
...
/* After each spiral stair step, snap Link's world position to the exit tile.
 * Computes an index (0-3) from cur_staircase_plane, byte_7E0492, and which_staircase_index
 * bit 2, then applies kSpiralStaircaseX/Y[i] offsets.
 * If TM_copy has bit 0x10 set (lower layer is main), handles plane 2 specially:
 * flips to upper-layer mode and adjusts Y.  Otherwise flips to lower-layer mode.
 * Reinitializes the follower sprite after repositioning. */
...
/* Set bit 0x2000 (high tile priority) on the 5-column × 4-row wall strip
 * around the staircase opening in dung_bg2 for an upward-exit transition.
 * Only active when which_staircase_index bit 2 is clear (going up / exiting
 * to a lower floor number).  Searches dung_inter_starcases[] for the matching
 * stair position, walks back 4 tiles, and stamps 4 rows of 5 columns. */
...
/* Submodule 0x0F: spotlight (iris-wipe) transition used after Link lands in a new
 * room (pit falls, dungeon entrances with falling animation).
 * Dispatches via kDungeon_Submodule_F[subsubmodule_index](), then runs
 * Link's animation and OAM update each frame. */
...
/* Spotlight step 0: open the iris spotlight (calls Spotlight_open()) and advance. */
void Module07_0F_00_InitSpotlight() {  // 82932d
  Spotlight_open();
  subsubmodule_index++;
}

/* Spotlight step 1: run sprites, update the iris table each frame.
 * When the spotlight finishes (submodule_index cleared by the iris handler),
 * disable all window masking registers (W12SEL, W34SEL, WOBJSEL, TMW, TSW),
 * reset step counters, and start any queued music track. */
...
/* Straight inter-room staircase step 0: stop any running dash (resets run flag,
 * sets speed_setting = 2), play the stair SFX (24 = ascending, 22 = descending),
 * stop music for Palace of Darkness / Misery Mire entrance stairs (rooms 48/64),
 * then run the full interface+property reset. */
...
/* Straight inter-room staircase step 1: while staircase_var1 >= 9, apply
 * palette-filter bounces each frame.  Advance to the next step when
 * palette_filter_countdown reaches exactly 23. */
...
/* Straight inter-room staircase step 2: load the destination room while the
 * palette is still faded.  Restores star-tile CHR, loads aux GFX and custom
 * tile attributes, adjusts for the new room layout, reinitializes the follower,
...
/* Straight inter-room staircase step 3: continue fading and trigger BG CHR
 * slots 3+4 NMI update (main tile-set graphics) then advance. */
void Module07_11_03_FilterAndLoadBGChars() {  // 829422
  ApplyPaletteFilter_bounce();
  DungeonTransition_TriggerBGC34UpdateAndAdvance();
}

/* Straight inter-room staircase step 4: continue fading, trigger BG CHR slots 5+6
 * NMI update (secondary/animated tile-set), sync dungeon_room_index2, and reset sprites. */
...
/* Straight inter-room staircase step 11: configure BG layers for the destination
 * room (TM/TS from room properties), adjust dung_cur_floor (±1), set walk timer
 * and SFX.  Offsets Link's Y coordinate for lower-level rooms (adds or subtracts
 * 32 px depending on staircase direction and whether this is a northward submodule).
 * Restores lower-level flags from kTeleportPitLevel1/2, plays floor-change blip,
 * restores torch HUD layer, then queues the water-quadrant upload and advances. */
...
/* Straight inter-room staircase step 9: apply a palette filter bounce,
 * decrement subsubmodule_index back by 1 (loops at this step while fading),
 * load new sprite GFX set, and sync translucency/palette registers. */
...
/* Straight inter-room staircase step 25: wait for the screen-scroll and
 * darkening to finish (overworld_map_state==5, darkening==0), then advance,
 * start the destination-room music (0x1C for Hyrule Castle, 0x10 for Agahnim),
 * and run incremental grayscale. */
...
/* Straight inter-room staircase step 17 (sliding phase): advance to the next
 * step only once staircase_var1 reaches zero; until then, run the incremental
 * grayscale each frame to keep fading while Link walks along the stairs. */
...
/* Submodule 0x14: recover from a pit fall (drown / hole recovery sequence).
 * Step 0: scroll the camera back to the cached position (Module07_14_00_ScrollCamera).
 * Step 1: run RecoverPositionAfterDrowning() to restore Link's coords. */
...
/* Camera-scroll recovery: nudge BG2HOFS and BG2VOFS by ±1 each frame
 * (two iterations per frame) toward the cached scroll values
 * (BG2HOFS/VOFS_copy2_cached).  Once both axes reach their targets, advance
 * to step 1.  Keeps BG1 mirrored to BG2 when not in lantern-dark mode. */
...
/* Submodule 0x15: warp-pad teleportation transition.
 * From step 3 onward, pumps the VRAM upload queue and the attribute loader.
 * Dispatches each frame via kDungeon_Teleport[subsubmodule_index](). */
...
/* Warp-pad step 1: apply the mosaic zoom effect (ConditionalMosaicControl),
 * set MOSAIC_copy to mosaic_level | 3 (enable on BG1+BG2), and apply
 * one palette-filter bounce (fading out toward the teleport). */
...
/* Warp-pad step 4: apply incremental grayscale, fix floor for Ganon's Tower
 * entrance (room 0x17 → floor 4), mirror BG scrolls, adjust quadrant layout,
 * set TM/TS from room properties, build one water quadrant for VRAM, advance. */
...
/* Warp-pad step 14: fade in while un-zooming the mosaic.
 * On every odd filter frame where mosaic_level > 0, subtract 0x10 (reducing
 * zoom by one step).  Applies the reduced mosaic to BGMODE and palette filter. */
...
/* Warp-pad step 15 (final): wait for overworld_map_state == 5 (scroll complete),
 * then save the visited quadrant flags, return to submodule 0, and cache
 * room entry properties to complete the teleport transition. */
...
/* Submodule 0x16: animate a crystal-peg toggle (hammer-struck orange/blue switch).
 * Advances subsubmodule_index every frame, but only acts every 4th frame.
 * Steps (each 4-frame group):
 *   0-1: Module07_16_UpdatePegs_Step1 (update peg tiles in tilemap).
 *   2:   Module07_16_UpdatePegs_Step2 (schedule VRAM DMA for changed tiles).
 *   3:   RecoverPegGFXFromMapping (restore peg attribute colors from mapping).
 *   4:   Dungeon_FlipCrystalPegAttribute() then return to submodule 0. */
...
/* Submodule 0x17: pressure-plate tile restore (small floor switch that
 * pops back up after Link steps off).  Counts subsubmodule_index down; when
 * it reaches 0, nudges Link's Y up by 2, restores the switch tile in the BG
 * at the stored position (word_7E04B6), and returns to the saved submodule. */
...
/* Submodule 0x18: boss-defeat crystal rescue cutscene setup.
 * Step 0: restore BG subtractive color math, copy palette slot 0 from slot 32,
 *   fade to white (darkening_or_lightening_screen → 255), then blank both BG
 *   layers, reset offsets and quadrant counter.
 * Step 1: set up palette filter for crystal sheen, immobilize Link, look up the
 *   boss index from kBossRooms, then stamp the crystal-maiden tile blocks into
 *   dung_bg1 using kCrystal_Tab0 as the base position.
 * Steps 2-9: alternating notDarkRoom/State4 pairs to upload 4 quadrants.
 * Step 10: start the NMI thread, initialize the polyhedral crystal sprite,
 *   launch CrystalCutscene_Initialize(), and return to submodule 0. */
...
/* Submodule 0x19: Magic Mirror exit fade — used when Link uses the mirror
 * to warp back to the overworld.
 * Each frame: run the mosaic reset (Overworld_ResetMosaic_alwaysIncrease) and
 * decrement INIDISP_copy (screen brightness).  When brightness reaches 0,
 * switch to overworld module 5, reset submodule/NMI flags, save current music
 * track, and swap palette back if a translucency palette was active. */
...
/* Submodule 0x1A: animate the Triforce room door opening (the gold gate in Ganon's
 * pyramid).  While R16 != 0, counts down a 16-bit timer; on expiry, plays the
 * door-creak ambient SFX and releases Link's sword-hold and direction lock.
 * Every 4 frames (subsubmodule_index & 3 == 0), stamps one column of door tiles
 * from kOpenGanonDoor_Tab[] into dung_bg2 at column 44, advancing 1 column per
 * group.  At step 16, writes passable collision attributes, opens the southern
 * room boundary (room_bounds_y.a0 = −64), and resets to submodule 0. */
...
/* Module 11 (dungeon falling entrance): handles entering a dungeon via a falling
 * animation (Link drops in from above, e.g. the Eastern Palace pit entrance).
 * Step 0: stop music if this is a fresh game start, then reset transition props.
 * Step 1: apply a palette filter bounce every other frame (gradual fade in).
 * Step 2: load the room (Module11_02_LoadEntrance — heavy initialization).
 * Step 3: load sprite GFX set.
 * Step 4/5: fade INIDISP up to 15 (full brightness), then run the pit landing
 *   handler; on completion, switch to Module 7, queue quadrant/entry caches,
 *   start the queued music, and save the music state. */
...
/* Heavy dungeon entrance initialization called during Module 11 step 2.
 * Enables force-blank, sets CGWSEL, loads the entrance/room data, restores
 * key count from per-dungeon save, rebuilds HUD, sets Link's initial state
 * (this_controls_sprite_oam, near_pit_state, visibility, speed, animation).
 * Computes the off-screen Y for the falling entry, clears torch/dark flags,
 * calls Dungeon_LoadAndDrawRoom(), loads custom tile attributes and animated
 * dungeon tiles, loads the attribute table, initializes tilesets, loads palettes,
 * restores torch HUD, clears button state, resets player state, loads bunny
 * palette if applicable, enables HDMA, refreshes HUD, sets ambient SFX, and
 * sets submodule_index = 7 (main dungeon loop). */
...
/* Ensure the correct music bank is loaded for the queued track.
 * If queued_music_control is 0xFF (no pending) or 0xF2 (stop), do nothing.
 * For overworld tracks (3, 7, 14), calls LoadOWMusicIfNeeded().
 * For all other dungeon tracks, marks flag_which_music_type = 1 and loads
 * the dungeon song bank via LoadDungeonSongs(). */
...
/* Called when Link uses the Magic Mirror inside a dungeon.
 * If not inside a dungeon (cur_palace_index_x2 == 0xFF), plays the "can't use"
 * SFX (60) and returns.  Otherwise sets submodule 25 (mirror fade), plays the
 * mirror SFX (51), saves the current room's quadrant-visit flags, and persists
 * the key count to per-dungeon SRAM. */
...
/* Write the current key count (link_num_keys) into the per-dungeon SRAM array.
 * Uses cur_palace_index_x2 >> 1 as the dungeon index; remaps index 2 → 0
 * (Hyrule Castle shares its key slot with the sewers).  No-ops if index is 0xFF. */
...
/* Translate Link's world position and all camera/room-bound variables by the
 * difference in room grid coordinates between dungeon_room_index and
 * dungeon_room_index_prev.  Used after a spiral staircase crosses a room boundary
 * to snap Link into the correct position in the new room without a visible jump.
 * X offset = (new_room_col − old_room_col) × 0x200 px.
 * Y offset = (new_room_row − old_room_row) × 0x200 px. */
...
/* Teleport-door room-alignment: snap Link and all scroll/camera/bound variables
 * so that Link is correctly positioned inside room `room` after crossing a
 * teleport-tile door (tile attribute 0x89).  `flag` is an X-alignment bias (0 or 1).
 * Computes the required X and Y world-coordinate delta from room's grid column/row
 * versus Link's current position, applies it to link_x/y_coord, BG2HOFS/VOFS,
 * all four room_bounds_{x,y} entries, and resets tagalong Y to match. */
...
/* Recompute the composite quadrant index and the full-size quadrant flags.
 * Updates composite_of_layout_and_quadrant from the room layout + current
 * quadrant.  Sets quadrant_fullsize_x/y to 2 if the blast-wall flag or the
 * layout-quadrant flag table indicates a full-width quadrant, else 0.
 * The dung_unk2 overrides allow scripted layout changes (e.g., blast-wall events). */
...
/* Nudge Link 8 pixels east before starting the eastward inter-room scroll.
 * Used when Link walks into the right edge of a quadrant to pre-advance his
 * position so the transition feels seamless. */
void HandleEdgeTransitionMovementEast_RightBy8() {  // 82b62e
  link_x_coord += 8;
  Dungeon_StartInterRoomTrans_Right();
}

/* Begin an eastward inter-room scroll transition.
 * Flips link_quadrant_x, recomputes the composite quadrant, advances the
 * A camera bound by one quadrant, saves the current room, and sets the camera
 * scroll target for rightward movement.  If crossing into a new grid room (quadrant
 * wraps to 0): also advances the B bound, updates dungeon_room_index (or handles
 * a teleport-door via Dungeon_AdjustForTeleportDoors / Dungeon_AdjustAfterSpiralStairs),
 * copies room_transitioning_flags to toggle lower-level and palace bits, then sets
 * submodule_index = 2 (supertile transition).  Updates the Y full-size flag. */
...
/* Nudge Link 16 pixels south before starting the southward inter-room scroll. */
 ...
 * Saves the key count and current quadrant visit bits to SRAM, sets
 * saved_module_for_menu = 8 (dungeon), then switches to main module 15
 * (overworld) and resets submodule/subsubmodule counters.  Also cancels
 * any dash-run state on Link. */
...
/* Complete an intra-quadrant rightward camera advance: flip link_quadrant_x,
 * recompute composite, advance the A camera bound, then save visited flags. */
...
/* Mark the current quadrant as visited in dung_quadrants_visited and persist
 * it to save_dung_info[dungeon_room_index].
 * Uses kQuadrantVisitingFlags indexed by (quadrant_fullsize_y<<2 | quadrant_fullsize_x<<1 |
 * link_quadrant_y + link_quadrant_x) to compute the correct visit bitmask. */
...
/* Persist the current room's visited quadrant bits to save_dung_info without
 * recomputing the visit mask (just OR's dung_quadrants_visited into the slot). */
void SaveQuadrantsToSram() {  // 82b8e5
  save_dung_info[dungeon_room_index] |= dung_quadrants_visited;
}

/* Complete an intra-quadrant leftward camera retreat: flip link_quadrant_x,
 * recompute composite, subtract one screen from the A camera bound, save flags. */
...
/* Complete an intra-quadrant downward camera advance: flip link_quadrant_y
 * (toggles bit 1), recompute composite, advance A camera bound on Y axis, save flags. */
...
/* Complete an intra-quadrant upward camera retreat: flip link_quadrant_y,
 * recompute composite, subtract one screen from the A camera bound on Y, save flags. */
...
/* Compute the current quadrant visit bitmask and OR it into dung_quadrants_visited,
 * then save the full room state (door-open bits, puzzle flags, visit bits) to
 * save_dung_info.  Called before room transitions to ensure progress is not lost. */
void Dungeon_FlagRoomData_Quadrants() {  // 82b929
  dung_quadrants_visited |= kQuadrantVisitingFlags[(quadrant_fullsize_y << 2) + (quadrant_fullsize_x << 1) + link_quadrant_y + link_quadrant_x];
  Dung_SaveDataForCurrentRoom();
}

/* Pack and write the current room's persistent data to save_dung_info[dungeon_room_index].
 * Stores: dung_savegame_state_bits >> 4 (puzzle/boss bits), dung_door_opened & 0xF000
 * (which doors are unlocked), and dung_quadrants_visited (which quadrants were entered). */
...
/* Configure the scroll trigger window for a room-edge transition.
 * Sets overworld_screen_transition = arg (1=west, 2=east, 3=north, 4=south in practice).
 * For horizontal transitions (link_direction & 3): picks camera_x_coord_scroll_low
 * from kCameraBoundsX[direction_bit | quadrant_x] and sets the hi bound = low + 2.
 * For vertical transitions: picks camera_y_coord_scroll_low from kCameraBoundsY[dir | qy]. */
...
/* Recompute composite_of_layout_and_quadrant by OR-ing the room layout byte
 * with link_quadrant_y | link_quadrant_x.  Used to index kLayoutQuadrantFlags. */
...
/* Camera tracking: scroll BG2 to follow Link at the transition thresholds.
 * Y-axis: if link_y_vel != 0, iterates |y_vel| times.  When Link's adjusted
 *   Y (coord − z, wrapped to 9 bits, + 12) crosses camera_y_coord_scroll_low/hi,
 *   nudges BG2VOFS_copy2 by ±1, then scrolls BG1VOFS_copy2 by ½ (sub-pixel
 *   tracked in BG1VOFS_subpixel) and moves the trigger window.
 * X-axis: same logic for link_x_vel and BG2/BG1HOFS.
 * After both axes, if in a normal room (not 0xFFFF) and bg2_properties is not
 *   the layer-independent type (1/5), snaps BG1 to BG2. */
...
/* Snap BG1 scroll registers to BG2: sets BG1HOFS_copy2 = BG2HOFS_copy2 and
 * BG1VOFS_copy2 = BG2VOFS_copy2.  Used at the end of room transitions to
 * ensure both background layers are aligned before gameplay resumes. */
void MirrorBg1Bg2Offs() {  // 82bb7b
  BG1HOFS_copy2 = BG2HOFS_copy2;
  BG1VOFS_copy2 = BG2VOFS_copy2;
}

/* Set the horizontal scroll start/end targets for a room transition.
 * arg is the new link_quadrant_x (0 or 1); kUpDownScroll maps it to a pair
 * of pixel positions: left_right_scroll_target and left_right_scroll_target_end. */
...
/* Set the vertical scroll start/end targets for a room transition.
 * arg selects a pair of pixel values from kUpDownScroll[arg]/[arg+1]:
 * up_down_scroll_target (current edge position) and up_down_scroll_target_end
 * (target edge position after the scroll). */
...
/* Advance one room-scroll tick and return true when the scroll reached its
 * target.  Keeping the original body isolated lets widescreen mode drain the
 * same math in a tight loop instead of maintaining a second snap formula. */
static bool DungeonTransition_ScrollRoomOneStep() {
  transition_counter++;
...
/* Advance the room-scroll by one step (called each frame during submodule 2).
 * Uses overworld_screen_transition (0=up, 1=down, 2=left, 3=right) to pick
 * direction delta from kStaircaseTab3.  Scrolls BG2HOFS or BG2VOFS (and mirrors BG1),
 * rounds to even pixels.  Once Link has moved enough frames (kStaircaseTab4), also
 * nudges link_x/y_coord.  When the scroll reaches (&up_down_scroll_target)[i],
 * marks the quadrant visited, increments subsubmodule_index, resets transition_counter,
 * and (for submodule 2) queues the next water-quadrant for VRAM upload. */
void DungeonTransition_ScrollRoom() {  // 82be03
  if (Dungeon_ShouldUseOpenWorldTransitions()) {
    for (int guard = 0; guard < 256; guard++) {
      if (DungeonTransition_ScrollRoomOneStep())
        return;
    }
  }
  DungeonTransition_ScrollRoomOneStep();
}

/* Straight inter-room stair scroll step: hide Link and his tagalong (visibility = 12),
 * then scroll BG2VOFS by kStaircaseTab3[direction] each frame (rounds to 4-pixel
 * boundary).  When BG1VOFS reaches the target (&up_down_scroll_target)[direction],
 * snap Link's Y by kStaircaseTab5, un-hide him, and advance the step. */
...
/* Find the tile-aligned landing position at the end of an intra-room transition.
 * Resets Link's dynamic state, computes the landing coordinate via
 * SubtileTransitionCalculateLanding(), advances the step, and persists
 * the quadrant visit flags. */
...
/* Compute the final sub-pixel coordinate where Link should land after an intra-room
 * transition.  Calls CalculateTransitionLanding() to read the attribute byte under
 * Link and classify the tile type (0=clear, 1=vert door, 2=horiz door, 3=stair, 4=corner).
 * Remaps values 2→1 and 4→2, offsets by screen_transition×5, then looks up the
 * exact byte coordinate from kStaircaseTab2 and applies ±8 bias depending on direction.
 * Writes to BYTE(link_y_coord) for vertical transitions or BYTE(link_x_coord) for
 * horizontal, and clears link_visibility_status. */
...
/* Inter-room scroll state 13: apply a dark-room palette filter bounce if needed,
 * then run Dungeon_IntraRoomTrans_State5 (move Link out through the door). */
...
/* Intra-room transition step 5 (walk-out phase): animate Link's walk each frame
 * and call DungeonTransition_MoveLinkOutDoor().  Once Link has fully exited the
 * door tile, clear the doorway-flag and force-direction overrides, reset
 * overworld_screen_transition, and advance the step. */
void Dungeon_IntraRoomTrans_State5() {  // 82c170
  ...
  if (Dungeon_ShouldUseOpenWorldTransitions()) {
    ...
    if (!finished)
      return;
  } else if (!DungeonTransition_MoveLinkOutDoor()) {
    return;
  }
...
/* Nudge Link 2 pixels per frame in the transition direction and test whether
 * he has reached the tile-aligned exit position.  Uses byte_7E004E (door slot
 * type) and overworld_screen_transition to look up the target coordinate from
 * kStaircaseTab2.  Returns true once BYTE(link_y/x_coord) matches (masked &0xFE). */
...
/* Sample the collision attribute byte under Link and classify the tile type.
 * Computes the attribute table word index from Link's Y and X coordinates plus
 * the lower-level bit.  Decodes the attribute byte (masking to 0x8E) into one of:
 *   0 = clear/no-door, 1 = vertical door (0x80), 2 = door variant (0x82/other),
 *   3 = horizontal door (0x84/0x88), 4 = corner/diagonal (0x86).
 * Stores the result in byte_7E004E and returns it. */
...
/* Initialize all dungeon state when Link enters a dungeon from the overworld.
 * Sets player_is_indoors = 1, then — unless recovering from a death (death_var5) —
 * snapshots all overworld scroll, camera, tile-theme, and palette indices to their
 * _exit copies so they can be restored when Link returns.
 *
 * Two initialization paths:
 *   Death/follower restart (follower_indicator==4 or death_var4 set): uses
 *     kStartingPoint_* arrays indexed by which_starting_point to set all room,
 *     scroll, camera, player, bounds, layer, quadrant, and music variables.
 *   Normal entrance (which_entrance): uses kEntranceData_* arrays to populate
 *     the same set of variables from the entrance record, including the music
 *     track (ZeldaGetEntranceMusicTrack), the doorway orientation flag, and the
 *     starting BG layer assignment for the staircase system.
 *
 * After both paths: resets player OAM offsets, direction masks, Z-coord, and
 * initializes movable-block, torch, and memorized-tile arrays from ROM templates. */
...
/* Drive one pushable-block object one frame.
 * j: the object's raw byte index (compared to (index_of_changable_dungeon_objs[1]-1)*2).
 * Sets the timeout to 9 and the step counter to 0, calls PushBlock_ApplyVelocity()
 * to advance the block's position, then calls PushBlock_HandleCollision() to test
 * whether the block has hit Link and should push him back. */
...
/* Handle a pushable block that has fallen into a pit.
 * y: the raw byte index >> 1 (slot number in dung_replacement_tile_state[]).
 * Counts down pushedblocks_maybe_timeout every frame (resets to 9 per cycle).
 * After 4 cycles (pushedblocks_some_index reaches 4), clears the replacement-tile
 * state for this slot and nulls the corresponding index_of_changable_dungeon_objs entry. */
...
/* Advance a pushable block's sub-pixel position by its velocity.
 * i: block slot (0 or 1).  Reads pushedblock_facing[i] to determine axis and
 * direction (kPushedBlockDirMask: 8=up, 4=down, 2=left, 1=right).
 * Applies ±12 sub-pixel units per frame to pushedblocks_x/y_{lo,hi,subpixel}.
 * When the low byte of the position reaches pushedblocks_target[i] (a tile boundary),
 * advances the replacement-tile state and releases the drag-state bit.
 * Also scans all active sprites: if any sprite is within 16×16 px of the block,
 * assigns a recoil velocity away from the block using kPushBlockTab1/2. */
...
/* Test whether a moving pushable block overlaps Link and push him back if so.
 * i: block slot, x/y: block's current world coordinates.
 * Determines the push axis from link_direction (first set bit in 0-3 order).
 * Builds bounding ranges for the block and Link on each axis using the kPushBlock_A-F
 * tables.  If the ranges overlap, sets bitmask_of_dragstate bit 4 (or bit 1 if
 * the block has no index).  If Link is within 8 px on the push axis, displaces
 * his coordinate and velocity to prevent overlap.  Calls HandleIndoorCameraAndDoors()
 * to update camera + door collision after repositioning. */
...
/* Draw OAM sprites for all active pushable blocks (slots 0 and 1).
 * Iterates both index_of_changable_dungeon_objs slots; if the slot is non-zero
 * (a block is registered), calls Sprite_HandlePushedBlocks_One(i) to render it. */
...
/* Prepare Link for a straight inter-room staircase walk.
 * Removes any type-13 ancilla (staircase trigger ancilla) from all slots.
 * Resets Link's animation and sub-pixel accumulators, sets the push-tired
 * timer to 28 and the staircase countdown to 32, and disables sprite damage.
 * Stores the staircase walk target in tiledetect_which_y_pos[1]:
 *   which_staircase_index bit 2 set (going down) → link_x − 15,
 *   otherwise (going up) → link_x + 16.
 * Stores link_y_coord in tiledetect_which_y_pos[0] for camera tracking. */
...
/* Animate Link's walk along a spiral staircase, called every frame from Module07_0E.
 * Saves previous coordinates, returns early if some_animation_timer_steps is set
 * (brief pause between stair steps).  Otherwise zeros damage/incapacitation state
 * and drives Link's velocity:
 *   Going down (bit 2 set): Y = −2; after push-timer expires, also X = −2.
 *   Going up:               Y = −2; after push-timer expires, also X = +2.
 * Calls Link_MovePosition() and Link_HandleMovingAnimation_StartWithDash().
 * Updates link_direction_facing based on which half of the spiral Link is on.
 * When Link's X matches tiledetect_which_y_pos[1] (the next stair tile target),
 * calls RepositionLinkAfterSpiralStairs(), updates tiledetect_which_y_pos[1]
 * for the next step, and plays the stair footstep SFX. */
...
/* Animate Link sliding to the landing position at the bottom of a spiral staircase.
 * Clears damage/incapacitation, re-enables sprite damage, saves previous coords.
 * Counts down countdown_timer_for_staircases; at 0, faces Link forward (direction 2).
 * Velocity phases:
 *   Default: X = +4, Y = 0 (sliding right toward landing).
 *   Going down (bit 2 set): X = −4, Y = +2.
 *   some_animation_timer_steps == 2: X = 0, Y = +16 (final drop).
 * Calls Link_MovePosition() and Link_HandleMovingAnimation_StartWithDash().
 * When BYTE(link_x_coord) reaches tiledetect_which_y_pos[1], sets
 * some_animation_timer_steps = 2 to trigger the final-drop phase. */
...
/* Dispatch the current room's BG layer effect.
 * dung_hdr_collision_2 indexes kDungeon_Effect_Handler[] to select the
 * per-room layer-effect function (scroll, Trinexx lava, Agahnim2 flash,
 * invisible floor, Ganon translucency, water rapids, or nothing). */
void Dungeon_HandleLayerEffect() {  // 8afe80
  kDungeon_Effect_Handler[dung_hdr_collision_2]();
}

/* No-op layer effect for rooms with dung_hdr_collision_2 == 0. */
void LayerEffect_Nothing() {  // 8afe87
}

/* Scrolling-floor layer effect (e.g., Swamp Palace currents, conveyor tiles).
 * Stops if the boss is defeated (dung_savegame_state_bits & 0x8000).
 * Accumulates dung_some_subpixel[1] by 0x80 per frame; the integer part becomes
 * the floor velocity.  If dung_floor_move_flags bit 1 is set, negates velocity.
 * Flags < 4 → horizontal scroll (dung_floor_x_vel, BG1HOFS += x_offs);
 * flags >= 4 → vertical scroll (dung_floor_y_vel, BG1VOFS += y_offs).
 * Flag bit 0 pauses the scroll without clearing velocity. */
...
/* Trinexx boss room layer effect: apply the accumulated floor scroll offsets
 * to BG1 this frame (x_offs += x_vel, y_offs += y_vel), then clear velocities
 * so the sprite code can set them fresh next frame for the lava-flow animation. */
...
/* Agahnim 2 boss room layer effect: flashes palette slots 0x6D/0x6E/0x6F/0x77
 * on specific frame counts (3, 5, 36, 38 mod 128) to create the lightning-flash
 * during the fight.  At frames 3/36 writes bright white/red colors; at 5/38
 * restores from aux_palette_buffer.  Also forces TS_copy = 2 each frame so
 * the translucent overlay layer remains enabled. */
...
/* Invisible floor layer effect (Skull Woods / Thieves' Town hidden tiles).
 * Counts dung_object_tilemap_pos[] entries with bit 0x8000 set (revealed tiles).
 * If any are visible, sets aux_palette_buffer[0x7B/0x7C] to a bright color pair
 * (0x2940/0x4E60) and schedules a CGRAM update.  If all are hidden, sets both to 0
 * (fully dark/invisible).  Also forces TS_copy = 2 each frame. */
...
/* Ganon's fight layer effect: counts lit torches (dung_object_tilemap_pos[] entries
 * with bit 0x8000 set) and stores in byte_7E04C5 for the sprite system to read.
 * Controls TS_copy and CGADSUB_copy based on the count:
 *   0 torches: TS=0 (sub-layer off), CGADSUB=0xB3 (standard dark-room color math).
 *   1 torch:   TS=2 (sub-layer on for translucency), CGADSUB=0x70 (half-subtraction).
 *   2+ torches: TS=0, CGADSUB=0x70 (floor illuminated but no overlay). */
...
/* Water-rapids layer effect (Eastern Palace / Swamp Palace current rooms).
 * Accumulates dung_some_subpixel[1] by 0x80 per frame; the integer part
 * (negated) becomes dung_floor_x_vel, which the sprite system uses to drift
 * sprites and Link along the current without actually moving BG2. */
...
/* Copy 0x80 bytes of per-dungeon tile attribute overrides into attributes_for_tile
 * starting at offset 0x140.  The source is kDungAttrsForTile indexed by
 * kDungAttrsForTile_Offs[aux_tile_theme_index], allowing each dungeon tileset
 * to override the default collision/attribute values for its custom tiles. */
...
/* Correct Link's player state after a wall-recoil event in the Dark World.
 * If Link's state is kPlayerState_RecoilWall, re-evaluates his bunny status:
 *   Not in Dark World (link_is_bunny_mirror == 0) → Ground state.
 *   Has Moon Pearl → TempBunny (temporarily human, returns to bunny on next update).
 *   No Moon Pearl  → PermaBunny (permanent bunny until leaving). */
...
/* Initialize the boss-defeat crystal rescue cutscene.
 * Configures CGADSUB for color-math translucency (0x33), clears the palette
 * filter, asserts the translucency palette swap, applies the crystal filter,
 * then writes the kCrystalMaiden_Pal gradient (8 entries starting at slot 112)
 * into main_palette_buffer and schedules a CGRAM update.
 * Spawns the maiden sprite and initializes the polyhedral crystal shape. */
...
/* Spawn the crystal maiden sprite for the boss-defeat cutscene.
 * Clears all sprite slots, dynamically spawns sprite 0xAB (maiden) near Link's
 * position, and sets its initial attributes (OAM flags, floor, subtype).
 * If in Hyrule Castle (cur_palace_index_x2 == 24), uses the Zelda-sprite OAM
 * flags (9) and follower type 1; otherwise uses type 6 (generic maiden).
 * Loads the follower graphics then immediately clears follower_indicator.
 * Computes dung_floor_{x,y}_offs to center the BG1 crystal overlay around Link. */
